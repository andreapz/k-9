package com.tiscali.appmail.ui.compose;
...
import com.tiscali.appmail.activity.MessageCompose.Action;
import com.tiscali.appmail.helper.HtmlConverter;
import com.tiscali.appmail.helper.QuotedMessageHelper;
...
import com.tiscali.appmail.mailstore.AttachmentResolver;
...
public class QuotedMessagePresenter {
    ...
    public QuotedMessagePresenter(MessageCompose messageCompose,
            QuotedMessageMvpView quotedMessageMvpView, Account account) {
     ...
     *
     * @param showQuotedText {@code true} if the quoted text should be shown, {@code false}
     *        otherwise.
     */
    public void populateUIWithQuotedMessage(MessageViewInfo messageViewInfo, boolean showQuotedText,
            Action action) throws MessagingException {
        ...
        if (forcePlainText || origMessageFormat == MessageFormat.TEXT) {
            ...
            quotedTextFormat = (MimeUtility.findFirstPartByMimeType(messageViewInfo.rootPart,
                    "text/html") == null) ? SimpleMessageFormat.TEXT : SimpleMessageFormat.HTML;
        } else {
            quotedTextFormat = SimpleMessageFormat.HTML;
        }

        // Handle the original message in the reply
        // If we already have sourceMessageBody, use that. It's pre-populated if we've got crypto
        // going on.
        String content = QuotedMessageHelper.getBodyTextFromMessage(messageViewInfo.rootPart,
                quotedTextFormat);
        ...
        if (quotedTextFormat == SimpleMessageFormat.HTML) {
            ...
            quotedHtmlContent = QuotedMessageHelper.quoteOriginalHtmlMessage(resources,
                    messageViewInfo.message, content, quoteStyle);
            ...
            view.setQuotedText(
                    QuotedMessageHelper.quoteOriginalTextMessage(resources, messageViewInfo.message,
                            QuotedMessageHelper.getBodyTextFromMessage(messageViewInfo.rootPart,
            ...
            view.setQuotedText(QuotedMessageHelper.quoteOriginalTextMessage(resources,
    ...
    public void builderSetProperties(MessageBuilder builder) {
        builder.setQuoteStyle(quoteStyle)
                ...
                .setQuotedText(view.getQuotedText()).setQuotedTextMode(quotedTextMode)
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        quotedTextFormat = (SimpleMessageFormat) savedInstanceState
    ...
    public void processDraftMessage(MessageViewInfo messageViewInfo,
            Map<IdentityField, String> k9identity) {
        ...
        if (k9identity.containsKey(IdentityField.CURSOR_POSITION)) {
            try {
                ...
                Log.e(K9.LOG_TAG, "Could not parse cursor position for MessageCompose; continuing.",
                        e);
            }
        }
        ...
        // TODO - The current implementation doesn't allow a user in HTML mode to edit a draft that
        // wasn't saved with K9mail.
        ...
        if (messageFormat == null) {
            ...
            view.setMessageContentCharacters(QuotedMessageHelper
                    .getBodyTextFromMessage(messageViewInfo.message, SimpleMessageFormat.TEXT));
            forcePlainText = true;
        ...
        if (messageFormat == MessageFormat.HTML) {
            ...
            if (part != null) { // Shouldn't happen if we were the one who saved it.
                ...
                if (K9.DEBUG) {
                    Log.d(K9.LOG_TAG, "Loading message with offset " + bodyOffset + ", length "
                            + bodyLength + ". Text length is " + text.length() + ".");
                }
            ...
            if (bodyPlainOffset != null && bodyPlainLength != null) {
                processSourceMessageText(messageViewInfo.rootPart, bodyPlainOffset, bodyPlainLength,
                        false);
            }
     ...
     */
    private void processSourceMessageText(Part rootMessagePart, int bodyOffset, int bodyLength,
        ...
        if (bodyLength != UNKNOWN_LENGTH) {
            try {
                ...
                if (bodyOffset == UNKNOWN_LENGTH
                    ...
                    quotedText.append(messageText.substring(0, bodyOffset)); // stuff before the
                                                                             // reply
