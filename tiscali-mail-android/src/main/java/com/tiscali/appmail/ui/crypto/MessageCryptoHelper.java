package com.tiscali.appmail.ui.crypto;
...
import com.tiscali.appmail.mailstore.CryptoResultAnnotation.CryptoError;
import com.tiscali.appmail.mailstore.LocalMessage;
import com.tiscali.appmail.mailstore.MessageHelper;
import com.tiscali.appmail.mailstore.MimePartStreamParser;
import com.tiscali.appmail.mailstore.util.FileFactory;
import com.tiscali.appmail.provider.DecryptedFileProvider;
...
public class MessageCryptoHelper {
    ...
    public MessageCryptoHelper(Context context, String openPgpProviderPackage) {
        ...
        if (openPgpProviderPackage == null
            ...
            throw new IllegalStateException(
                    "MessageCryptoHelper must only be called with a openpgp provider!");
        }
    }

    public void asyncStartOrResumeProcessingMessage(LocalMessage message,
            MessageCryptoCallback callback, OpenPgpDecryptionResult cachedDecryptionResult) {
    ...
    private void runSecondPass() {
        List<Part> signedParts =
                MessageDecryptVerifier.findSignedParts(currentMessage, messageAnnotations);
    ...
    private void processFoundEncryptedParts(List<Part> foundParts) {
        for (Part part : foundParts) {
            if (!MessageHelper.isCompletePartAvailable(part)) {
                addErrorAnnotation(part, CryptoError.OPENPGP_ENCRYPTED_BUT_INCOMPLETE,
            ...
            addErrorAnnotation(part, CryptoError.ENCRYPTED_BUT_UNSUPPORTED,
                    MessageHelper.createEmptyPart());
        }
    }
    ...
    private void processFoundSignedParts(List<Part> foundParts) {
        for (Part part : foundParts) {
            if (!MessageHelper.isCompletePartAvailable(part)) {
                ...
                addErrorAnnotation(part, CryptoError.OPENPGP_SIGNED_BUT_INCOMPLETE,
                        replacementPart);
                continue;
            }
            if (MessageDecryptVerifier.isPgpMimeEncryptedOrSignedPart(part)) {
    ...
    private void addErrorAnnotation(Part part, CryptoError error, MimeBodyPart replacementPart) {
        CryptoResultAnnotation annotation =
                CryptoResultAnnotation.createErrorAnnotation(error, replacementPart);
    ...
    private void connectToCryptoProviderService() {
        openPgpServiceConnection =
                new OpenPgpServiceConnection(context, openPgpProviderPackage, new OnBound() {
                    @Override
        ...
        return new OpenPgpDataSink<MimeBodyPart>() {
            ...
            public MimeBodyPart processData(InputStream is) throws IOException {
                try {
                    ...
                    TextBody body =
                            new TextBody(new String(decryptedByteOutputStream.toByteArray()));
    ...
    private void callAsyncDecrypt(Intent intent) throws IOException {
        ...
        cancelableBackgroundOperation = openPgpApi.executeApiAsync(intent, dataSource,
                openPgpDataSink, new IOpenPgpSinkResultCallback<MimeBodyPart>() {
                    @Override
            ...
            public void writeTo(OutputStream os) throws IOException {
                try {
                    Multipart multipartSignedMultipart =
                            (Multipart) currentCryptoPart.part.getBody();
            ...
            public void writeTo(OutputStream os) throws IOException {
                try {
                    ...
                    if (cryptoPartType == CryptoPartType.PGP_ENCRYPTED) {
                        ...
                        throw new IllegalStateException(
                                "part to stream must be encrypted or inline!");
                    }
                } catch (MessagingException e) {
                    Log.e(K9.LOG_TAG, "MessagingException while writing message to crypto provider",
                            e);
                }
            }
        };
        ...
        return new OpenPgpDataSink<MimeBodyPart>() {
            ...
            public MimeBodyPart processData(InputStream is) throws IOException {
                try {
                    ...
                    Log.e(K9.LOG_TAG, "Something went wrong while parsing the decrypted MIME part",
                            e);
    ...
    private void handleCryptoOperationResult(MimeBodyPart outputPart) {
        int resultCode = currentCryptoResult.getIntExtra(OpenPgpApi.RESULT_CODE,
                INVALID_OPENPGP_RESULT_CODE);
        if (K9.DEBUG) {
    ...
    private void handleCryptoOperationSuccess(MimeBodyPart outputPart) {
        ...
        CryptoResultAnnotation resultAnnotation =
                CryptoResultAnnotation.createOpenPgpResultAnnotation(decryptionResult,
                        signatureResult, pendingIntent, outputPart);
    ...
    private void propagateEncapsulatedSignedPart(CryptoResultAnnotation resultAnnotation,
            Part part) {
        Part encapsulatingPart = messageAnnotations.findKeyForAnnotationWithReplacementPart(part);
        CryptoResultAnnotation encapsulatingPartAnnotation =
                messageAnnotations.get(encapsulatingPart);
    ...
    private void onCryptoOperationFailed(OpenPgpError error) {
        CryptoResultAnnotation errorPart =
                CryptoResultAnnotation.createOpenPgpErrorAnnotation(error);
        addCryptoResultAnnotationToMessage(errorPart);
        onCryptoFinished();
    }
    ...
    private void deliverResult() {
        ...
        if (queuedResult != null) {
            ...
            callback.startPendingIntentForCryptoHelper(queuedPendingIntent.getIntentSender(),
                    REQUEST_CODE_USER_INTERACTION, null, 0, 0, 0);
    ...
    private enum CryptoPartType {
        PGP_INLINE, PGP_ENCRYPTED, PGP_SIGNED
    }
