...
public class AttachmentController {
    ...
    private Intent getBestViewIntentAndSaveFileIfNecessary() {
        ...
        if (!resolvedIntentInfo.hasResolvedActivities()) {
            resolvedIntentInfo =
                    getBestViewIntentForMimeType(MimeUtility.DEFAULT_ATTACHMENT_MIME_TYPE);
        }
        ...
        if (resolvedIntentInfo.hasResolvedActivities() && resolvedIntentInfo.containsFileUri()) {
            try {
                ...
                if (K9.DEBUG) {
                    Log.e(K9.LOG_TAG,
                            "Error while saving attachment to use file:// URI with ACTION_VIEW Intent",
                            e);
                }
                viewIntent = createViewIntentForAttachmentProviderUri(
    ...
    private Uri getAttachmentUriForMimeType(AttachmentViewInfo attachment, String mimeType) {
        ...
        return attachment.uri.buildUpon().appendPath(mimeType).build();
    }
    ...
    private class ViewAttachmentAsyncTask extends AsyncTask<Void, Void, Intent> {
        ...
        private void viewAttachment(Intent intent) {
            try {
                ...
                String message =
                        context.getString(R.string.message_view_no_viewer, attachment.mimeType);
                displayMessageToUser(message);
            }
        }
    }
