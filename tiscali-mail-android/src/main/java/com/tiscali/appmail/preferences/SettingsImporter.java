...
public class SettingsImporter {
     ...
     * @param overwrite {@code true} if existing accounts should be overwritten when an account with
     *        the same UUID is found in the settings file.<br>
            ...
            if (globalSettings) {
                try {
                    ...
                    if (editor.commit()) {
                        if (K9.DEBUG) {
                            Log.v(K9.LOG_TAG,
                                    "Committed global settings to the preference " + "storage.");
                        }
                            ...
                            try {
                                ...
                                if (editor.commit()) {
                                    ...
                                    if (!importResult.overwritten) {
                                        ...
                                        String oldAccountUuids =
                                                storage.getString("accountUuids", "");
                                ...
                                errorneousAccounts
                                        .add(new AccountDescription(account.name, account.uuid));
                            } catch (Exception e) {
    ...
    private static void importGlobalSettings(Storage storage, StorageEditor editor,
            int contentVersion, ImportedSettings settings) {

        // Validate global settings
        Map<String, Object> validatedSettings =
                GlobalSettings.validate(contentVersion, settings.settings);
    ...
    private static AccountDescriptionPair importAccount(Context context, StorageEditor editor,
            int contentVersion, ImportedAccount account, boolean overwrite)
        ...
        if (account.outgoing == null
                && !ServerSettings.Type.WebDAV.name().equals(account.incoming.type)) {
        ...
        if (account.outgoing != null) {
            ...
            putString(editor, accountKeyPrefix + Account.TRANSPORT_URI_KEY,
                    Base64.encode(transportUri));

            /*
             * Mark account as disabled if the settings file contained a username but no password.
             * However, no password is required for the outgoing server for WebDAV accounts, because
             * incoming and outgoing servers are identical for this account type. Nor is a password
             * required if the AuthType is EXTERNAL.
             */
        ...
        Map<String, Object> validatedSettings = AccountSettings.validate(contentVersion,
                account.settings.settings, !mergeImportedAccount);
    ...
    private static void importFolder(StorageEditor editor, int contentVersion, String uuid,
    ...
    private static void importIdentities(StorageEditor editor, int contentVersion, String uuid,
            ImportedAccount account, boolean overwrite, Account existingAccount, Preferences prefs)
            throws InvalidSettingValueException {
        ...
        for (ImportedIdentity identity : account.identities) {
            ...
            String identityDescription =
                    (identity.description == null) ? "Imported" : identity.description;
            ...
            if (identity.settings != null) {
                ...
                Map<String, Object> validatedSettings = IdentitySettings.validate(contentVersion,
                ...
                Map<String, String> writeSettings;
                if (mergeSettings) {
                    writeSettings = new HashMap<String, String>(IdentitySettings
                            .getIdentitySettings(prefs.getStorage(), uuid, writeIdentityIndex));
                    writeSettings.putAll(stringSettings);
                } else {
                    writeSettings = stringSettings;
                }
    ...
    private static Imported parseSettings(InputStream inputStream, boolean globalSettings,
            List<String> accountUuids, boolean overview) throws SettingsImportExportException {
    ...
    private static Imported parseRoot(XmlPullParser xpp, boolean globalSettings,
        ...
        String fileFormatVersionString =
                xpp.getAttributeValue(null, SettingsExporter.FILE_FORMAT_ATTRIBUTE);
        validateFileFormatVersion(fileFormatVersionString);

        String contentVersionString =
                xpp.getAttributeValue(null, SettingsExporter.VERSION_ATTRIBUTE);
                        ...
                        if (result.globalSettings == null) {
                            if (overview) {
                                ...
                                result.globalSettings =
                                        parseSettings(xpp, SettingsExporter.GLOBAL_ELEMENT);
                            }
                        } else {
                            skipToEndTag(xpp, SettingsExporter.GLOBAL_ELEMENT);
                            Log.w(K9.LOG_TAG,
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                if (SettingsExporter.VALUE_ELEMENT.equals(element)) {
                    ...
                    if (result.settings.containsKey(key)) {
                        Log.w(K9.LOG_TAG, "Already read key \"" + key + "\". Ignoring value \""
                                + value + "\"");
                    } else {
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (SettingsExporter.NAME_ELEMENT.equals(element)) {
                        ...
                        if (overview) {
                            ...
                            account.settings =
                                    parseSettings(xpp, SettingsExporter.SETTINGS_ELEMENT);
                        }
    ...
    private static class ImportedServerSettings extends ServerSettings {
        ...
        public ImportedServerSettings(ImportedServer server) {
            super(ServerSettings.Type.valueOf(server.type), server.host, convertPort(server.port),
                    convertConnectionSecurity(server.connectionSecurity), server.authenticationType,
                    server.username, server.password, server.clientCertificateAlias);
        ...
        private static ConnectionSecurity convertConnectionSecurity(String connectionSecurity) {
            try {
                /*
                 * TODO: Add proper settings validation and upgrade capability for server settings.
