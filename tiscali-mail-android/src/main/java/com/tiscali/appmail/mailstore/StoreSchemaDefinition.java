...
import com.tiscali.appmail.mailstore.migrations.Migrations;
import com.tiscali.appmail.mailstore.migrations.MigrationsHelper;
...
class StoreSchemaDefinition implements LockableDatabase.SchemaDefinition {
    ...
    private void upgradeDatabase(final SQLiteDatabase db) {
        Log.i(K9.LOG_TAG,
                String.format(Locale.US, "Upgrading database from version %d to version %d",
    ...
    private static void dbCreateDatabaseFromScratch(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE folders (" + "id INTEGER PRIMARY KEY," + "name TEXT, "
                ...
                + "status TEXT, " + "push_state TEXT, " + "last_pushed INTEGER, "
                + "flagged_count INTEGER default 0, " + "integrate INTEGER, "
                + "top_group INTEGER, " + "poll_class TEXT, " + "push_class TEXT, "
                + "display_class TEXT, " + "notify_class TEXT, "
        ...
        db.execSQL("CREATE TABLE messages (" + "id INTEGER PRIMARY KEY, "
                ...
                + "preview_type TEXT default \"none\", " + "preview TEXT, " + "mime_type TEXT, "
                + "normalized_subject_hash INTEGER, " + "empty INTEGER default 0, "
                + "read INTEGER default 0, " + "flagged INTEGER default 0, "
                + "answered INTEGER default 0, " + "forwarded INTEGER default 0, "
        ...
        db.execSQL(
                "CREATE INDEX IF NOT EXISTS msg_composite ON messages (deleted, empty,folder_id,flagged,read)");


        db.execSQL("DROP TABLE IF EXISTS threads");
        db.execSQL("CREATE TABLE threads (" + "id INTEGER PRIMARY KEY, " + "message_id INTEGER, "
                + "root INTEGER, " + "parent INTEGER" + ")");
        ...
        db.execSQL("CREATE TRIGGER set_thread_root " + "AFTER INSERT ON threads " + "BEGIN "
        ...
        db.execSQL("CREATE TRIGGER delete_message " + "BEFORE DELETE ON messages " + "BEGIN "
                + "DELETE FROM message_parts WHERE root = OLD.message_part_id;" + "END");
