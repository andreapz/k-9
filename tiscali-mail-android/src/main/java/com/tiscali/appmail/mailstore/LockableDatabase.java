...
public class LockableDatabase {
    ...
    /**
     * Callback interface for DB operations. Concept is similar to Spring HibernateCallback.
     *
     * @param <T> Return value type for {@link #doDbWork(SQLiteDatabase)}
     */
    public interface DbCallback<T> {
        /**
         * @param db The locked database on which the work should occur. Never <code>null</code>.
         * @return Any relevant data. Can be <code>null</code>.
         * @throws WrappedException
         * @throws com.tiscali.appmail.mail.MessagingException
         * @throws com.tiscali.appmail.mailstore.UnavailableStorageException
         */
    ...
    /**
     * Workaround exception wrapper used to keep the inner exception generated in a
     * {@link DbCallback}.
     */
        ...
        public void onUnmount(final String providerId) {
            ...
            if (K9.DEBUG) {
                Log.d(K9.LOG_TAG, "LockableDatabase: Closing DB " + uUid
                        + " due to unmount event on StorageProvider: " + providerId);
            }
        ...
        public void onMount(final String providerId) {
            ...
            if (K9.DEBUG) {
                Log.d(K9.LOG_TAG, "LockableDatabase: Opening DB " + uUid
                        + " due to mount event on StorageProvider: " + providerId);
            }
    ...
    /**
     * Reentrant write lock (if you lock it 2x from the same thread, you have to unlock it 2x to
     * release it)
     */
    ...
    /**
     * {@link ThreadLocal} to check whether a DB transaction is occuring in the current
     * {@link Thread}.
     *
     ...
     */
    public LockableDatabase(final Context context, final String uUid,
    ...
    /**
     * Lock the storage for shared operations (concurrent threads are allowed to run
     * simultaneously).
     *
     * <p>
     * You <strong>have to</strong> invoke {@link #unlockRead()} when you're done with the storage.
     * </p>
     *
    ...
    /**
     * Lock the storage for exclusive access (other threads aren't allowed to run simultaneously)
     *
     * <p>
     * You <strong>have to</strong> invoke {@link #unlockWrite()} when you're done with the storage.
     * </p>
     *
     * @throws UnavailableStorageException If storage can't be locked because it is not available.
     */
    ...
    /**
     * Execute a DB callback in a shared context (doesn't prevent concurrent shared executions),
     * taking care of locking the DB storage.
     *
     * <p>
     * Can be instructed to start a transaction if none is currently active in the current thread.
     * Callback will participe in any active transaction (no inner transaction created).
     * </p>
     *
     * @param transactional <code>true</code> the callback must be executed in a transactional
     *        context.
                    ...
                    if (debug) {
                        Log.v(K9.LOG_TAG,
                                "LockableDatabase: Transaction ended, took "
                                        ...
                                        + "ms / " + new Exception().getStackTrace()[1].toString());
                    }
                }
            }
        } finally {
    ...
    public void switchProvider(final String newProviderId) throws MessagingException {
        ...
        try {
            ...
            try {
                ...
                FileHelper.moveRecursive(oldDatabase,
    ...
    private void openOrCreateDataspace() throws UnavailableStorageException {
        ...
        try {
            ...
            try {
                ...
                Log.w(K9.LOG_TAG,
                        "Unable to open DB " + databaseFile + " - removing file and retrying", e);
                if (databaseFile.exists() && !databaseFile.delete()) {
                    Log.d(K9.LOG_TAG,
                            "Failed to remove " + databaseFile + " that couldn't be opened");
                }
                doOpenOrCreateDb(databaseFile);
            }
    ...
    protected File prepareStorage(final String providerId) throws UnavailableStorageException {
        ...
        if (databaseParentDir.isFile()) {
            // should be safe to unconditionally delete clashing file: user is not supposed to mess
            // with our directory
        ...
        try {
            ...
            try {
                final File attachmentDirectory =
                        storageManager.getAttachmentDirectory(uUid, mStorageProviderId);
                ...
                if (K9.DEBUG)
                    Log.d(K9.LOG_TAG,
                            "Exception caught in clearing attachments: " + e.getMessage());
            }
            try {
                deleteDatabase(storageManager.getDatabase(uUid, mStorageProviderId));
            } catch (Exception e) {
                Log.i(K9.LOG_TAG, "LockableDatabase: delete(): Unable to delete backing DB file",
                        e);
            }
