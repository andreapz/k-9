...
public class LocalMessage extends MimeMessage {
    ...
    /*
     * If a message is being marked as deleted we want to clear out its content. Delete will not
     * actually remove the row since we need to retain the UID for synchronization purposes.
     */
     ...
     *
     * @return Message ID of the parent message if there exists a parent and it is empty. Otherwise
     *         {@code -1}.
     */
    private long getEmptyThreadParent(SQLiteDatabase db, long messageId) {
        Cursor cursor = db.rawQuery(
                "SELECT m.id " + "FROM threads t1 " + "JOIN threads t2 ON (t1.parent = t2.id) "
    ...
    private boolean hasThreadChildren(SQLiteDatabase db, long messageId) {
        Cursor cursor = db.rawQuery(
                "SELECT COUNT(t2.id) " + "FROM threads t1 "
                        + "JOIN threads t2 ON (t2.parent = t1.id) " + "WHERE t1.message_id = ?",
                new String[] {Long.toString(messageId)});
    ...
    public MessageReference makeMessageReference() {
        if (mReference == null) {
            mReference = new MessageReference(getFolder().getAccountUuid(), getFolder().getName(),
                    mUid, null);
        }
        return mReference;
    }
    ...
    public String getUri() {
        return "email://messages/" + getAccount().getAccountNumber() + "/" + getFolder().getName()
                + "/" + getUid();
    }
    ...
    public boolean equals(Object o) {
        ...
        return thisAccountUuid != null ? thisAccountUuid.equals(thatAccountUuid)
                : thatAccountUuid == null;
    }
