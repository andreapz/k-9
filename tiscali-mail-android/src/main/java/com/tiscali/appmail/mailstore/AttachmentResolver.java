...
/**
 * This class is used to encapsulate a message part, providing an interface to get relevant info for
 * a given Content-ID URI.
 *
 * The point of this class is to keep the Content-ID loading code agnostic of the underlying part
 * structure.
 */
public class AttachmentResolver {
    ...
    public static AttachmentResolver createFromPart(Part part) {
        ...
        Map<String, Uri> contentIdToAttachmentUriMap =
                buildCidToAttachmentUriMap(attachmentInfoExtractor, part);
        return new AttachmentResolver(contentIdToAttachmentUriMap);
    }

    @VisibleForTesting
    static Map<String, Uri> buildCidToAttachmentUriMap(
            AttachmentInfoExtractor attachmentInfoExtractor, Part rootPart) {
