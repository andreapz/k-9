    ...
    public static void createFtsSearchTable(SQLiteDatabase db, MigrationsHelper migrationsHelper) {
        ...
        try {
            ...
            for (LocalFolder folder : folders) {
                Iterator<LocalMessage> localMessages =
                        new ArrayList<>(folder.getMessages(null, false)).iterator();
                while (localMessages.hasNext()) {
                    LocalMessage localMessage = localMessages.next();
                    // The LocalMessage objects are heavy once they have been loaded, so we free
                    // them asap
                    ...
                    if (!TextUtils.isEmpty(fulltext)) {
                        Log.d(K9.LOG_TAG, "fulltext for msg id " + localMessage.getId() + " is "
                                + fulltext.length() + " chars long");
                        cv.clear();
            ...
            Log.e(K9.LOG_TAG, "error indexing fulltext - skipping rest, fts index is incomplete!",
                    e);
        }
    }
}
