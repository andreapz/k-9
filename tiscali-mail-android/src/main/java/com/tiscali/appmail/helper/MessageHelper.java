...
public class MessageHelper {
    /**
     * If the number of addresses exceeds this value the addresses aren't resolved to the names of
     * Android contacts.
     *
     * <p>
     ...
     * </p>
     *
     * @see #toFriendly(Address[], com.tiscali.appmail.helper.Contacts)
     */
    ...
    public void populate(final MessageInfoHolder target, final LocalMessage message,
        ...
        if (addrs.length > 0 && account.isAnIdentity(addrs[0])) {
            ...
            target.sender =
                    new SpannableStringBuilder(mContext.getString(R.string.message_to_label))
                            .append(to);
        } else {
    ...
    /**
     * Returns the name of the contact this email address belongs to if the {@link Contacts
     * contacts} parameter is not {@code null} and a contact is found. Otherwise the personal
     * portion of the {@link Address} is returned. If that isn't available either, the email address
     * is returned.
     *
     * @param address An {@link com.tiscali.appmail.mail.Address}
    ...
    public static CharSequence toFriendly(Address address, Contacts contacts) {
        return toFriendly(address, contacts, K9.showCorrespondentNames(),
                K9.changeContactNameColor(), K9.getContactNameColor());
    }
    ...
    /* package, for testing */ static CharSequence toFriendly(Address address, Contacts contacts,
            boolean showCorrespondentNames, boolean changeContactNameColor, int contactNameColor) {
        if (!showCorrespondentNames) {
            ...
            if (name != null) {
                if (changeContactNameColor) {
                    ...
                    coloredName.setSpan(new ForegroundColorSpan(contactNameColor), 0,
                            coloredName.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    return coloredName;
                } else {
                    return name;
                }
