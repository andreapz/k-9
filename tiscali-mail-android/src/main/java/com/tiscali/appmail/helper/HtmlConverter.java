...
public class HtmlConverter {
    ...
    /**
     * Goegular expression to match all IANA top-level domains for WEB_URL. List accurate as of
     * 2011/01/12. List taken from: http://data.iana.org/TLD/tlds-alpha-by-domain.txt This pattern
     * is auto-generated by frameworks/base/common/tools/make-iana-tld-pattern.py
     */
    ...
    private static class HtmlToTextTagHandler implements Html.TagHandler {
        ...
        /**
         * When we come upon an ignored tag, we mark it with an Annotation object with a specific
         * key and value as above. We don't really need to be checking these values since
         * Html.fromHtml() doesn't use Annotation spans, but we should do it now to be safe in case
         * they do start using it in the future.
         * 
        ...
        private void handleIgnoredTag(boolean opening, Editable output) {
            ...
            if (opening) {
                output.setSpan(new Annotation(IGNORED_ANNOTATION_KEY, IGNORED_ANNOTATION_VALUE),
                ...
                if (start != null) {
                    ...
                    // Delete everything between the start of the Annotation and the end of the
                    // string
     ...
     * </p>
     * <p>
     * No HTML headers or footers are added to the result. Headers and footers are added at display
     * time in {@link com.tiscali.appmail.view#MessageWebView.setText(String)
    ...
    private static String simpleTextToHtml(String text) {
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            switch (c) {
                case '\n':
                    // pine treats <br> as two newlines, but <br/> as one newline. Use <br/> so our
                    // messages aren't
                    // doublespaced.
                    buff.append("<br />");
                    break;
    ...
    public static String textToHtml(String text) {
        ...
        // Make newlines at the end of blockquotes nicer by putting newlines beyond the first one
        // outside of the
        // blockquote.
        text = text.replaceAll("\\Q" + HTML_NEWLINE + "\\E((\\Q" + HTML_NEWLINE + "\\E)+?)\\Q"
                + HTML_BLOCKQUOTE_END + "\\E", HTML_BLOCKQUOTE_END + "$1");
    ...
    private static void appendbq(StringBuilder buff, int quotesThisLine, int quoteDepth) {
        ...
        if (quotesThisLine > quoteDepth) {
            for (int i = quoteDepth; i < quotesThisLine; i++) {
                buff.append(HTML_BLOCKQUOTE_START.replace(HTML_BLOCKQUOTE_COLOR_TOKEN,
                        getQuoteColor(i + 1)));
            }
    ...
    protected static void linkifyText(final String text, final StringBuffer outputBuffer) {
        ...
        while (m.find()) {
            ...
            if (start == 0 || (start != 0 && prepared.charAt(start - 1) != '@')) {
                if (m.group().indexOf(':') > 0) { // With no URI-schema we may get "http:/" links
    ...
    /*
     * Lightweight method to check whether the message contains emoji or not. Useful to avoid
     * calling the heavyweight convertEmoji2Img method. We don't use String.codePointAt here for
     * performance reasons.
     */
    ...
    public static String wrapMessageContent(CharSequence messageContent) {
        ...
        return "<html><head><meta name=\"viewport\" content=\"width=device-width\"/>"
                + HtmlConverter.cssStyleTheme() + HtmlConverter.cssStylePre() + "</head><body>"
     ...
     *
     * <p>
     * The style incorporates the user's current preference setting for the font family used for
     * plain text messages.
     * </p>
     *
     * @return A {@code <style>} element that can be dynamically included in the HTML {@code <head>}
     *         element when messages are displayed.
     */
    private static String cssStylePre() {
        final String font = K9.messageViewFixedWidthFont() ? "monospace" : "sans-serif";
        return "<style type=\"text/css\"> pre." + K9MAIL_CSS_CLASS
                + " {white-space: pre-wrap; word-wrap:break-word; " + "font-family: " + font
    ...
    public static String textToHtmlFragment(final String text) {
        ...
        // Linkify the message.
        StringBuffer linkified =
                new StringBuffer(htmlified.length() + TEXT_TO_HTML_EXTRA_BUFFER_LENGTH);
        linkifyText(htmlified, linkified);

        // Add newlines and unescaping.
        //
        // For some reason, TextUtils.htmlEncode escapes ' into &apos;, which is technically part of
        // the XHTML 1.0
