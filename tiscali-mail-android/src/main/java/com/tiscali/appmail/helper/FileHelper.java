...
public class FileHelper {
        ...
        try {
            if (!file.exists()) {
                ...
                if (!file.setLastModified(System.currentTimeMillis())) {
                    Log.d(K9.LOG_TAG,
                            "Unable to change last modification date: " + file.getAbsolutePath());
                }
            }
        } catch (Exception e) {
    ...
    public static boolean move(final File from, final File to) {
        ...
        if (!to.getParentFile().mkdirs()) {
            Log.d(K9.LOG_TAG,
                    "Unable to make directories: " + to.getParentFile().getAbsolutePath());
        }
        ...
        try {
            ...
            Log.w(K9.LOG_TAG,
                    "cannot move " + from.getAbsolutePath() + " to " + to.getAbsolutePath(), e);
            return false;
        }
    }
    ...
    public static void moveRecursive(final File fromDir, final File toDir) {
        ...
        if (!fromDir.isDirectory()) {
            if (toDir.exists()) {
                if (!toDir.delete()) {
                    Log.w(K9.LOG_TAG, "cannot delete already existing file/directory "
                            + toDir.getAbsolutePath());
                }
            }
            if (!fromDir.renameTo(toDir)) {
                Log.w(K9.LOG_TAG, "cannot rename " + fromDir.getAbsolutePath() + " to "
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                if (!file.renameTo(target)) {
                    Log.w(K9.LOG_TAG, "cannot rename " + file.getAbsolutePath() + " to "
