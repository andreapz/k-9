...
public class Utility {
    ...
    private static final Pattern TAG_PATTERN =
            Pattern.compile("\\[[-_a-z0-9]+\\] ", Pattern.CASE_INSENSITIVE);
    ...
    /**
     * Combines the given array of Objects into a single String using each Object's toString()
     * method and the separator character between each part.
     *
     * @param parts
     * @param separator
     * @return new String
    ...
    /**
     * Combines the given Objects into a single String using each Object's toString() method and the
     * separator character between each part.
     *
     * @param parts
     * @param separator
     * @return new String
     ...
     * allocations. This version is around 3x as fast as the standard one and I'm using it hundreds
     * of times in places that slow down the UI, so it helps.
     */
    ...
    /*
     * TODO disabled this method globally. It is used in all the settings screens but I just noticed
     * that an unrelated icon was dimmed. Android must share drawables internally.
     */
     ...
     * /trunk/src/main/java/org/apache/commons/lang3/text/WordUtils.java SVN Revision 925967, Mon
     * Mar 22 06:16:49 2010 UTC
     *
    ...
    /**
     * Extract the 'original' subject value, by ignoring leading response/forward marker and '[XX]'
     * formatted tags (as many mailing-list softwares do).
     *
     * <p>
    ...
    public static String stripSubject(final String subject) {
        ...
        while (lastPrefix < subject.length() - 1 && matcher.find(lastPrefix)
                && matcher.start() == lastPrefix && (!tagPresent || tag == null
                        || subject.regionMatches(matcher.end(), tag, 0, tag.length()))) {
    ...
    /**
     * Figure out if this part has images. TODO: should only return true if we're an html part
     * 
    ...
    /**
     * Unconditionally close a Cursor. Equivalent to {@link Cursor#close()}, if cursor is non-null.
     ...
     *
     * On 4.3, we pass the address name as extra info so that if the contact doesn't exist the name
     * is auto-populated.
     *
