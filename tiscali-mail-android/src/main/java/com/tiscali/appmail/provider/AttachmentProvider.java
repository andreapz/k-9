...
import com.tiscali.appmail.mailstore.LocalStore.AttachmentInfo;
...
public class AttachmentProvider extends ContentProvider {
    ...
    public static Uri getAttachmentUri(String accountUuid, long id) {
        return CONTENT_URI.buildUpon().appendPath(accountUuid).appendPath(Long.toString(id))
                .build();
    }
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        try {
            final Account account =
                    Preferences.getPreferences(getContext()).getAccount(accountUuid);
            attachmentInfo = LocalStore.getInstance(account, getContext()).getAttachmentInfo(id);
        } catch (MessagingException e) {
            Log.e(K9.LOG_TAG, "Unable to retrieve attachment info from local store for ID: " + id,
                    e);
            return null;
        }
    ...
    @Nullable
    private InputStream getAttachmentInputStream(String accountUuid, String attachmentId)
