...
import com.tiscali.appmail.cache.EmailProviderCacheCursor;
...
import com.tiscali.appmail.mailstore.LockableDatabase;
...
public class EmailProvider extends ContentProvider {
    ...
    private static final Map<String, String> THREAD_AGGREGATION_FUNCS =
    ...
    private static final String[] FIXUP_AGGREGATED_MESSAGES_COLUMNS = {MessageColumns.DATE,
            MessageColumns.INTERNAL_DATE, MessageColumns.ATTACHMENT_COUNT, MessageColumns.READ,
            MessageColumns.FLAGGED, MessageColumns.ANSWERED, MessageColumns.FORWARDED};

    private static final String FOLDERS_TABLE = "folders";

    private static final String[] FOLDERS_COLUMNS = {FolderColumns.ID, FolderColumns.NAME,
            FolderColumns.LAST_UPDATED, FolderColumns.UNREAD_COUNT, FolderColumns.VISIBLE_LIMIT,
            FolderColumns.STATUS, FolderColumns.PUSH_STATE, FolderColumns.LAST_PUSHED,
            FolderColumns.FLAGGED_COUNT, FolderColumns.INTEGRATE, FolderColumns.TOP_GROUP,
            FolderColumns.POLL_CLASS, FolderColumns.PUSH_CLASS, FolderColumns.DISPLAY_CLASS};
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case MESSAGES_THREAD: {
                ...
                Uri notificationUri =
                        Uri.withAppendedPath(CONTENT_URI, "account/" + accountUuid + "/messages");
            ...
            return database.execute(false, new DbCallback<Cursor>() {
                ...
                public Cursor doDbWork(SQLiteDatabase db)
                    ...
                    if (TextUtils.isEmpty(selection)) {
                        ...
                        where = "(" + selection + ") AND " + InternalMessageColumns.DELETED
                                + " = 0 AND " + InternalMessageColumns.EMPTY + " = 0";
                    }
                    ...
                    if (Utility.arrayContainsAny(projection, (Object[]) FOLDERS_COLUMNS)) {
                        ...
                        query.append(" FROM messages m "
                                ...
                                + "LEFT JOIN folders f ON (m.folder_id = f.id) " + "WHERE ");
        ...
        try {
            return database.execute(false, new DbCallback<Cursor>() {
                ...
                public Cursor doDbWork(SQLiteDatabase db)
                    ...
                    query.append("JOIN " + THREADS_TABLE + " t " + "ON (t." + ThreadColumns.ROOT
                            + " = a.thread_root) " + "JOIN " + MESSAGES_TABLE + " m " + "ON (m."
                            + MessageColumns.ID + " = t." + ThreadColumns.MESSAGE_ID + " AND "
                            + "m." + InternalMessageColumns.EMPTY + "=0 AND " + "m."
                            + InternalMessageColumns.DELETED + "=0 AND " + "m."
                            + MessageColumns.DATE + " = a." + MessageColumns.DATE + ") ");

                    if (Utility.arrayContainsAny(projection, (Object[]) FOLDERS_COLUMNS)) {
                        query.append("JOIN " + FOLDERS_TABLE + " f " + "ON (m."
    ...
    private void createThreadedSubQuery(String[] projection, String selection,
        ...
        for (String columnName : projection) {
            ...
            if (SpecialColumns.THREAD_COUNT.equals(columnName)) {
                query.append(
                        ",COUNT(t." + ThreadColumns.ROOT + ") AS " + SpecialColumns.THREAD_COUNT);
            } else if (aggregationFunc != null) {
                query.append(",");
        ...
        query.append(" FROM " + MESSAGES_TABLE + " m " + "JOIN " + THREADS_TABLE + " t " + "ON (t."
                + ThreadColumns.MESSAGE_ID + " = m." + MessageColumns.ID + ")");

        if (Utility.arrayContainsAny(projection, (Object[]) FOLDERS_COLUMNS)) {
            query.append(" JOIN " + FOLDERS_TABLE + " f " + "ON (m." + MessageColumns.FOLDER_ID
                    + " = f." + FolderColumns.ID + ")");
        }

        query.append(" WHERE (t." + ThreadColumns.ROOT + " IN (" + "SELECT " + ThreadColumns.ROOT
                + " " + "FROM " + MESSAGES_TABLE + " m " + "JOIN " + THREADS_TABLE + " t "
                + "ON (t." + ThreadColumns.MESSAGE_ID + " = m." + MessageColumns.ID + ") "
                + "WHERE " + "m." + InternalMessageColumns.EMPTY + " = 0 AND " + "m."
        ...
        query.append(") AND " + InternalMessageColumns.DELETED + " = 0 AND "
                + InternalMessageColumns.EMPTY + " = 0");
        ...
        try {
            return database.execute(false, new DbCallback<Cursor>() {
                ...
                public Cursor doDbWork(SQLiteDatabase db)
                    ...
                    query.append(" ORDER BY ");
                    query.append(SqlQueryBuilder.addPrefixToSelection(FIXUP_MESSAGES_COLUMNS, "m.",
                            sortOrder));
    ...
    static class SpecialColumnsCursor extends CursorWrapper {
        ...
        public SpecialColumnsCursor(Cursor cursor, String[] allColumnNames,
            ...
            for (int i = 0, columnIndex = 0, specialColumnCount = 0, len =
                    allColumnNames.length; i < len; i++) {
