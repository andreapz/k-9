...
import com.tiscali.appmail.activity.MessageInfoHolder;
...
public class MessageProvider extends ContentProvider {
    ...
    /**
     * Extracts the {@link LocalMessage#getId() ID} from the given {@link MessageInfoHolder}. The
     * underlying {@link Message} is expected to be a {@link LocalMessage}.
     */
    ...
    public static class DeleteUriExtractor implements FieldExtractor<MessageInfoHolder, String> {
        ...
        public String getField(final MessageInfoHolder source) {
            ...
            return CONTENT_URI.buildUpon().appendPath("delete_message")
                    .appendPath(Integer.toString(accountNumber))
                    .appendPath(message.getFolder().getName()).appendPath(message.getUid()).build()
                    .toString();
        }
    }
    ...
    public static class SenderAddressExtractor
            implements FieldExtractor<MessageInfoHolder, String> {
        @Override
        public String getField(final MessageInfoHolder source) {
    ...
    public static class AccountColorExtractor
            implements FieldExtractor<MessageInfoHolder, Integer> {
        @Override
    ...
    public static class AccountNumberExtractor
            implements FieldExtractor<MessageInfoHolder, Integer> {
        @Override
        public Integer getField(final MessageInfoHolder source) {
    ...
    public static class HasAttachmentsExtractor
            implements FieldExtractor<MessageInfoHolder, Boolean> {
        @Override
    ...
    /**
     * @deprecated having an incremental value has no real interest, implemented for compatibility
     *             only
     */
    @Deprecated
    // TODO remove
    ...
    /**
     * Cursor wrapper that release a semaphore on close. Close is also triggered on
     * {@link #finalize()}.
     */
    protected static class MonitoredCursor implements CrossProcessCursor {
         ...
         * @param semaphore The semaphore to release on close. Never <code>null</code>.
         */
    ...
    protected class ThrottlingQueryHandler implements QueryHandler {
        ...
        public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
            ...
            final MonitoredCursor wrapped =
                    new MonitoredCursor((CrossProcessCursor) cursor, mSemaphore);
    ...
    /**
     * Synchronized listener used to retrieve {@link MessageInfoHolder}s using a given
     * {@link BlockingQueue}.
     */
    protected class MesssageInfoHolderRetrieverListener extends MessagingListener {
        ...
        /**
         * @param queue Never <code>null</code>. The synchronized channel to use to retrieve
         *        {@link MessageInfoHolder}s.
         */
    ...
    private static final String[] DEFAULT_MESSAGE_PROJECTION =
            new String[] {MessageColumns._ID, MessageColumns.SEND_DATE, MessageColumns.SENDER,
                    MessageColumns.SUBJECT, MessageColumns.PREVIEW, MessageColumns.ACCOUNT,
                    MessageColumns.URI, MessageColumns.DELETE_URI, MessageColumns.SENDER_ADDRESS};

    /**
     * URI matcher used for {@link #query(Uri, String[], String, String[], String)}
     */
    private UriMatcher mUriMatcher = new UriMatcher(UriMatcher.NO_MATCH);

    /**
     * Handlers registered to respond to {@link #query(Uri, String[], String, String[], String)}
     */
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        if (myAccount != null) {
            MessageReference messageReference =
                    new MessageReference(myAccount.getUuid(), folderName, msgUid, null);
