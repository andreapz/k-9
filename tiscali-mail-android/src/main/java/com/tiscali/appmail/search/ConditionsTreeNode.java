package com.tiscali.appmail.search;
...
/**
 * This class stores search conditions. It's basically a boolean expression binary tree. The output
 * will be SQL queries ( obtained by traversing inorder ).
 *
...
public class ConditionsTreeNode implements Parcelable {
    ...
    /*
     * If mValue isn't CONDITION then mCondition contains a real condition, otherwise it's null.
     */
    public Operator mValue;
    public SearchCondition mCondition;

    /*
     * Used for storing and retrieving the tree to/from the database. The algorithm is called
     ...
     *
     * @param cursor Cursor pointing to the first of a bunch or rows. Each rows should contains 1
     *        tree node.
    ...
    /**
     * Convenience method. Adds the provided condition as the second argument of an AND clause to
     * this node.
     *
    ...
    /**
     * Convenience method. Adds the provided condition as the second argument of an OR clause to
     * this node.
     *
     ...
     *
     * For a description on MPTT see: http://www.sitepoint.com/hierarchical-data-database-2/
     */
    ...
    /**
     * Returns a list of all the nodes in the subtree of which this node is the root. The list
     * contains the nodes in a pre traversal order.
     *
    ...
    ///////////////////////////////////////////////////////////////
    /**
     * Adds two new ConditionTreeNodes, one for the operator and one for the new condition. The
     * current node will end up on the same level as the one provided in the arguments, they will be
     * siblings. Their common parent node will be one containing the operator provided in the
     * arguments. The method will update all the required references so the tree ends up in a valid
     * state.
     *
    ...
    /**
     * Helper method that replaces a child of the current node with a new node. If the provided old
     * child node was the left one, left will be replaced with the new one. Same goes for the right
     * one.
     *
