...
public class TemporaryAttachmentStore {
    ...
    private static File createOrCleanAttachmentDirectory(Context context) throws IOException {
        ...
        if (directory.exists()) {
            ...
            if (!directory.mkdir()) {
                throw new IOException("Couldn't create temporary attachment store: "
        ...
        for (File file : files) {
            if (file.lastModified() < cutOffTime) {
                if (file.delete()) {
                    if (K9.DEBUG) {
                        Log.d(K9.LOG_TAG,
                                "Deleted from temporary attachment store: " + file.getName());
                    }
                } else {
                    Log.w(K9.LOG_TAG,
                            "Couldn't delete from temporary attachment store: " + file.getName());
                }
            }
        }
    }
