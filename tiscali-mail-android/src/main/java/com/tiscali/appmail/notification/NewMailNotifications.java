 ...
 * <p>
 * We call the notification shown on the device <em>summary notification</em>, even when there's
 * only one new message. Notifications on an Android Wear device are displayed as a stack of cards
 * and that's why we call them <em>stacked notifications</em>. We have to keep track of stacked
 * notifications individually and recreate/update the summary notification when one or more of the
 * stacked notifications are added/removed.<br>
 * {@link NotificationData} keeps track of all data required to (re)create the actual system
 * notifications.
 * </p>
 */
class NewMailNotifications {
    ...
    NewMailNotifications(NotificationController controller,
    ...
    public static NewMailNotifications newInstance(NotificationController controller,
        ...
        NotificationContentCreator contentCreator =
                new NotificationContentCreator(controller.getContext());
        ...
        return new NewMailNotifications(controller, contentCreator, deviceNotifications,
                wearNotifications);
    }
    ...
    public void addNewMailNotification(Account account, LocalMessage message,
        ...
        synchronized (lock) {
            NotificationData notificationData =
                    getOrCreateNotificationData(account, unreadMessageCount);
    ...
    public void removeNewMailNotification(Account account, MessageReference messageReference) {
        synchronized (lock) {
            ...
            RemoveNotificationResult result =
                    notificationData.removeNotificationForMessage(messageReference);
    ...
    private NotificationData getOrCreateNotificationData(Account account, int unreadMessageCount) {
        ...
        NotificationData newNotificationHolder =
                createNotificationData(account, unreadMessageCount);
    ...
    private void createSummaryNotification(Account account, NotificationData notificationData,
            boolean silent) {
        Notification notification =
                deviceNotifications.buildSummaryNotification(account, notificationData, silent);
        int notificationId = NotificationIds.getNewMailSummaryNotificationId(account);
