...
import static com.tiscali.appmail.notification.NotificationController.NOTIFICATION_LED_BLINK_FAST;
import static com.tiscali.appmail.notification.NotificationController.NOTIFICATION_LED_FAILURE_COLOR;

import com.tiscali.appmail.Account;
import com.tiscali.appmail.R;
import com.tiscali.appmail.activity.setup.AccountSetupIncoming;
import com.tiscali.appmail.activity.setup.AccountSetupOutgoing;
...
class AuthenticationErrorNotifications {
    ...
    public void showAuthenticationErrorNotification(Account account, boolean incoming) {
        int notificationId =
                NotificationIds.getAuthenticationErrorNotificationId(account, incoming);
        Context context = controller.getContext();

        PendingIntent editServerSettingsPendingIntent =
                createContentIntent(context, account, incoming);
        String title = context.getString(R.string.notification_authentication_error_title);
        String text = context.getString(R.string.notification_authentication_error_text,
        ...
        NotificationCompat.Builder builder = controller.createNotificationBuilder()
                ...
                .setWhen(System.currentTimeMillis()).setAutoCancel(true).setTicker(title)
    ...
    PendingIntent createContentIntent(Context context, Account account, boolean incoming) {
        Intent editServerSettingsIntent =
                incoming ? AccountSetupIncoming.intentActionEditIncomingSettings(context, account)
        ...
        return PendingIntent.getActivity(context, account.getAccountNumber(),
                editServerSettingsIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    }
