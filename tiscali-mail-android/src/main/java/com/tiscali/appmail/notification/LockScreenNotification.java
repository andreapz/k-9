...
class LockScreenNotification {
    ...
    public void configureLockScreenNotification(Builder builder,
        ...
        switch (K9.getLockScreenNotificationVisibility()) {
            ...
            case SENDERS: {
                Notification publicNotification =
                        createPublicNotificationWithSenderList(notificationData);
                builder.setPublicVersion(publicNotification);
                break;
            }
            case MESSAGE_COUNT: {
                Notification publicNotification =
                        createPublicNotificationWithNewMessagesCount(notificationData);
                builder.setPublicVersion(publicNotification);
                break;
            }
        }
    ...
    private Notification createPublicNotificationWithSenderList(NotificationData notificationData) {
        ...
        if (newMessages == 1) {
            ...
            List<NotificationContent> contents =
                    notificationData.getContentForSummaryNotification();
    ...
    private Notification createPublicNotificationWithNewMessagesCount(
            NotificationData notificationData) {
        Builder builder = createPublicNotification(notificationData);
    ...
    private Builder createPublicNotification(NotificationData notificationData) {
        ...
        return controller.createNotificationBuilder()
                ...
                .setColor(account.getChipColor()).setNumber(unreadCount).setContentTitle(title);
    }


    String createCommaSeparatedListOfSenders(List<NotificationContent> contents) {
        // Use a LinkedHashSet so that we preserve ordering (newest to oldest), but still remove
        // duplicates
        Set<CharSequence> senders =
                new LinkedHashSet<CharSequence>(MAX_NUMBER_OF_SENDERS_IN_LOCK_SCREEN_NOTIFICATION);
