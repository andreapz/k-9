...
import com.tiscali.appmail.service.CoreService;
...
public class NotificationActionService extends CoreService {
    ...
    static Intent createMarkMessageAsReadIntent(Context context,
    ...
    static Intent createMarkMessageAsSpamIntent(Context context,
            MessageReference messageReference) {
        Intent intent = new Intent(context, NotificationActionService.class);
    ...
    private static ArrayList<MessageReference> createSingleItemArrayList(
            MessageReference messageReference) {
    ...
    private void markMessagesAsRead(Intent intent, Account account,
        ...
        List<MessageReference> messageReferences =
                intent.getParcelableArrayListExtra(EXTRA_MESSAGE_REFERENCES);
        for (MessageReference messageReference : messageReferences) {
            controller.setFlag(account, messageReference.getFolderName(), messageReference.getUid(),
                    Flag.SEEN, true);
        }
    }
    ...
    private void archiveMessages(Intent intent, Account account, MessagingController controller) {
        ...
        for (MessageReference messageReference : messageReferences) {
            if (controller.isMoveCapable(messageReference)) {
                ...
                controller.moveMessage(account, sourceFolderName, messageReference,
                        archiveFolderName);
            }
        }
    }
    ...
    private void cancelNotifications(Intent intent, Account account,
            MessagingController controller) {
