...
class SyncNotifications {
    ...
    public SyncNotifications(NotificationController controller,
    ...
    public void showSendingNotification(Account account) {
        ...
        PendingIntent showMessageListPendingIntent = actionBuilder
                .createViewFolderPendingIntent(account, outboxFolderName, notificationId);
        ...
        NotificationCompat.Builder builder = controller.createNotificationBuilder()
                ...
                .setOngoing(true).setTicker(tickerText).setContentTitle(title)
                .setContentText(accountName).setContentIntent(showMessageListPendingIntent)
                .setVisibility(NotificationCompat.VISIBILITY_PUBLIC);

        if (NOTIFICATION_LED_WHILE_SYNCING) {
            controller.configureNotification(builder, null, null,
                    account.getNotificationSetting().getLedColor(), NOTIFICATION_LED_BLINK_FAST,
                    true);
        }
    ...
    public void showFetchingMailNotification(Account account, Folder folder) {
        ...
        String text = accountName + context.getString(R.string.notification_bg_title_separator)
                + folderName;

        int notificationId = NotificationIds.getFetchingMailNotificationId(account);
        PendingIntent showMessageListPendingIntent =
                actionBuilder.createViewFolderPendingIntent(account, folderName, notificationId);
