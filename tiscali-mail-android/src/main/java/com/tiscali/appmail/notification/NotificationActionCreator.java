...
/**
 * This class contains methods to create the {@link PendingIntent}s for the actions of new mail
 * notifications.
 * <p/>
 * <strong>Note:</strong> We need to take special care to ensure the {@code PendingIntent}s are
 * unique as defined in the documentation of {@link PendingIntent}. Otherwise selecting a
 * notification action might perform the action on the wrong message.
 * <p/>
 * We use the notification ID as {@code requestCode} argument to ensure each notification/action
 * pair gets a unique {@code PendingIntent}.
 */
class NotificationActionCreator {
    ...
    public PendingIntent createViewMessagesPendingIntent(Account account,
    ...
    public PendingIntent createDismissMessagePendingIntent(Context context,
            MessageReference messageReference, int notificationId) {

        Intent intent =
                NotificationActionService.createDismissMessageIntent(context, messageReference);
    ...
    public PendingIntent createMarkMessageAsReadPendingIntent(MessageReference messageReference,
            int notificationId) {
        Intent intent =
                NotificationActionService.createMarkMessageAsReadIntent(context, messageReference);
    ...
    public PendingIntent getMarkAllAsReadPendingIntent(Account account,
            ArrayList<MessageReference> messageReferences, int notificationId) {
        return getMarkAsReadPendingIntent(account, messageReferences, notificationId, context,
                PendingIntent.FLAG_NO_CREATE);
    }

    private PendingIntent getMarkAsReadPendingIntent(Account account,
            ArrayList<MessageReference> messageReferences, int notificationId, Context context,
            int flags) {
        String accountUuid = account.getUuid();
        Intent intent = NotificationActionService.createMarkAllAsReadIntent(context, accountUuid,
                messageReferences);
    ...
    private PendingIntent createDeleteServicePendingIntent(MessageReference messageReference,
            int notificationId) {
        Intent intent =
                NotificationActionService.createDeleteMessageIntent(context, messageReference);
    ...
    private PendingIntent getDeleteAllConfirmationPendingIntent(
    ...
    private PendingIntent getDeleteAllServicePendingIntent(Account account,
            ArrayList<MessageReference> messageReferences, int notificationId, int flags) {
        String accountUuid = account.getUuid();
        Intent intent = NotificationActionService.createDeleteAllMessagesIntent(context,
                accountUuid, messageReferences);
    ...
    public PendingIntent createArchiveMessagePendingIntent(MessageReference messageReference,
            int notificationId) {
        Intent intent =
                NotificationActionService.createArchiveMessageIntent(context, messageReference);
    ...
    public PendingIntent createArchiveAllPendingIntent(Account account,
            ArrayList<MessageReference> messageReferences, int notificationId) {
        Intent intent = NotificationActionService.createArchiveAllIntent(context, account,
                messageReferences);
    ...
    public PendingIntent createMarkMessageAsSpamPendingIntent(MessageReference messageReference,
            int notificationId) {
        Intent intent =
                NotificationActionService.createMarkMessageAsSpamIntent(context, messageReference);
    ...
    private TaskStackBuilder buildMessageListBackStack(Account account, String folderName) {
        ...
        Intent intent =
                NavigationDrawerActivity.intentDisplaySearch(context, search, false, true, true);
    ...
    private TaskStackBuilder buildMessageViewBackStack(MessageReference message) {
        ...
        Intent intent = NavigationDrawerActivity.actionDisplayMessageIntent(context, message);
