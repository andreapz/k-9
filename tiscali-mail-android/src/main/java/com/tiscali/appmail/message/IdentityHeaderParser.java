...
public class IdentityHeaderParser {
     ...
     *
     * @param identityString The encoded identity string that was saved in a drafts header.
     *
    ...
    public static Map<IdentityField, String> parse(final String identityString) {
        ...
        if (identityString.charAt(0) == IdentityField.IDENTITY_VERSION_1.charAt(0)
                && identityString.length() > 2) {
            Uri.Builder builder = new Uri.Builder();
            builder.encodedQuery(identityString.substring(1)); // Need to cut off the ! at the
                                                               // beginning.
            ...
            for (IdentityField key : IdentityField.getIntegerFields()) {
                if (identity.get(key) != null) {
                    try {
                        ...
                        Log.e(K9.LOG_TAG, "Invalid " + key.name() + " field in identity: "
                                + identity.get(key));
                    }
                }
            }
            ...
            // First item is the body length. We use this to separate the composed reply from the
            // quoted text.
