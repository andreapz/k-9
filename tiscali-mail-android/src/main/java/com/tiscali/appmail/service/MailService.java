...
public class MailService extends CoreService {
    private static final String ACTION_CHECK_MAIL =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_WAKEUP";
    private static final String ACTION_RESET =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_RESET";
    private static final String ACTION_RESCHEDULE_POLL =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_RESCHEDULE_POLL";
    private static final String ACTION_CANCEL =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_CANCEL";
    private static final String ACTION_REFRESH_PUSHERS =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_REFRESH_PUSHERS";
    private static final String ACTION_RESTART_PUSHERS =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_RESTART_PUSHERS";
    private static final String CONNECTIVITY_CHANGE =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_CONNECTIVITY_CHANGE";
    private static final String CANCEL_CONNECTIVITY_NOTICE =
            "com.tiscali.appmail.intent.action.MAIL_SERVICE_CANCEL_CONNECTIVITY_NOTICE";
    ...
    public static void actionCancel(Context context, Integer wakeLockId) {
        ...
        addWakeLockId(context, i, wakeLockId, false); // CK:Q: why should we not create a wake lock
                                                      // if one is not already existing like for
                                                      // example in actionReschedulePoll?
        context.startService(i);
    }
    ...
    public int startService(Intent intent, int startId) {
        ...
        if (K9.DEBUG)
            Log.i(K9.LOG_TAG,
                    "MailService.onStart(" + intent + ", " + startId + "), hasConnectivity = "
        ...
        if (ACTION_CHECK_MAIL.equals(intent.getAction())) {
            ...
            rescheduleAllInBackground(hasConnectivity, doBackground, startId);
            if (K9.DEBUG)
                Log.i(K9.LOG_TAG, "Got connectivity action with hasConnectivity = "
                        + hasConnectivity + ", doBackground = " + doBackground);
        ...
        if (K9.DEBUG)
            Log.i(K9.LOG_TAG,
                    "MailService.onStart took " + (System.currentTimeMillis() - startTime) + "ms");
    ...
    private void reschedulePoll(final boolean hasConnectivity, final boolean doBackground,
        ...
        if (shortestInterval == -1) {
            if (K9.DEBUG) {
                Log.i(K9.LOG_TAG,
                        "No next check scheduled for package " + getApplication().getPackageName());
            }
            ...
            long base = (previousInterval == -1 || lastCheckEnd == -1 || !considerLastCheckEnd
                    ? System.currentTimeMillis() : lastCheckEnd);
            long nextTime = base + delay;

            if (K9.DEBUG) {
                Log.i(K9.LOG_TAG,
                        "previousInterval = " + previousInterval + ", shortestInterval = "
                                + shortestInterval + ", lastCheckEnd = " + new Date(lastCheckEnd)
            ...
            try {
                if (K9.DEBUG) {
                    Log.i(K9.LOG_TAG, "Next check for package " + getApplication().getPackageName()
    ...
    private void setupPushers() {
        ...
        for (Account account : Preferences.getPreferences(MailService.this).getAccounts()) {
            ...
            if (account.isEnabled() && account.isAvailable(getApplicationContext())) {
                ...
                // TODO: setupPushing of unavailable accounts when they become available (sd-card
                // inserted)
            }
        }
        if (pushing) {
    ...
    private void refreshPushers() {
        try {
            ...
            Collection<Pusher> pushers =
                    MessagingController.getInstance(getApplication()).getPushers();
            for (Pusher pusher : pushers) {
                ...
                if (sinceLast + 10000 > refreshInterval) { // Add 10 seconds to keep pushers in
                                                           // sync, avoid drift
                    if (K9.DEBUG) {
                        Log.d(K9.LOG_TAG,
                                "PUSHREFRESH: refreshing lastRefresh = " + lastRefresh
                                        + ", interval = " + refreshInterval + ", nowTime = "
                                        + nowTime + ", sinceLast = " + sinceLast);
                    }
                    pusher.refresh();
                    ...
                    if (K9.DEBUG) {
                        Log.d(K9.LOG_TAG,
                                "PUSHREFRESH: NOT refreshing lastRefresh = " + lastRefresh
                                        + ", interval = " + refreshInterval + ", nowTime = "
                                        + nowTime + ", sinceLast = " + sinceLast);
                    }
                }
