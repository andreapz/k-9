 ...
 * processing or some just some monitoring, like listening on a network port for incoming
 * connections or listing on an open network connection for incoming data (push mechanism).
 * </p>
 * <p>
 * To make sure the service is running only when required, is must be shutdown after tasks are done.
 * As the execution of tasks is abstracted away in this class, it also handles proper shutdown. If a
 * Service doesn't want this, it needs to call {@code enableAutoShutdown(true)} in its
 * {@link Service#onCreate()} method.
 * </p>
 * <p>
 * While a service is running its tasks, it is usually not a good idea to let the device go to sleep
 * mode. Wake locks are used to avoid this. CoreService provides a central registry (singleton) that
 * can be used application-wide to store wake locks.
 * </p>
 * <p>
...
public abstract class CoreService extends Service {
    ...
    public static final String WAKE_LOCK_ID = "com.tiscali.appmail.service.CoreService.wakeLockId";
     ...
     *
     * <p>
     * <strong>Note:</strong> This is used to ignore (expected) {@link RejectedExecutionException}s
     * thrown by {@link ExecutorService#execute(Runnable)} after the service (and with that, the
     * thread pool) was shut down.
     * </p>
     */
     ...
     *
     * <p>
     * The default service life-cycle model is that the service should run only as long as a task is
     * running. If a service should behave differently, disable auto shutdown using
     ...
     *
     * @param context A {@link Context} instance. Never {@code null}.
     * @param intent The {@link Intent} to add the wake lock registy ID as extra to. Never
     *        {@code null}.
     * @param wakeLockId The wake lock registry ID of an existing wake lock or {@code null}.
     * @param createIfNotExists If {@code wakeLockId} is {@code null} and this parameter is
     *        {@code true} a new wake lock is created, registered, and added to {@code intent}.
     */
     ...
     *
     * @param wakeLock The {@link TracingWakeLock} instance that should be registered with the wake
     *        lock registry. Never {@code null}.
     *
     ...
     *
     * @param context A {@link Context} instance. Never {@code null}.
     * @param tag The tag to supply to {@link TracingPowerManager}.
    ...
    public final int onStartCommand(Intent intent, int flags, int startId) {
        ...
        TracingWakeLock wakeLock =
                acquireWakeLock(this, "CoreService onStart", K9.MAIL_SERVICE_WAKE_LOCK_TIMEOUT);

        if (K9.DEBUG) {
            Log.i(K9.LOG_TAG,
                    "CoreService: " + className + ".onStart(" + intent + ", " + startId + ")");
        }
        ...
        if (coreWakeLockId != -1) {
            ...
            if (coreWakeLock != null) {
                if (K9.DEBUG) {
                    Log.d(K9.LOG_TAG,
                            "Found core wake lock with id " + coreWakeLockId + ", releasing");
                }
     ...
     *
     * @param context A {@link Context} instance. Never {@code null}.
     * @param runner The code to be executed in the background thread.
     * @param wakeLockTime The timeout for the wake lock that will be acquired by this method.
     * @param startId The {@code startId} value received in {@link #onStart(Intent, int)} or
     *        {@code null} if you don't want the service to be shut down after {@code runner} has
     *        been executed (e.g. because you need to run another background task).<br>
    ...
    public void execute(Context context, final Runnable runner, int wakeLockTime,
        ...
        final TracingWakeLock wakeLock =
                acquireWakeLock(context, "CoreService execute", wakeLockTime);
        ...
        if (mThreadPool == null) {
            Log.e(K9.LOG_TAG,
                    "CoreService.execute (" + className + ") called with no thread "
     ...
     * </p>
     * 
     * @param intent The Intent supplied to {@link Context#startService(Intent)}.
     ...
     *
     * @param autoShutdown {@code true} to enable auto shutdown. {@code false} to disable.
     *
     * @see #mAutoShutdown
     */
