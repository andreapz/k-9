...
import com.tiscali.appmail.activity.loader.AttachmentContentLoader;
import com.tiscali.appmail.activity.loader.AttachmentInfoLoader;
import com.tiscali.appmail.activity.misc.Attachment;
import com.tiscali.appmail.activity.misc.Attachment.LoadingState;
...
public class AttachmentPresenter {
    private static final String STATE_KEY_ATTACHMENTS =
            "com.tiscali.appmail.activity.MessageCompose.attachments";
    ...
    public AttachmentPresenter(Context context, AttachmentMvpView attachmentMvpView,
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        actionToPerformAfterWaiting = WaitingAction
                .valueOf(savedInstanceState.getString(STATE_KEY_WAITING_FOR_ATTACHMENTS));
    ...
    public void addAttachment(AttachmentViewInfo attachmentViewInfo) {
        ...
        Attachment attachment = Attachment.createAttachment(attachmentViewInfo.uri, loaderId,
                attachmentViewInfo.mimeType);
        attachment = attachment.deriveWithMetadataLoaded(attachmentViewInfo.mimeType,
                attachmentViewInfo.displayName, attachmentViewInfo.size);
    ...
    private void addAttachmentAndStartLoader(Attachment attachment) {
        ...
        if (attachment.state == LoadingState.URI_ONLY) {
            ...
            throw new IllegalStateException(
                    "Attachment can only be added in URI_ONLY or METADATA state!");
        }
    }
    ...
    private void initAttachmentInfoLoader(Attachment attachment) {
        if (attachment.state != LoadingState.URI_ONLY) {
            throw new IllegalStateException(
                    "initAttachmentInfoLoader can only be called for URI_ONLY state!");
        }
    ...
    private void initAttachmentContentLoader(Attachment attachment) {
        if (attachment.state != LoadingState.METADATA) {
            throw new IllegalStateException(
                    "initAttachmentContentLoader can only be called for METADATA state!");
        }
