...
import com.tiscali.appmail.helper.RetainFragment;
...
import com.tiscali.appmail.ui.crypto.MessageCryptoCallback;
import com.tiscali.appmail.ui.crypto.MessageCryptoHelper;
import com.tiscali.appmail.ui.message.LocalMessageExtractorLoader;
import com.tiscali.appmail.ui.message.LocalMessageLoader;
...
/**
 * This class is responsible for loading a message start to finish, and retaining or reloading the
 * loading state on configuration changes.
 *
 * In particular, it takes care of the following: - load raw message data from the database, using
 * LocalMessageLoader - download partial message content if it is missing using MessagingController
 * - apply crypto operations if applicable, using MessageCryptoHelper - extract MessageViewInfo from
 * the message and crypto data using DecodeMessageLoader - download complete message content for
 * partially downloaded messages if requested
 *
 * No state is retained in this object itself. Instead, state is stored in the message loaders and
 * the MessageCryptoHelper which is stored in a RetainFragment. The public interface is intended for
 * use by an Activity or Fragment, which should construct a new instance of this class in onCreate,
 * then call asyncStartOrResumeLoadingMessage to start or resume loading the message, receiving
 * callbacks when it is loaded.
 *
 * When the Activity or Fragment is ultimately destroyed, it should call onDestroy, which stops
 * loading and deletes all state kept in loaders and fragments by this object. If it is only
 * destroyed for a configuration change, it should call onDestroyChangingConfigurations, which
 * cancels any further callbacks from this object but retains the loading state to resume from at
 * the next call to asyncStartOrResumeLoadingMessage.
 *
 * If the message is already loaded, a call to asyncStartOrResumeLoadingMessage will typically load
 * by starting the decode message loader, retrieving the already cached LocalMessage. This message
 * will be passed to the retained CryptoMessageHelper instance, returning the already cached
 * MessageCryptoAnnotations. These two objects will be checked against the retained
 * DecodeMessageLoader, returning the final result. At each intermediate step, the input of the
 * respective loaders will be checked for consistency, reloading if there is a mismatch.
 *
 */
public class MessageLoaderHelper {
    ...
    @Nullable // make this explicitly nullable, make sure to cancel/ignore any operation if this is
              // null
    ...
    public MessageLoaderHelper(Context context, LoaderManager loaderManager,
            FragmentManager fragmentManager, @NonNull MessageLoaderCallbacks callback) {
    ...
    public void asyncStartOrResumeLoadingMessage(MessageReference messageReference,
        ...
        this.account =
                Preferences.getPreferences(context).getAccount(messageReference.getAccountUuid());
    ...
    private void onLoadMessageFromDatabaseFinished() {
        ...
        boolean messageIncomplete = !localMessage.isSet(Flag.X_DOWNLOADED_FULL)
    ...
    private LoaderCallbacks<LocalMessage> localMessageLoaderCallback =
            new LoaderCallbacks<LocalMessage>() {
                @Override
                public Loader<LocalMessage> onCreateLoader(int id, Bundle args) {
                    ...
                    return new LocalMessageLoader(context, MessagingController.getInstance(context),
                            account, messageReference);
                }
    ...
    private void startOrResumeCryptoOperation() {
        RetainFragment<MessageCryptoHelper> retainCryptoHelperFragment =
                getMessageCryptoHelperRetainFragment();
    ...
    private RetainFragment<MessageCryptoHelper> getMessageCryptoHelperRetainFragment() {
        return RetainFragment.findOrCreate(fragmentManager,
                "crypto_helper_" + messageReference.hashCode());
    }
    ...
    private MessageCryptoCallback messageCryptoCallback = new MessageCryptoCallback() {
        ...
        public void onCryptoHelperProgress(int current, int max) {
            if (callback == null) {
                throw new IllegalStateException(
                        "unexpected call when callback is already detached");
            }
        ...
        @Override
        public void startPendingIntentForCryptoHelper(IntentSender si, int requestCode,
            ...
            callback.startIntentSenderForMessageLoaderHelper(si, requestCode, fillIntent, flagsMask,
                    flagValues, extraFlags);
        }
    };
    ...
    private void startOrResumeDecodeMessage() {
        LocalMessageExtractorLoader loader = (LocalMessageExtractorLoader) loaderManager
                .<MessageViewInfo>getLoader(DECODE_MESSAGE_LOADER_ID);
        boolean isLoaderStale =
                (loader == null) || !loader.isCreatedFor(localMessage, messageCryptoAnnotations);

        if (isLoaderStale) {
            Log.d(K9.LOG_TAG, "Creating new decode message loader");
            loaderManager.restartLoader(DECODE_MESSAGE_LOADER_ID, null,
    ...
    private LoaderCallbacks<MessageViewInfo> decodeMessageLoaderCallback =
            new LoaderCallbacks<MessageViewInfo>() {
                ...
                public Loader<MessageViewInfo> onCreateLoader(int id, Bundle args) {
                    ...
                    return new LocalMessageExtractorLoader(context, localMessage,
                            messageCryptoAnnotations);
                }

                @Override
                public void onLoadFinished(Loader<MessageViewInfo> loader,
    ...
    private void startDownloadingMessageBody(boolean downloadComplete) {
        if (downloadComplete) {
            MessagingController.getInstance(context).loadMessageRemote(account,
                    messageReference.getFolderName(), messageReference.getUid(),
                    downloadMessageListener);
        } else {
            MessagingController.getInstance(context).loadMessageRemotePartial(account,
                    messageReference.getFolderName(), messageReference.getUid(),
                    downloadMessageListener);
        }
    }
    ...
    public interface MessageLoaderCallbacks {
        ...
        void startIntentSenderForMessageLoaderHelper(IntentSender si, int requestCode,
                Intent fillIntent, int flagsMask, int flagValues, int extraFlags);
