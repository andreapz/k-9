...
import com.tiscali.appmail.view.RecipientSelectView.RecipientCryptoStatus;
...
public class RecipientLoader extends AsyncTaskLoader<List<Recipient>> {
    ...
    private static final String[] PROJECTION = {ContactsContract.CommonDataKinds.Email._ID,
            ContactsContract.Contacts.DISPLAY_NAME_PRIMARY, ContactsContract.Contacts.LOOKUP_KEY,
    ...
    public List<Recipient> loadInBackground() {
        ...
        if (addresses != null) {
            ...
            throw new IllegalStateException(
                    "loader must be initialized with query or list of addresses!");
        }
    ...
    private void fillContactDataFromEmailContentUri(Uri contactUri, List<Recipient> recipients,
            Map<String, Recipient> recipientMap) {
        Cursor cursor =
                getContext().getContentResolver().query(contactUri, PROJECTION, null, null, null);
    ...
    private void fillContactDataFromLookupKey(Uri lookupKeyUri, List<Recipient> recipients,
            Map<String, Recipient> recipientMap) {
        // We could use the contact id from the URI directly, but getting it from the lookup key is
        // safer
        Uri contactContentUri =
                Contacts.lookupContact(getContext().getContentResolver(), lookupKeyUri);
    ...
    private void fillContactDataFromQuery(String query, List<Recipient> recipients,
        ...
        String selection = Contacts.DISPLAY_NAME_PRIMARY + " LIKE ? " + " OR (" + Email.ADDRESS
                + " LIKE ? AND " + Data.MIMETYPE + " = '" + Email.CONTENT_ITEM_TYPE + "')";
        String[] selectionArgs = {query, query};
        Cursor cursor =
                contentResolver.query(queryUri, PROJECTION, selection, selectionArgs, SORT_ORDER);
        ...
        while (cursor.moveToNext()) {
            ...
            switch (addressType) {
                ...
                case ContactsContract.CommonDataKinds.Email.TYPE_MOBILE: {
                    // mobile isn't listed as an option contacts app, but it has a constant so we
                    // better support it
            ...
            if (recipient.isValidEmailAddress()) {
                Uri photoUri = cursor.isNull(INDEX_PHOTO_URI) ? null
                        : Uri.parse(cursor.getString(INDEX_PHOTO_URI));
    ...
    private void fillCryptoStatusData(Map<String, Recipient> recipientMap) {
        ...
        try {
            cursor = getContext().getContentResolver().query(queryUri, PROJECTION_CRYPTO_STATUS,
                    null, recipientAddresses, null);
                    ...
                    switch (status) {
                        case CRYPTO_PROVIDER_STATUS_UNTRUSTED: {
                            if (recipient.getCryptoStatus() == RecipientCryptoStatus.UNAVAILABLE) {
                                recipient
                                        .setCryptoStatus(RecipientCryptoStatus.AVAILABLE_UNTRUSTED);
                            }
                            break;
                        }
                        case CRYPTO_PROVIDER_STATUS_TRUSTED: {
                            if (recipient
                                    .getCryptoStatus() != RecipientCryptoStatus.AVAILABLE_TRUSTED) {
