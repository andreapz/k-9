...
import com.tiscali.appmail.service.DatabaseUpgradeService;
 ...
 * {@link SharedPreferences} file to {@link com.tiscali.appmail.mailstore.LocalStore#DB_VERSION}.
 * This is done as an optimization because it's faster than opening all of the accounts' databases
 * one by one.</li>
 ...
 * the locking in {@link Store#getLocalInstance(Account, Context)} the upgrade service will block in
 * the {@link Account#getLocalStore()} call and from the outside (especially for this activity) it
 * will appear as if {@link DatabaseUpgradeService} is performing the upgrade.
 * </p>
 */
public class UpgradeDatabases extends K9Activity {
     ...
     * @param startIntent After the database upgrade is complete an activity is started using this
     *        intent. Usually this is the intent that was used to start the calling activity. Never
     *        {@code null}.
     *
    ...
    class UpgradeDatabaseBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, final Intent intent) {
            ...
            if (DatabaseUpgradeService.ACTION_UPGRADE_PROGRESS.equals(action)) {
                ...
                String accountUuid =
                        intent.getStringExtra(DatabaseUpgradeService.EXTRA_ACCOUNT_UUID);
