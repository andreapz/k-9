...
import com.tiscali.appmail.activity.ChooseIdentity;
import com.tiscali.appmail.activity.ColorPickerDialog;
import com.tiscali.appmail.activity.K9PreferenceActivity;
import com.tiscali.appmail.activity.ManageIdentities;
import com.tiscali.appmail.crypto.OpenPgpApiHelper;
...
public class AccountSettings extends K9PreferenceActivity {
    ...
    private static final String PREFERENCE_MARK_MESSAGE_AS_READ_ON_VIEW =
            "mark_message_as_read_on_view";
    ...
    private static final String PREFERENCE_NOTIFY_CONTACTS_MAIL_ONLY =
            "account_notify_contacts_mail_only";
    ...
    private static final String PREFERENCE_SYNC_REMOTE_DELETIONS =
    ...
    private static final String PREFERENCE_REMOTE_SEARCH_NUM_RESULTS =
            "account_remote_search_num_results";
    private static final String PREFERENCE_REMOTE_SEARCH_FULL_TEXT =
            "account_remote_search_full_text";
    ...
    /*
     * Temporarily removed because search results aren't displayed to the user. So this feature is
     * useless.
     */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAccountDefaultQuotedTextShown =
                (CheckBoxPreference) findPreference(PREFERENCE_DEFAULT_QUOTED_TEXT_SHOWN);
        ...
        Preference.OnPreferenceChangeListener quoteStyleListener =
        ...
        mSyncRemoteDeletions =
                (CheckBoxPreference) findPreference(PREFERENCE_SYNC_REMOTE_DELETIONS);
        ...
        mRemoteSearchNumResults =
                (ListPreference) findPreference(PREFERENCE_REMOTE_SEARCH_NUM_RESULTS);
        ...
        if (mIsPushCapable) {
            ...
            PreferenceScreen incomingPrefs =
                    (PreferenceScreen) findPreference(PREFERENCE_SCREEN_INCOMING);
            incomingPrefs.removePreference(
                    (PreferenceScreen) findPreference(PREFERENCE_SCREEN_PUSH_ADVANCED));
        ...
        mAccountNotifyContactsMailOnly =
                (CheckBoxPreference) findPreference(PREFERENCE_NOTIFY_CONTACTS_MAIL_ONLY);
        ...
        String currentRingtone = (!mAccount.getNotificationSetting().shouldRing() ? null
                : mAccount.getNotificationSetting().getRingtone());
        ...
        mAccountVibratePattern
                .setValue(String.valueOf(mAccount.getNotificationSetting().getVibratePattern()));
        ...
        mAccountVibrateTimes
                .setValue(String.valueOf(mAccount.getNotificationSetting().getVibrateTimes()));
        mAccountVibrateTimes
                .setSummary(String.valueOf(mAccount.getNotificationSetting().getVibrateTimes()));
        mAccountVibrateTimes
                .setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                    @Override
                    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        findPreference(PREFERENCE_COMPOSITION)
    ...
    private void saveSettings() {
        ...
        mAccount.getNotificationSetting()
                .setVibratePattern(Integer.parseInt(mAccountVibratePattern.getValue()));
        mAccount.getNotificationSetting()
        ...
        if (mIsPushCapable) {
            ...
            mAccount.setRemoteSearchNumResults(
                    Integer.parseInt(mRemoteSearchNumResults.getValue()));
            // mAccount.setRemoteSearchFullText(mRemoteSearchFullText.isChecked());
        }

        boolean needsRefresh = mAccount
                .setAutomaticCheckIntervalMinutes(Integer.parseInt(mCheckFrequency.getValue()));
        ...
        // IMAP specific stuff
        if (mIsPushCapable) {
            boolean needsPushRestart =
                    mAccount.setFolderPushMode(FolderMode.valueOf(mPushMode.getValue()));
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (mCryptoKey != null
                && mCryptoKey.handleOnActivityResult(requestCode, resultCode, data)) {
            return;
        }
    ...
    private void doVibrateTest(Preference preference) {
        ...
        Vibrator vibrate =
                (Vibrator) preference.getContext().getSystemService(Context.VIBRATOR_SERVICE);
    ...
    /**
     * Remote search result limit summary contains the current limit. On load or change, update this
     * value.
     * 
    ...
    private void updateRemoteSearchLimit(String maxResults) {
        if (maxResults != null) {
            if (maxResults.equals("0")) {
                maxResults =
                        getString(R.string.account_settings_remote_search_num_results_entries_all);
            }
