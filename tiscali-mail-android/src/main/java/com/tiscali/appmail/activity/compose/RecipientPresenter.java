package com.tiscali.appmail.activity.compose;
...
import com.tiscali.appmail.helper.ReplyToParser.ReplyToAddresses;
...
public class RecipientPresenter implements PermissionPingCallback {
    ...
    // TODO initialize cryptoMode to other values under some circumstances, e.g. if we reply to an
    // encrypted e-mail
    ...
    public RecipientPresenter(Context context, LoaderManager loaderManager,
            RecipientMvpView recipientMvpView, Account account,
    ...
    public void initFromReplyToMessage(Message message, boolean isReplyAll) {
        ReplyToAddresses replyToAddresses =
                isReplyAll ? replyToParser.getRecipientsToReplyAllTo(message, account)
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        lastFocusedType =
                RecipientType.valueOf(savedInstanceState.getString(STATE_KEY_LAST_FOCUSED_TYPE));
        currentCryptoMode =
                CryptoMode.valueOf(savedInstanceState.getString(STATE_KEY_CURRENT_CRYPTO_MODE));
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        menu.findItem(R.id.openpgp_inline_enable)
                .setVisible(isCryptoConfigured && !cryptoEnablePgpInline);
        menu.findItem(R.id.openpgp_inline_disable)
                .setVisible(isCryptoConfigured && cryptoEnablePgpInline);
    ...
    public void onSwitchIdentity(Identity identity) {
        ...
        /*
         * if (mIdentityChanged) { mBccWrapper.setVisibility(View.VISIBLE); } mBccView.setText("");
    ...
    private void updateRecipientExpanderVisibility() {
        boolean notBothAreVisible =
                !(recipientMvpView.isCcVisible() && recipientMvpView.isBccVisible());
    ...
    public void updateCryptoStatus() {
        ...
        recipientMvpView
                .showPgpInlineModeIndicator(getCurrentCryptoStatus().isPgpInlineModeEnabled());
    }
    ...
    public ComposeCryptoStatus getCurrentCryptoStatus() {
        if (cachedCryptoStatus == null) {
            ComposeCryptoStatusBuilder builder = new ComposeCryptoStatusBuilder()
                    .setCryptoProviderState(cryptoProviderState).setCryptoMode(currentCryptoMode)
    ...
    private void addRecipientsFromAddresses(final RecipientType recipientType,
    ...
    public void onClickCryptoStatus() {
        switch (cryptoProviderState) {
            case UNCONFIGURED:
                Log.e(K9.LOG_TAG,
                        "click on crypto status while unconfigured - this should not really happen?!");
                return;
            case OK:
    ...
    /**
     * Does the device actually have a Contacts application suitable for picking a contact. As hard
     * as it is to believe, some vendors ship without it.
     *
    ...
    private void setCryptoProvider(String cryptoProvider) {
        ...
        boolean isSameProvider =
                cryptoProvider != null && cryptoProvider.equals(this.cryptoProvider);
        ...
        openPgpServiceConnection =
                new OpenPgpServiceConnection(context, cryptoProvider, new OnBound() {
                    @Override
    ...
    private void cryptoProviderBindOrCheckPermission() {
        ...
        if (pendingUserInteractionIntent != null) {
            recipientMvpView.launchUserInteractionPendingIntent(pendingUserInteractionIntent,
    ...
    public OpenPgpApi getOpenPgpApi() {
        if (openPgpServiceConnection == null || !openPgpServiceConnection.isBound()) {
            Log.e(K9.LOG_TAG,
                    "obtained openpgpapi object, but service is not bound! inconsistent state?");
        }
    ...
    public enum CryptoProviderState {
        UNCONFIGURED, UNINITIALIZED, LOST_CONNECTION, ERROR, OK
    }

    public enum CryptoMode {
        DISABLE, SIGN_ONLY, OPPORTUNISTIC, PRIVATE,
    }
}
