 ...
 */
public class AccountSetupCheckSettings extends K9Activity
        ...
        mHandler.post(new Runnable() {
            public void run() {
                ...
                for (int i = 0; i < chain.length; i++) {
                    ...
                    chainInfo.append("Subject: ").append(chain[i].getSubjectDN().toString())
                            .append("\n");
                    ...
                    try {
                        final Collection<List<?>> subjectAlternativeNames =
                                chain[i].getSubjectAlternativeNames();
                        if (subjectAlternativeNames != null) {
                            ...
                            String transportURIHost =
                                    (Uri.parse(mAccount.getTransportUri())).getHost();
                            ...
                            for (List<?> subjectAlternativeName : subjectAlternativeNames) {
                                ...
                                // if some of the SubjectAltNames match the store or transport
                                // -host,
                                // display them
                    ...
                    chainInfo.append("Issuer: ").append(chain[i].getIssuerDN().toString())
                            .append("\n");
                    if (sha1 != null) {
                        sha1.reset();
                        try {
                            char[] sha1sum = Hex.encodeHex(sha1.digest(chain[i].getEncoded()));
                            chainInfo.append("Fingerprint (SHA-1): ").append(new String(sha1sum))
                                    .append("\n");
                ...
                new AlertDialog.Builder(AccountSetupCheckSettings.this)
                        ...
                        .setPositiveButton(
                                getString(
                                        R.string.account_setup_failed_dlg_invalid_certificate_accept),
                        ...
                        .setNegativeButton(
                                getString(
                                        R.string.account_setup_failed_dlg_invalid_certificate_reject),
                                new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog, int which) {
                                        finish();
                                    }
    ...
    /**
     * FIXME: Don't use an AsyncTask to perform network operations. See also discussion in
     * https://github.com/k9mail/k-9/pull/560
     */
    private class CheckAccountTask extends AsyncTask<CheckDirection, Integer, Void> {
        ...
        protected Void doInBackground(CheckDirection... params) {
            ...
            try {
                /*
                 * This task could be interrupted at any point, but network operations can block, so
                 * relying on InterruptedException is not enough. Instead, check after each
        ...
        private void checkIncoming() throws MessagingException {
            ...
            // MessagingController.getInstance(getApplication()).listFoldersSynchronous(account,
            // true, null);
