 ...
 *
 * This is used by {@link com.tiscali.appmail.activity.MessageCompose} to fetch and manage
 * attachments.
 */
public class Attachment implements Parcelable {
    ...
    public enum LoadingState {
        URI_ONLY, METADATA, COMPLETE, CANCELLED
    }

    private Attachment(Uri uri, LoadingState state, int loaderId, String contentType, String name,
    ...
    public static Attachment createAttachment(Uri uri, int loaderId, String contentType) {
        return new Attachment(uri, Attachment.LoadingState.URI_ONLY, loaderId, contentType, null,
                null, null);
    }
    ...
    public Attachment deriveWithMetadataLoaded(String usableContentType, String name, long size) {
        ...
        return new Attachment(uri, Attachment.LoadingState.METADATA, loaderId, usableContentType,
                name, size, null);
    }
    ...
    public Attachment deriveWithLoadCancelled() {
        if (state != Attachment.LoadingState.METADATA) {
            throw new IllegalStateException(
                    "deriveWitLoadCancelled can only be called on a METADATA attachment!");
        }
        return new Attachment(uri, Attachment.LoadingState.CANCELLED, loaderId, contentType, name,
                size, null);
    }
    ...
    public Attachment deriveWithLoadComplete(String absolutePath) {
        ...
        return new Attachment(uri, Attachment.LoadingState.COMPLETE, loaderId, contentType, name,
                size, absolutePath);
    }
