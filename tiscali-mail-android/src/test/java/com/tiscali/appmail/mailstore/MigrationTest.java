...
public class MigrationTest {
    ...
    public void setUp() throws Exception {
        ...
        databaseFile =
                storageManager.getDatabase(account.getUuid(), account.getLocalStorageProviderId());
        Assert.assertTrue(
                databaseFile.getParentFile().isDirectory() || databaseFile.getParentFile().mkdir());

        attachmentDir = StorageManager.getInstance(RuntimeEnvironment.application)
    ...
    private SQLiteDatabase createV50Database() {
        ...
        SQLiteDatabase db = RuntimeEnvironment.application
                .openOrCreateDatabase(databaseFile.getName(), Context.MODE_PRIVATE, null);
    ...
    private void insertSimplePlaintextMessage(SQLiteDatabase db) {
        String[] statements =
                new String[] {"INSERT INTO messages VALUES(2,0,16,'3','regular mail',1453380493000,"
    ...
    public void migrateTextPlain() throws Exception {
        ...
        Assert.assertEquals("text/plain", MimeUtility
                .getHeaderParameter(msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE)[0], null));
        Assert.assertEquals("utf-8", MimeUtility
                .getHeaderParameter(msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE)[0], "charset"));
    ...
    private void insertMixedWithAttachments(SQLiteDatabase db) throws Exception {
        String[] statements = new String[] {
                ...
                "INSERT INTO attachments VALUES(3,3,'2',"
                        + "'content://com.tiscali.appmail.attachmentprovider/" + account.getUuid()
                        + "/3/RAW',2250," + "'k9small.png','image/png',NULL,'attachment')",
    ...
    public void migrateMixedWithAttachments() throws Exception {
        ...
        Assert.assertEquals(1, msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE).length);
        Assert.assertEquals("multipart/mixed", MimeUtility
                .getHeaderParameter(msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE)[0], null));
        Assert.assertEquals("----5D6OUTIYLNN2X63O0R2M0V53TOUAQP", MimeUtility
                .getHeaderParameter(msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE)[0], "boundary"));
        ...
        Assert.assertEquals("attachment",
                MimeUtility.getHeaderParameter(attachmentPart.getDisposition(), null));
        Assert.assertEquals("k9small.png",
                MimeUtility.getHeaderParameter(attachmentPart.getDisposition(), "filename"));
        Assert.assertEquals("2250",
                MimeUtility.getHeaderParameter(attachmentPart.getDisposition(), "size"));
    ...
    private void insertPgpMimeEncryptedMessage(SQLiteDatabase db) {
        String[] statements = new String[] {
                ...
                "INSERT INTO attachments VALUES(1,5,NULL,'content://com.tiscali.appmail.attachmentprovider/"
                        + account.getUuid()
                        + "/1/RAW',12,NULL,'application/pgp-encrypted',NULL,'attachment')",
                "INSERT INTO attachments VALUES(2,5,NULL,'content://com.tiscali.appmail.attachmentprovider/"
                        + account.getUuid()
                        + "/2/RAW',1946,'msg.asc','application/octet-stream',NULL,'attachment')",};
    ...
    public void migratePgpMimeEncryptedMessage() throws Exception {
        ...
        Multipart body = (Multipart) msg.getBody();
        Assert.assertEquals(1, msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE).length);
        Assert.assertEquals("application/pgp-encrypted", MimeUtility
                .getHeaderParameter(msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE)[0], "protocol"));
        Assert.assertEquals("UoPmpPX/dBe4BELn", MimeUtility
                .getHeaderParameter(msg.getHeader(MimeHeader.HEADER_CONTENT_TYPE)[0], "boundary"));
    ...
    private void insertPgpInlineEncryptedMessage(SQLiteDatabase db) {
        String[] statements = new String[] {
                "INSERT INTO messages VALUES(6,0,16,'7','pgp/inline encrypted',1453380759000,"
                        ...
                        + "j8JjG3kNUmcrXP0PEctwdi9phnJscL5abfOrI9mT3eYfXIVy<br />" + "=tD4Z<br />"
                        + "-----END PGP MESSAGE-----<br />"
    ...
    public void migratePgpInlineEncryptedMessage() throws Exception {
        ...
        String msgTextContent = MessageExtractor.getTextFromPart(msg);
        Assert.assertEquals(OpenPgpUtils.PARSE_RESULT_MESSAGE,
                OpenPgpUtils.parseMessage(msgTextContent));
    }
    ...
    public void migratePgpInlineClearsignedMessage() throws Exception {
        ...
        String msgTextContent = MessageExtractor.getTextFromPart(msg);
        Assert.assertEquals(OpenPgpUtils.PARSE_RESULT_SIGNED_MESSAGE,
                OpenPgpUtils.parseMessage(msgTextContent));
    }
    ...
    private void insertHtmlWithRelatedMessage(SQLiteDatabase db) {
        String[] statements = new String[] {
                "INSERT INTO messages VALUES(9,0,16,'10','html with multipart/related content',1453922845000,"
                        ...
                        + "    <img alt=\"Alternative_Text\" src=\"content://com.tiscali.appmail.attachmentprovider/"
                        + account.getUuid() + "/6/RAW\"\n" + "      height=\"177\" width=\"220\">\n"
                ...
                "INSERT INTO attachments VALUES(6,9,NULL,'content://com.tiscali.appmail.attachmentprovider/"
                        + account.getUuid() + "/6/RAW',"
    ...
    private Account getNewAccount() {
        ...
        // FIXME: This is a hack to get Preferences into a state where it's safe to call
        // newAccount()
