...
import com.tiscali.appmail.mail.internet.ListHeaders;


@RunWith(RobolectricTestRunner.class)
@Config(manifest = Config.NONE, sdk = 21)
public class ReplyToParserTest {
    private static final Address[] REPLY_TO_ADDRESSES =
            Address.parse("replyTo1@example.com, replyTo2@example.com");
    ...
    private static final String[] LIST_POST_HEADER_VALUES =
            new String[] {"<mailto:listPost@example.com>"};
    ...
    public void getRecipientsToReplyAllTo_should_returnFromAndToAndCcRecipients() throws Exception {
        ...
        assertArrayEquals(arrayConcatenate(FROM_ADDRESSES, TO_ADDRESSES, Address.class),
                recipientsToReplyAllTo.to);
    ...
    public void getRecipientsToReplyAllTo_should_excludeDuplicates() throws Exception {
        ...
        when(message.getFrom())
                .thenReturn(arrayConcatenate(FROM_ADDRESSES, REPLY_TO_ADDRESSES, Address.class));
        when(message.getRecipients(RecipientType.TO))
                .thenReturn(arrayConcatenate(FROM_ADDRESSES, TO_ADDRESSES, Address.class));
        when(message.getRecipients(RecipientType.CC))
                .thenReturn(arrayConcatenate(CC_ADDRESSES, TO_ADDRESSES, Address.class));
        when(message.getHeader(ListHeaders.LIST_POST_HEADER)).thenReturn(new String[0]);
        ...
        int totalExpectedAddresses =
                REPLY_TO_ADDRESSES.length + FROM_ADDRESSES.length + TO_ADDRESSES.length;
    ...
    public <T> void assertArrayContainsAll(T[] expecteds, T[] actual) {
        for (T expected : expecteds) {
            assertTrue("Element must be in array (" + expected + ")",
                    Utility.arrayContains(actual, expected));
        }
    }
