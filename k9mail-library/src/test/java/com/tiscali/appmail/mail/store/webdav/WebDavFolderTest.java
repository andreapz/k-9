...
public class WebDavFolderTest {
    ...
    private void setupFolderWithMessages(int count) throws MessagingException {
        ...
        when(mockStore.processRequest("https://localhost/webDavStoreUrl/testFolder", "SEARCH",
                messageCountXml, headers)).thenReturn(mockDataSet);
    ...
    private void setupGetUrlsRequestResponse(String uid, String url) throws MessagingException {
        ...
        headers.put("Brief", "t");
        when(mockStore.processRequest("https://localhost/webDavStoreUrl/testFolder", "SEARCH",
                getUrlsXml, headers)).thenReturn(mockDataSet);
    ...
    public void folder_can_fetch_less_than_10_envelopes() throws MessagingException {
        when(mockStore.processRequest(anyString(), anyString(), anyString(),
                anyMapOf(String.class, String.class))).thenReturn(mockDataSet);
    ...
    public void folder_can_fetch_sensible_body_data_and_notifies_listener()
        ...
        when(mockHttpClient.executeOverride(any(HttpUriRequest.class), any(HttpContext.class)))
    ...
    @Test
    public void folder_can_handle_empty_response_to_body_request()
    ...
    @Test
    public void folder_ignores_exception_thrown_when_closing()
            throws MessagingException, IOException {
        setupStoreForMessageFetching();
        List<WebDavMessage> messages = setup25MessagesToFetch();
    ...
    public void getMessages_should_request_message_search() throws MessagingException {
        ...
        when(mockStore.processRequest(eq("https://localhost/webDavStoreUrl/testFolder"),
                eq("SEARCH"), eq(messagesXml), Matchers.<Map<String, String>>any()))
    ...
    public void getMessages_shouldProvideCorrectHeadersInRequest() throws MessagingException {
        ...
        assertEquals(
                "rows=" + (totalMessages - (messageEnd)) + "-" + (totalMessages - messageStart),
    ...
    private void setupMoveOrCopy() throws MessagingException {
        ...
        when(mockStore.getMoveOrCopyMessagesReadXml(eq(new String[] {url}), anyBoolean()))
                .thenReturn(moveOrCopyXml);
    ...
    public void moveMessages_should_requestMoveXml() throws Exception {
        ...
        verify(mockStore).getMoveOrCopyMessagesReadXml(eq(new String[] {"url1"}), eq(true));
    }
    ...
    public void copyMessages_should_requestCopyXml() throws Exception {
        ...
        verify(mockStore).getMoveOrCopyMessagesReadXml(eq(new String[] {"url1"}), eq(false));
    }
    ...
    @Test
    public void appendWebDavMessages_replaces_messages_with_WebDAV_versions()
            throws MessagingException, IOException {
        when(mockHttpClient.executeOverride(any(HttpUriRequest.class), any(HttpContext.class)))
