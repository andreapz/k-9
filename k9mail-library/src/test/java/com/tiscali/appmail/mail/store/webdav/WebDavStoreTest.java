...
public class WebDavStoreTest {
    ...
    @Test
    public void checkSettings_withHttpPrefixedServerName_shouldUseInsecureConnection()
            throws Exception {
    ...
    @Test
    public void checkSettings_withOkResponse_shouldPerformFormBasedAuthentication()
            throws Exception {
        ...
        when(mockHttpClient.executeOverride(requestCaptor.capture(), any(HttpContext.class)))
                .thenReturn(OK_200_RESPONSE).thenReturn(createOkResponseWithForm())
                .thenAnswer(createOkResponseWithCookie()).thenReturn(OK_200_RESPONSE);
        ...
        assertEquals("http://example.org:80/exchweb/bin/auth/owaauth.dll",
                requests.get(1).getURI().toString());
        assertEquals("POST", requests.get(2).getMethod()); // Confirming login
        assertEquals("http://example.org:80/exchweb/bin/auth/owaauth.dll",
                requests.get(2).getURI().toString());
        assertEquals("GET", requests.get(3).getMethod()); // Getting response
    }

    @Test
    public void checkSettings_withInitialUnauthorizedResponse_shouldPerformBasicAuthentication()
            throws Exception {
        WebDavStore webDavStore = createDefaultWebDavStore();
        configureHttpResponses(UNAUTHORIZED_401_RESPONSE, OK_200_RESPONSE);
    ...
    public void checkSettings_withSslException_shouldThrowCertificateValidationException()
            throws Exception {
        WebDavStore webDavStore = createDefaultWebDavStore();
        ArgumentCaptor<HttpGeneric> requestCaptor = ArgumentCaptor.forClass(HttpGeneric.class);
        when(mockHttpClient.executeOverride(requestCaptor.capture(), any(HttpContext.class)))
    ...
    public void checkSettings_shouldRegisterHttpsSchemeWithRegistry() throws Exception {
        ...
        assertEquals(WebDavSocketFactory.class,
                schemeCaptor.getValue().getSocketFactory().getClass());
    }
    ...
    public void getPersonalNamespaces_shouldRequestSpecialFolders() throws Exception {
        ...
        configureHttpResponses(UNAUTHORIZED_401_RESPONSE, OK_200_RESPONSE,
                createOkPropfindResponse(), createOkSearchResponse());
    ...
    @Test
    public void getPersonalNamespaces_shouldProvideListOfAllFoldersSentFromResponses()
            throws Exception {
        StoreConfig storeConfig = createStoreConfig("webdav://user:password@example.org:80");
        WebDavStore webDavStore = new WebDavStore(storeConfig, mockHttpClientFactory);
        configureHttpResponses(UNAUTHORIZED_401_RESPONSE, OK_200_RESPONSE,
    ...
    private BasicHttpResponse createOkSearchResponse() throws UnsupportedEncodingException {
        ...
        HttpEntity searchResponseEntity =
                new StringEntity("" + "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
    ...
    private BasicHttpResponse createOkPropfindResponse() throws UnsupportedEncodingException {
        ...
        HttpEntity propfindResponseEntity =
                new StringEntity("" + "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
    ...
    private void configureHttpResponses(HttpResponse... responses) throws IOException {
        ...
        OngoingStubbing<HttpResponse> stubbing = when(
                mockHttpClient.executeOverride(requestCaptor.capture(), any(HttpContext.class)));
