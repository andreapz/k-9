...
import com.tiscali.appmail.mail.CertificateValidationException.Reason;
...
import com.tiscali.appmail.mail.store.imap.mockserver.MockImapServer;
...
public class ImapConnectionTest {
    ...
    @Test
    public void open_withCapabilitiesInInitialResponse_shouldNotIssueCapabilitiesCommand()
            throws Exception {
    ...
    @Test
    public void open_authPlainAndLoginFallbackWithAuthenticationFailure_shouldThrow()
            throws Exception {
        settings.setAuthType(AuthType.PLAIN);
        MockImapServer server = new MockImapServer();
        preAuthenticationDialog(server, "AUTH=PLAIN");
    ...
    public void open_authCramMd5WithoutAuthCramMd5Capability_shouldThrow() throws Exception {
        ...
        try {
            ...
            assertEquals("Server doesn't support encrypted passwords using CRAM-MD5.",
                    e.getMessage());
        }
    ...
    public void open_withConnectionError_shouldThrow() throws Exception {
        ...
        ImapConnection imapConnection =
                createImapConnection(settings, socketFactory, connectivityManager);
    ...
    private ImapConnection createImapConnection(ImapSettings settings,
            TrustedSocketFactory socketFactory, ConnectivityManager connectivityManager) {
        return new ImapConnection(settings, socketFactory, connectivityManager,
                SOCKET_CONNECT_TIMEOUT, SOCKET_READ_TIMEOUT);
    }

    private ImapConnection startServerAndCreateImapConnection(MockImapServer server)
            throws IOException {
        server.start();
    ...
    private ImapConnection simpleOpenWithCapabilities(MockImapServer server, String capabilities)
            throws Exception {
