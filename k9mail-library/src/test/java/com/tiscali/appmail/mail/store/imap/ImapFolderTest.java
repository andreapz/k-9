...
import static com.tiscali.appmail.mail.Folder.OPEN_MODE_RO;
import static com.tiscali.appmail.mail.Folder.OPEN_MODE_RW;
import static com.tiscali.appmail.mail.store.imap.ImapResponseHelper.createImapResponse;
...
public class ImapFolderTest {
    ...
    @Test
    public void open_shouldMakeExistReturnTrueWithoutExecutingAdditionalCommands()
            throws Exception {
        ImapFolder imapFolder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
    ...
    @Test
    public void open_withConnectionThrowingOnReUse_shouldCreateNewImapConnection()
            throws Exception {
        ImapFolder imapFolder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        imapFolder.open(OPEN_MODE_RW);
    ...
    public void open_withMessagingException_shouldThrowMessagingException() throws Exception {
        ImapFolder imapFolder = createFolder("Folder");
        when(imapStore.getConnection()).thenReturn(imapConnection);
        doThrow(MessagingException.class).when(imapConnection)
                .executeSimpleCommand("SELECT \"Folder\"");
    ...
    public void open_withoutExistsResponse_shouldThrowMessagingException() throws Exception {
        ImapFolder imapFolder = createFolder("Folder");
        when(imapStore.getConnection()).thenReturn(imapConnection);
        List<ImapResponse> selectResponses =
    ...
    @Test
    public void exists_withClosedFolder_shouldOpenConnectionAndIssueStatusCommand()
            throws Exception {
        ImapFolder imapFolder = createFolder("Folder");
        when(imapStore.getConnection()).thenReturn(imapConnection);
    ...
    @Test
    public void create_withClosedFolder_shouldOpenConnectionAndIssueCreateCommand()
            throws Exception {
        ImapFolder imapFolder = createFolder("Folder");
        when(imapStore.getConnection()).thenReturn(imapConnection);
    ...
    public void create_withNegativeImapResponse_shouldReturnFalse() throws Exception {
        ...
        when(imapStore.getConnection()).thenReturn(imapConnection);
        doThrow(NegativeImapResponseException.class).when(imapConnection)
                .executeSimpleCommand("CREATE \"Folder\"");
    ...
    public void copyMessages() throws Exception {
        ...
        List<ImapResponse> copyResponses =
                singletonList(createImapResponse("x OK [COPYUID 23 1 101] Success"));
        when(imapConnection.executeSimpleCommand("UID COPY 1 \"Destination\""))
                .thenReturn(copyResponses);
    ...
    public void delete_shouldMoveMessagesToTrashFolder() throws Exception {
        ...
        List<ImapResponse> copyResponses =
                singletonList(createImapResponse("x OK [COPYUID 23 2 102] Success"));
    ...
    public void getFlaggedMessageCount() throws Exception {
        ...
        List<ImapResponse> imapResponses =
                asList(createImapResponse("* SEARCH 1 2"), createImapResponse("* SEARCH 23 42"));
        when(imapConnection.executeSimpleCommand("SEARCH 1:* FLAGGED NOT DELETED"))
                .thenReturn(imapResponses);
        folder.open(OPEN_MODE_RW);
    ...
    public void getHighestUid_imapConnectionThrowsNegativesResponse_shouldReturnMinusOne()
        ...
        prepareImapFolderForOpen(OPEN_MODE_RW);
        doThrow(NegativeImapResponseException.class).when(imapConnection)
                .executeSimpleCommand("UID SEARCH *:*");
        folder.open(OPEN_MODE_RW);
    ...
    public void getMessages_withoutDateConstraint() throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        List<ImapResponse> imapResponses = asList(createImapResponse("* SEARCH 3"),
                createImapResponse("* SEARCH 5"), createImapResponse("* SEARCH 6"));
        when(imapConnection.executeSimpleCommand("UID SEARCH 1:10 NOT DELETED"))
                .thenReturn(imapResponses);
        folder.open(OPEN_MODE_RW);
    ...
    public void getMessages_withDateConstraint() throws Exception {
        ...
        prepareImapFolderForOpen(OPEN_MODE_RW);
        List<ImapResponse> imapResponses =
                asList(createImapResponse("* SEARCH 47"), createImapResponse("* SEARCH 18"));
    ...
    public void getMessages_sequenceNumbers() throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        List<ImapResponse> imapResponses = asList(createImapResponse("* SEARCH 17"),
                createImapResponse("* SEARCH 18"), createImapResponse("* SEARCH 49"));
        when(imapConnection.executeSimpleCommand("UID SEARCH 1,2,5 NOT DELETED"))
                .thenReturn(imapResponses);
        folder.open(OPEN_MODE_RW);
    ...
    public void getMessagesFromUids() throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        List<ImapResponse> imapResponses = asList(createImapResponse("* SEARCH 11"),
                createImapResponse("* SEARCH 22"), createImapResponse("* SEARCH 25"));
        when(imapConnection.executeSimpleCommand("UID SEARCH UID 11,22,25"))
                .thenReturn(imapResponses);
        folder.open(OPEN_MODE_RW);
    ...
    @Test
    public void areMoreMessagesAvailable_withAdditionalMessages_shouldReturnTrue()
            throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        List<ImapResponse> imapResponses = singletonList(createImapResponse("* SEARCH 42"));
        when(imapConnection.executeSimpleCommand("SEARCH 1:9 NOT DELETED"))
                .thenReturn(imapResponses);
        folder.open(OPEN_MODE_RW);
    ...
    @Test
    public void areMoreMessagesAvailable_withIndexOfOne_shouldReturnFalseWithoutPerformingSearch()
            throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        folder.open(OPEN_MODE_RW);
    ...
    public void fetch_withEnvelopeFetchProfile_shouldIssueRespectiveCommand() throws Exception {
        ...
        verify(imapConnection)
                .sendCommand("UID FETCH 1 (UID INTERNALDATE RFC822.SIZE BODY.PEEK[HEADER.FIELDS "
    ...
    @Test
    public void fetch_withBodySaneFetchProfileAndNoMaximumDownloadSize_shouldIssueRespectiveCommand()
            throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RO);
        folder.open(OPEN_MODE_RO);
    ...
    @Test
    public void fetch_withBodyFetchProfileAndNoMaximumDownloadSize_shouldIssueRespectiveCommand()
            throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RO);
        folder.open(OPEN_MODE_RO);
    ...
    @Test
    public void getUidFromMessageId_withMessageIdHeader_shouldIssueUidSearchCommand()
            throws Exception {
        ImapFolder folder = createFolder("Folder");
        prepareImapFolderForOpen(OPEN_MODE_RW);
        folder.open(OPEN_MODE_RW);
        ...
        when(message.getHeader("Message-ID"))
                .thenReturn(new String[] {"<00000000.0000000@example.org>"});

        folder.getUidFromMessageId(message);

        verify(imapConnection).executeSimpleCommand(
                "UID SEARCH HEADER MESSAGE-ID \"<00000000.0000000@example.org>\"");
    }
    ...
    public void getUidFromMessageId() throws Exception {
        ...
        when(message.getHeader("Message-ID"))
                .thenReturn(new String[] {"<00000000.0000000@example.org>"});
        when(imapConnection.executeSimpleCommand(
                "UID SEARCH HEADER MESSAGE-ID \"<00000000.0000000@example.org>\""))
    ...
    public void search_withFullTextSearchDisabled_shouldIssueRespectiveCommand() throws Exception {
        ...
        verify(imapConnection)
                .executeSimpleCommand("UID SEARCH OR SUBJECT \"query\" FROM \"query\"");
    }
    ...
    public void search_withRemoteSearchDisabled_shouldThrow() throws Exception {
        ...
        try {
            ...
            assertEquals("Your settings do not allow remote searching of this account",
                    e.getMessage());
        }
    }
    ...
    private Part createPlainTextPart(String serverExtra) {
        ...
        when(part.getHeader(MimeHeader.HEADER_CONTENT_TRANSFER_ENCODING))
                .thenReturn(new String[] {MimeUtil.ENC_7BIT});
        when(part.getHeader(MimeHeader.HEADER_CONTENT_TYPE))
                .thenReturn(new String[] {"text/plain"});
        return part;
    }
    ...
    private void prepareImapFolderForOpen(int openMode) throws MessagingException, IOException {
        ...
        List<ImapResponse> imapResponses = asList(
                ...
                createImapResponse("* 23 EXISTS"), createImapResponse("* 0 RECENT"),
        ...
        if (openMode == OPEN_MODE_RW) {
            when(imapConnection.executeSimpleCommand("SELECT \"Folder\""))
                    .thenReturn(imapResponses);
        } else {
            when(imapConnection.executeSimpleCommand("EXAMINE \"Folder\""))
                    .thenReturn(imapResponses);
        }
    }
