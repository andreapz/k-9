 ...
 *
 * This class is copied from the org.apache.james.mime4j.decoder.DecoderUtil class. It's modified
 * here in order to decode emoji characters in the Subject headers. The method to decode emoji
 * depends on the MimeMessage class because it has to be determined with the sender address, the
 * mailer and so on.
 */
class DecoderUtil {
    /**
     * Decodes an encoded word encoded with the 'B' encoding (described in RFC 2047) found in a
     * header field body.
     *
    ...
    /**
     * Decodes an encoded word encoded with the 'Q' encoding (described in RFC 2047) found in a
     * header field body.
     *
     * @param encodedWord the encoded word to decode.
     * @param charset the Java charset to use.
    ...
    private static String decodeQ(String encodedWord, String charset) {
        ...
        QuotedPrintableInputStream is =
                new QuotedPrintableInputStream(new ByteArrayInputStream(bytes));
        try {
            return CharsetSupport.readToString(is, charset);
        } catch (IOException e) {
            return null;
    ...
    /**
     * Decodes a string containing encoded words as defined by RFC 2047. Encoded words in have the
     * form =?charset?enc?Encoded word?= where enc is either 'Q' or 'q' for quoted-printable and 'B'
     * or 'b' for Base64.
     *
    ...
    private static String decodeEncodedWord(String body, int begin, int end, Message message) {
        ...
        if (encodedText.isEmpty()) {
            Log.w(LOG_TAG,
                    "Missing encoded text in encoded word: '" + body.substring(begin, end) + "'");
            return null;
        }
        ...
        if (encoding.equalsIgnoreCase("Q")) {
            ...
            Log.w(LOG_TAG, "Warning: Unknown encoding in encoded word '"
