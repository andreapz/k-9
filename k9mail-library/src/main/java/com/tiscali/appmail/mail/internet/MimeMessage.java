...
/**
 * An implementation of Message that stores all of it's metadata in RFC 822 and RFC 2045 style
 * headers.
 */
public class MimeMessage extends Message {
    ...
    public static MimeMessage parseMimeMessage(InputStream in, boolean recurse)
    ...
    /**
     * Parse the given InputStream using Apache Mime4J to build a MimeMessage. Does not recurse
     * through nested bodyparts.
     */
    ...
    /**
     * Sets the sent date object member as well as *adds* the 'Date' header instead of setting it
    ...
    public void setReferences(String references) {
        /*
         * Make sure the References header doesn't exceed the maximum header line length and won't
         * get (Q-)encoded later. Otherwise some clients will break threads apart.
         *
        ...
        /*
         * NOTE: Usually the maximum header line is 998 + CRLF = 1000 characters. But at least one
         * implementations seems to have problems with 998 characters, so we adjust for that fact.
         */
        ...
        if (originalLength >= limit) {
            ...
            final String firstReference =
                    references.substring(start, references.indexOf('<', start + 1));

            // Find longest tail
            final String tail = references.substring(
                    references.indexOf('<', firstReference.length() + originalLength - limit));
    ...
    private class MimeMessageBuilder implements ContentHandler {
        ...
        public void endMultipart() {
            ...
            if (hasNoBodyParts && hasNoEpilogue) {
                /*
                 * The parser is calling startMultipart(), preamble(), and endMultipart() when all
                 * we have is headers of a "multipart/*" part. But there's really no point in
                 * keeping a Multipart body if all of the content is missing.
                 */
                expect(Part.class);
                Part part = (Part) stack.peek();
                part.setBody(null);
    ...
    /**
     * Convert a top level message into a bodypart. Returned body part shouldn't contain
     * inappropriate headers such as smtp headers or MIME-VERSION. Both Message and MimeBodyPart
     * might share structures.
     * 
