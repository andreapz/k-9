...
class WebDavFolder extends Folder<WebDavMessage> {
    ...
    private void moveOrCopyMessages(List<? extends Message> messages, String folderName,
    ...
    @Override
    public List<WebDavMessage> getMessages(int start, int end, Date earliestDate,
            MessageRetrievalListener<WebDavMessage> listener) throws MessagingException {
    ...
    @Override
    public void fetch(List<WebDavMessage> messages, FetchProfile fp,
            MessageRetrievalListener<WebDavMessage> listener) throws MessagingException {
    ...
    /**
     * Fetches the full messages or up to {@param lines} lines and passes them to the message
     * parser.
     */
    private void fetchMessages(List<WebDavMessage> messages,
            MessageRetrievalListener<WebDavMessage> listener, int lines) throws MessagingException {
        ...
        for (int i = 0, count = messages.size(); i < count; i++) {
            ...
            /**
             * If fetch is called outside of the initial list (ie, a locally stored message), it may
             * not have a URL associated. Verify and fix that
             */
            if (wdMessage.getUrl().equals("")) {
                wdMessage.setUrl(
                        getMessageUrls(new String[] {wdMessage.getUid()}).get(wdMessage.getUid()));
            ...
            try {
                ...
                Log.e(LOG_TAG, "URISyntaxException caught " + use + "\nTrace: "
                        + WebDavUtils.processException(use));
                throw new MessagingException("URISyntaxException caught", use);
            } catch (IOException ioe) {
                Log.e(LOG_TAG,
                        "Non-success response code loading message, response code was " + statusCode
     ...
     */
    private void fetchFlags(List<WebDavMessage> startMessages,
            MessageRetrievalListener<WebDavMessage> listener) throws MessagingException {
        ...
        if (startMessages.size() > 20) {
            List<WebDavMessage> newMessages =
                    new ArrayList<WebDavMessage>(startMessages.size() - 20);
    ...
    /**
     * Fetches and parses the message envelopes for the supplied messages. The idea is to have this
     * be recursive so that we do a series of medium calls instead of one large massive call or a
     * large number of smaller calls. Call it a happy balance
     */
    private void fetchEnvelope(List<WebDavMessage> startMessages,
            MessageRetrievalListener<WebDavMessage> listener) throws MessagingException {
        Map<String, String> headers = new HashMap<String, String>();
        String messageBody;
        String[] uids;
        ...
        if (startMessages.size() > 10) {
            List<WebDavMessage> newMessages =
                    new ArrayList<WebDavMessage>(startMessages.size() - 10);
            for (int i = 0, count = startMessages.size(); i < count; i++) {
                if (i < 10) {
        ...
        for (int i = messages.size() - 1; i >= 0; i--) {
            ...
            if (envelope != null) {
                ...
                Log.e(LOG_TAG,
                        "Asked to get metadata for a non-existent message: " + message.getUid());
            }
    ...
    public List<? extends Message> appendWebDavMessages(List<? extends Message> messages)
        ...
        for (Message message : messages) {
            ...
            try {
                ...
                messageURL +=
                        encodeUtf8(message.getUid() + ":" + System.currentTimeMillis() + ".eml");
