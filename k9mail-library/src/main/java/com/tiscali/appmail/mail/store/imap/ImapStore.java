...
public class ImapStore extends RemoteStore {
    ...
    /**
     * Cache of ImapFolder objects. ImapFolders are attached to a given folder on the server and as
     * long as their associated connection remains open they are reusable between requests. This
     * cache lets us make sure we always reuse, if possible, for a given folder name.
     */
    ...
    private Set<String> listFolders(ImapConnection connection, boolean subscribedOnly)
        ...
        List<ImapResponse> responses = connection.executeSimpleCommand(String.format("%s \"\" %s",
                commandResponse, ImapUtility.encodeString(getCombinedPrefix() + "*")));

        List<ListResponse> listResponses = (subscribedOnly) ? ListResponse.parseLsub(responses)
        ...
        for (ListResponse listResponse : listResponses) {
            ...
            if (folder.equalsIgnoreCase(mStoreConfig.getInboxFolderName())) {
                ...
                /*
                 * There is a folder on the server with the same name as our local outbox. Until we
                 * have a good plan to deal with this situation we simply ignore the folder on the
                 * server.
                 */
                continue;
            } else {
    ...
    void autoconfigureFolders(final ImapConnection connection)
        ...
        String command = String.format("LIST (SPECIAL-USE) \"\" %s",
        ...
        for (ListResponse listResponse : listResponses) {
            ...
            if (listResponse.hasAttribute("\\Archive") || listResponse.hasAttribute("\\All")) {
                ...
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Folder auto-configuration detected Archive folder: "
                            + decodedFolderName);
                }
            } else if (listResponse.hasAttribute("\\Drafts")) {
                mStoreConfig.setDraftsFolderName(decodedFolderName);
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Folder auto-configuration detected Drafts folder: "
                            + decodedFolderName);
                }
                ...
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG,
                            "Folder auto-configuration detected Sent folder: " + decodedFolderName);
                }
            } else if (listResponse.hasAttribute("\\Junk")) {
                mStoreConfig.setSpamFolderName(decodedFolderName);
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG,
                            "Folder auto-configuration detected Spam folder: " + decodedFolderName);
                }
            } else if (listResponse.hasAttribute("\\Trash")) {
                mStoreConfig.setTrashFolderName(decodedFolderName);
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Folder auto-configuration detected Trash folder: "
                            + decodedFolderName);
                }
            }
        }
    ...
    ImapConnection createImapConnection() {
        return new ImapConnection(new StoreImapSettings(), mTrustedSocketFactory,
                connectivityManager);
    }
