package com.tiscali.appmail.mail.power;
...
public class TracingPowerManager {
    ...
    public class TracingWakeLock {
        ...
        public void acquire(long timeout) {
            ...
            if (K9MailLib.isDebug()) {
                Log.v(LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + " for " + timeout
                        + " ms: acquired");
            }
            raiseNotification();
            if (startTime == null) {
        ...
        public void acquire() {
            ...
            raiseNotification();
            if (K9MailLib.isDebug()) {
                Log.w(LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id
        ...
        public void release() {
            if (startTime != null) {
                ...
                if (K9MailLib.isDebug()) {
                    Log.v(LOG_TAG,
                            "TracingWakeLock for tag " + tag + " / id " + id + ": releasing after "
                                    + (endTime - startTime) + " ms, timeout = " + timeout + " ms");
                }
            } else {
                if (K9MailLib.isDebug()) {
                    Log.v(LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ", timeout = "
                            + timeout + " ms: releasing");
                }
            }
            cancelNotification();
            synchronized (wakeLock) {
                            ...
                            if (startTime != null) {
                                ...
                                Log.i(LOG_TAG,
                                        "TracingWakeLock for tag " + tag + " / id " + id
                                                + ": has been active for " + (endTime - startTime)
