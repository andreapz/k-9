...
/**
 * Static methods for encoding header field values. This includes encoded-words as defined in
 * <a href='http://www.faqs.org/rfcs/rfc2047.html'>RFC 2047</a> or display-names of an e-mail
 * address, for example.
 *
 * This class is copied from the org.apache.james.mime4j.decoder.EncoderUtil class. It's modified
 * here in order to encode emoji characters in the Subject headers. The method to decode emoji
 * depends on the MimeMessage class because it has to be determined with the sender address.
 */
class EncoderUtil {
    ...
    /**
     * Encodes the specified text into an encoded word or a sequence of encoded words separated by
     * space. The text is separated into a sequence of encoded words if it does not fit in a single
     * one.
     *
     * @param text text to encode.
     * @param charset the Java charset that should be used to encode the specified string into a
     *        byte array. A suitable charset is detected automatically if this parameter is
     *        <code>null</code>.
     * @return the encoded word (or sequence of encoded words if the given text does not fit in a
     *         single encoded word).
     */
    ...
    private static String encodeB(String prefix, String text, Charset charset, byte[] bytes) {
        ...
        if (totalLength <= ENCODED_WORD_MAX_LENGTH) {
            return prefix + org.apache.james.mime4j.codec.EncoderUtil.encodeB(bytes)
                    + ENC_WORD_SUFFIX;
        } else {
    ...
    private static String encodeQ(String prefix, String text, Charset charset, byte[] bytes) {
        ...
        if (totalLength <= ENCODED_WORD_MAX_LENGTH) {
            return prefix
                    + org.apache.james.mime4j.codec.EncoderUtil.encodeQ(bytes,
