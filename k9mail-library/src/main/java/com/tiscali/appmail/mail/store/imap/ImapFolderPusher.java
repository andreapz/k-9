...
import static com.tiscali.appmail.mail.K9MailLib.PUSH_WAKE_LOCK_TIMEOUT;
...
class ImapFolderPusher extends ImapFolder {
    ...
    public void stop() {
        ...
        if (conn != null) {
            ...
            Log.w(LOG_TAG,
                    "Attempt to interrupt null connection to stop pushing on folderPusher for "
                            + getLogId());
        }
    }
    ...
    private class PushRunnable implements Runnable, UntaggedHandler {
        ...
        public void run() {
            ...
            while (!stop) {
                try {
                    ...
                    /*
                     * This makes sure 'oldUidNext' is never smaller than 'UIDNEXT' from the last
                     * loop iteration. This way we avoid looping endlessly causing the battery to
                     * drain.
                     *
                     * See issue 4907
                     */
                    ...
                    if (K9MailLib.isDebug()) {
                        Log.e(K9MailLib.LOG_TAG,
                                "Authentication failed. Stopping ImapFolderPusher.", e);
                    }
                    ...
                    if (stop) {
                        ...
                        if (idleFailureCount > IDLE_FAILURE_COUNT_LIMIT) {
                            Log.e(LOG_TAG, "Disabling pusher for " + getLogId() + " after "
                                    + idleFailureCount + " consecutive errors");
                            pushReceiver.pushError("Push disabled for " + getName() + " after "
                                    + idleFailureCount + " consecutive errors", e);
                            stop = true;
                        }
                    }
                }
        ...
        public void handleAsyncUntaggedResponse(ImapResponse response) {
            ...
            if (stop) {
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Got async untagged response: " + response
                ...
                if (response.getTag() == null) {
                    if (response.size() > 1) {
                        Object responseType = response.get(1);
                        if (equalsIgnoreCase(responseType, "EXISTS")
                                || equalsIgnoreCase(responseType, "EXPUNGE")
                                || equalsIgnoreCase(responseType, "FETCH")) {

                            wakeLock.acquire(PUSH_WAKE_LOCK_TIMEOUT);

                            if (K9MailLib.isDebug()) {
                                Log.d(LOG_TAG, "Got useful async untagged response: " + response
                                        + " for " + getLogId());
                            }
        ...
        private void processStoredUntaggedResponses() throws MessagingException {
            while (true) {
                ...
                if (K9MailLib.isDebug()) {
                    Log.i(LOG_TAG, "Processing " + untaggedResponses.size()
        ...
        private void processUntaggedResponses(List<ImapResponse> responses)
            ...
            for (ImapResponse response : responses) {
                oldMessageCount += processUntaggedResponse(oldMessageCount, response,
            ...
            if (K9MailLib.isDebug()) {
                Log.d(LOG_TAG, "UIDs for messages needing flag sync are " + flagSyncMsgSeqs
                        + "  for " + getLogId());
            }
        ...
        private int processUntaggedResponse(long oldMessageCount, ImapResponse response,
                List<Long> flagSyncMsgSeqs, List<String> removeMsgUids) {
            ...
            if (response.getTag() == null && response.size() > 1) {
                try {
                    ...
                    if (equalsIgnoreCase(responseType, "FETCH")) {
                        ...
                        if (K9MailLib.isDebug()) {
                            Log.d(LOG_TAG, "Got untagged FETCH for msgseq " + msgSeq + " for "
                                    + getLogId());
                        }
                    ...
                    if (equalsIgnoreCase(responseType, "EXPUNGE")) {
                        ...
                        if (K9MailLib.isDebug()) {
                            Log.d(LOG_TAG, "Got untagged EXPUNGE for msgseq " + msgSeq + " for "
                                    + getLogId());
                        }
                        ...
                        Collections.sort(msgSeqs); // Have to do comparisons in order because of
                                                   // msgSeq reductions

                        for (long msgSeqNum : msgSeqs) {
                            if (K9MailLib.isDebug()) {
                                Log.v(LOG_TAG,
                                        "Comparing EXPUNGEd msgSeq " + msgSeq + " to " + msgSeqNum);
                            }
                            ...
                            if (msgSeqNum == msgSeq) {
                                ...
                                if (K9MailLib.isDebug()) {
                                    Log.d(LOG_TAG, "Scheduling removal of UID " + uid
                                ...
                                if (K9MailLib.isDebug()) {
                                    Log.d(LOG_TAG, "Reducing msgSeq for UID " + uid + " from "
        ...
        private void syncMessages(int end) throws MessagingException {
            ...
            if (messageList != null && messageList.size() > 0) {
                ...
                if (K9MailLib.isDebug()) {
                    Log.i(LOG_TAG,
                            "Got newUid " + newUid + " for message " + end + " on " + getLogId());
                }
                ...
                if (newUid >= startUid) {
                    if (K9MailLib.isDebug()) {
                        Log.i(LOG_TAG, "Needs sync from uid " + startUid + " to " + newUid + " for "
                                + getLogId());
                    }
                    ...
                    for (long uid = startUid; uid <= newUid; uid++) {
                        ImapMessage message =
                                new ImapMessage(Long.toString(uid), ImapFolderPusher.this);
        ...
        private void removeMessages(List<String> removeUids) {
            ...
            try {
                ...
                for (Message existingMessage : existingMessages) {
                    ...
                    Log.w(LOG_TAG, "Message with UID " + existingUid
        ...
        private void notifyMessagesArrived(long startUid, long uidNext) {
            if (K9MailLib.isDebug()) {
                Log.i(LOG_TAG, "Needs sync from uid " + startUid + " to " + uidNext + " for "
                        + getLogId());
            }
    ...
    /**
     * Ensure the DONE continuation is only sent when the IDLE command was sent and hasn't completed
     * yet.
     */
