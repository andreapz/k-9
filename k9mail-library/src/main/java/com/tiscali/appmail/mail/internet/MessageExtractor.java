...
import static com.tiscali.appmail.mail.internet.CharsetSupport.fixupCharset;
import static com.tiscali.appmail.mail.internet.MimeUtility.getHeaderParameter;
import static com.tiscali.appmail.mail.internet.MimeUtility.isSameMimeType;
import static com.tiscali.appmail.mail.internet.Viewable.Alternative;
import static com.tiscali.appmail.mail.internet.Viewable.Html;
import static com.tiscali.appmail.mail.internet.Viewable.MessageHeader;
import static com.tiscali.appmail.mail.internet.Viewable.Text;
import static com.tiscali.appmail.mail.internet.Viewable.Textual;
...
public class MessageExtractor {
    ...
    private static String getTextFromTextPart(Part part, Body body, String mimeType,
            long textSizeLimit) throws IOException, MessagingException {
        /*
        ...
        /*
         * Now we read the part into a buffer for further processing. Because the stream is now
         * wrapped we'll remove any transfer encoding at this point.
         */
        InputStream in = MimeUtility.decodeBody(body);
        InputStream possiblyLimitedIn = textSizeLimit != NO_TEXT_SIZE_LIMIT
                ? new BoundedInputStream(in, textSizeLimit) : in;
        try {
    ...
    public static void findViewablesAndAttachments(Part part,
            ...
            @Nullable List<Part> outputNonViewableParts) throws MessagingException {
        ...
        if (skipSavingNonViewableParts && skipSavingViewableParts) {
            throw new IllegalArgumentException(
                    "method was called but no output is to be collected - this a bug!");
        }
        ...
        if (body instanceof Multipart) {
            ...
            if (isSameMimeType(part.getMimeType(), "multipart/alternative")) {
                ...
                List<Viewable> html =
                        findHtmlPart(multipart, knownTextParts, outputNonViewableParts, true);
                ...
                for (Part bodyPart : multipart.getBodyParts()) {
                    findViewablesAndAttachments(bodyPart, outputViewableParts,
                            outputNonViewableParts);
                }
            }
     ...
     * @param directChild If {@code true}, this method will return after the first
    ...
    private static List<Viewable> findTextPart(Multipart multipart, boolean directChild)
        ...
        for (Part part : multipart.getBodyParts()) {
            ...
            if (body instanceof Multipart) {
                 ...
                 * multipart/alternative we don't want to stop after the first text/plain part we
                 * find. This will allow to get all text parts for constructions like this:
                 *
                 * 1. multipart/alternative 1.1. multipart/mixed 1.1.1. text/plain 1.1.2. text/plain
                 * 1.2. text/html
                 */
                ...
                if (!textViewables.isEmpty()) {
                    ...
                    && isSameMimeType(part.getMimeType(), "text/plain")) {
                Text text = new Text(part);
                viewables.add(text);
                if (directChild) {
                    break;
    ...
    /**
     * Search the children of a {@link Multipart} for {@code text/html} parts. Every part that is
     * not a {@code text/html} we want to display, we add to 'attachments'.
     *
     * @param multipart The {@code Multipart} to search through.
     * @param knownTextParts A set of {@code text/plain} parts that shouldn't be added to
     *        'attachments'.
     * @param outputNonViewableParts A list that will receive the parts that are considered
     *        attachments.
     * @param directChild If {@code true}, this method will add all {@code text/html} parts except
     *        the first found to 'attachments'.
     *
     * @return A list of {@link Text} viewables.
     *
     * @throws MessagingException In case of an error.
            ...
            if (body instanceof Multipart) {
                ...
                if (directChild && partFound) {
                    if (saveNonViewableParts) {
                        // We already found our text/html part. Now we're only looking for
                        // attachments.
                     ...
                     * multipart/alternative we don't want to stop after the first text/html part we
                     * find. This will allow to get all text parts for constructions like this:
                     *
                     * 1. multipart/alternative 1.1. text/plain 1.2. multipart/mixed 1.2.1.
                     * text/html 1.2.2. text/html 1.3. image/jpeg
                     */
     ...
     *
     * @param viewables A list of {@link Viewable}s containing references to the message parts to
     *        include in the set.
     *
    ...
    private static Boolean isPartTextualBody(Part part) throws MessagingException {
        ...
        boolean isAttachmentDisposition =
                "attachment".equalsIgnoreCase(dispositionType) || dispositionFilename != null;
