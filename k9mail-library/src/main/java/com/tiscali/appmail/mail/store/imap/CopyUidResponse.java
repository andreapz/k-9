...
import static com.tiscali.appmail.mail.store.imap.ImapUtility.getImapSequenceValues;
...
class CopyUidResponse {
    ...
    public static CopyUidResponse parse(ImapResponse response) {
        if (!response.isTagged() || response.size() < 2
                || !equalsIgnoreCase(response.get(0), Responses.OK) || !response.isList(1)) {
            return null;
        }

        ImapList responseTextList = response.getList(1);
        if (responseTextList.size() < 4
                || !equalsIgnoreCase(responseTextList.get(0), Responses.COPYUID)
                || !responseTextList.isString(1) || !responseTextList.isString(2)
