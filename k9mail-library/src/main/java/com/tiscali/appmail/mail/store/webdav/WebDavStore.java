package com.tiscali.appmail.mail.store.webdav;

import static com.tiscali.appmail.mail.K9MailLib.DEBUG_PROTOCOL_WEBDAV;
...
public class WebDavStore extends RemoteStore {
    ...
    public static WebDavStoreSettings decodeUri(String uri) {
         ...
         * re-map them appropriately: webdav+tls webdav+tls+ webdav+ssl
         */
    ...
    public static String createUri(ServerSettings server) {
        ...
        try {
            return new URI(scheme, userInfo, server.host, server.port, uriPath, null, null)
                    .toString();
    ...
    private String mAuthPath; /*
                               * Stores the path off of the server to post data to for form based
                               * authentication
                               */
    ...
    public WebDavStore(StoreConfig storeConfig,
            WebDavHttpClient.WebDavHttpClientFactory clientFactory) throws MessagingException {
        ...
        if (mAuthPath != null && !mAuthPath.equals("") && !mAuthPath.startsWith("/")) {
            mAuthPath = "/" + mAuthPath;
        }
    ...
    public List<? extends Folder> getPersonalNamespaces(boolean forceListAll)
        ...
        /**
         * Firstly we get the "special" folders list (inbox, outbox, etc) and setup the account
         * accordingly
         */
        ...
        String folderName =
                getFolderName(specialFoldersMap.get(WebDavConstants.DAV_MAIL_INBOX_FOLDER));
        if (folderName != null) {
        ...
        /*
         * folderName = getFolderName(specialFoldersMap.get(DAV_MAIL_OUTBOX_FOLDER)); if (folderName
         * != null) mAccount.setOutboxFolderName(folderName);
         */
    ...
    /**
     * Creates a folder using the URL passed as parameter (only if it has not been already created)
     * and adds this to our store folder map.
     *
     * @param folderUrl
     * @return
     */
    ...
    private String getSpecialFoldersList() {
        ...
        builder.append("<prop>");
        builder.append("<").append(WebDavConstants.DAV_MAIL_INBOX_FOLDER)
                .append(" xmlns=\"urn:schemas:httpmail:\"/>");
        builder.append("<").append(WebDavConstants.DAV_MAIL_DRAFTS_FOLDER)
                .append(" xmlns=\"urn:schemas:httpmail:\"/>");
        builder.append("<").append(WebDavConstants.DAV_MAIL_OUTBOX_FOLDER)
                .append(" xmlns=\"urn:schemas:httpmail:\"/>");
        builder.append("<").append(WebDavConstants.DAV_MAIL_SENT_FOLDER)
                .append(" xmlns=\"urn:schemas:httpmail:\"/>");
        builder.append("<").append(WebDavConstants.DAV_MAIL_TRASH_FOLDER)
                .append(" xmlns=\"urn:schemas:httpmail:\"/>");
        ...
        builder.append("<").append(WebDavConstants.DAV_MAIL_SPAM_FOLDER)
                .append(" xmlns=\"urn:schemas:httpmail:\"/>");
            ...
            if (mAuthentication == WebDavConstants.AUTH_TYPE_NONE) {
                ...
                if (info.requiredAuthType == WebDavConstants.AUTH_TYPE_BASIC) {
                    ...
                    if (statusCode >= 200 && statusCode < 300) {
                        ...
                        throw new MessagingException(
                                "Error with code " + response.getStatusLine().getStatusCode()
    ...
    /**
     * Makes the initial connection to Exchange for authentication. Determines the type of
     * authentication necessary for the server.
     *
     * @throws MessagingException
     */
    ...
    public void doFBA(ConnectionInfo info) throws IOException, MessagingException {
        ...
        if (!authenticated) {
            ...
            String formAction =
                    findFormAction(WebDavHttpClient.getUngzippedContent(response.getEntity()));
            if (formAction == null) {
                // If there is no form action, try using our redirect URL from the initial
                // connection.
                if (info != null && info.redirectUrl != null && !info.redirectUrl.equals("")) {
                    ...
                    formAction = findFormAction(
                            WebDavHttpClient.getUngzippedContent(response.getEntity()));
                }
            }
            if (formAction != null) {
                try {
                    ...
                    if (formActionUri.isAbsolute()) {
                        ...
                        // Reconstruct the login URL based on the original login URL and the form
                        // action.
                        URI finalUri = new URI(loginUri.getScheme(), loginUri.getUserInfo(),
                                loginUri.getHost(), loginUri.getPort(), urlPath, null, null);
                    ...
                    Log.e(LOG_TAG, "URISyntaxException caught " + e + "\nTrace: "
                            + WebDavUtils.processException(e));
    ...
    private boolean testAuthenticationResponse(HttpResponse response) throws MessagingException {
        ...
        if (((statusCode >= 200 && statusCode < 300) || statusCode == 302) && mAuthCookies != null
                && !mAuthCookies.getCookies().isEmpty()) {
            // We may be authenticated, we need to send a test request to know for sure.
            // Exchange 2007 adds the same cookies whether the username and password were valid or
            // not.
            ConnectionInfo info = doInitialConnection();
            if (info.statusCode >= 200 && info.statusCode < 300) {
                authenticated = true;
            } else if (info.statusCode == 302) {
                // If we are successfully authenticated, Exchange will try to redirect us to our OWA
                // inbox.
    ...
    private InputStream sendRequest(String url, String method, StringEntity messageBody,
            Map<String, String> headers, boolean tryAuth) throws MessagingException {
        ...
        try {
            ...
            Log.e(LOG_TAG, "UnsupportedEncodingException: " + uee + "\nTrace: "
                    + WebDavUtils.processException(uee));
    ...
    /**
     * Performs an httprequest to the supplied url using the supplied method. messageBody and
     * headers are optional as not all requests will need them. There are two signatures to support
     * calls that don't require parsing of the response.
     */
    DataSet processRequest(String url, String method, String messageBody,
            Map<String, String> headers) throws MessagingException {
        return processRequest(url, method, messageBody, headers, true);
    }

    DataSet processRequest(String url, String method, String messageBody,
            Map<String, String> headers, boolean needsParsing) throws MessagingException {
        DataSet dataset = new DataSet();
        if (K9MailLib.isDebug() && DEBUG_PROTOCOL_WEBDAV) {
            Log.v(LOG_TAG, "processRequest url = '" + url + "', method = '" + method
        ...
        try {
            ...
            if (istream != null && needsParsing) {
                try {
                    ...
                    spf.setNamespaceAware(true); // This should be a no-op on Android, but makes the
                                                 // tests work
                    ...
                    Log.e(LOG_TAG, "SAXException in processRequest() " + se + "\nTrace: "
                            + WebDavUtils.processException(se));
                    ...
                    throw new MessagingException(
                            "ParserConfigurationException in processRequest() ", pce);
                }
