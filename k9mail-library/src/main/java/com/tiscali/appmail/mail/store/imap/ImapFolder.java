...
import static com.tiscali.appmail.mail.store.imap.ImapUtility.getLastResponse;
...
import com.tiscali.appmail.mail.filter.EOLConvertingOutputStream;
...
class ImapFolder extends Folder<ImapMessage> {
    private static final ThreadLocal<SimpleDateFormat> RFC3501_DATE =
    ...
    protected List<ImapResponse> internalOpen(int mode) throws MessagingException {
        ...
        try {
            ...
            /*
             * If the command succeeds we expect the folder has been opened read-write unless we are
             * notified otherwise in the responses.
             */
            this.mode = mode;
    ...
    private boolean exists(String escapedFolderName) throws MessagingException {
        try {
            // Since we don't care about RECENT, we'll use that for the check, because we're
            // checking
    ...
    public boolean exists() throws MessagingException {
        ...
        /*
         * This method needs to operate in the unselected mode as well as the selected mode so we
         * must get the connection ourselves if it's not there. We are specifically not calling
         * checkOpen() since we don't care if the folder is open.
         */
        ImapConnection connection;
        synchronized (this) {
            if (this.connection == null) {
        ...
        try {
            ...
            connection.executeSimpleCommand(
                    String.format("STATUS %s (UIDVALIDITY)", escapedFolderName));
     ...
     *
     * <p>
     * <strong>Note:</strong> Only the UIDs of the given {@link Message} instances are used. It is
     * assumed that all UIDs represent valid messages in this folder.
     * </p>
     *
     * @param messages The messages to copy to the specfied folder.
    ...
    public Map<String, String> copyMessages(List<? extends Message> messages, Folder folder)
        ...
        try {
            String encodedDestinationFolderName =
                    folderNameCodec.encode(imapFolder.getPrefixedName());
            String escapedDestinationFolderName =
    ...
    public void delete(List<? extends Message> messages, String trashFolderName)
        ...
        if (trashFolderName == null || getName().equalsIgnoreCase(trashFolderName)) {
            ...
            String encodedTrashFolderName =
                    folderNameCodec.encode(remoteTrashFolder.getPrefixedName());
            String escapedTrashFolderName = ImapUtility.encodeString(encodedTrashFolderName);

            if (!exists(escapedTrashFolderName)) {
                if (K9MailLib.isDebug()) {
                    Log.i(LOG_TAG, "IMAPMessage.delete: attempting to create remote '"
            ...
            if (exists(escapedTrashFolderName)) {
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "IMAPMessage.delete: copying remote " + messages.size()
                ...
                throw new MessagingException(
                        "IMAPMessage.delete: remote Trash folder " + trashFolderName
    ...
    protected List<ImapMessage> getMessages(final int start, final int end, Date earliestDate,
        ...
        ImapSearcher searcher = new ImapSearcher() {
            ...
            public List<ImapResponse> search() throws IOException, MessagingException {
                String command = String.format(Locale.US, "UID SEARCH %d:%d%s%s", start, end,
                        dateSearchString, includeDeleted ? "" : " NOT DELETED");
    ...
    private boolean existsNonDeletedMessageInRange(int startIndex, int endIndex,
            String dateSearchString) throws MessagingException, IOException {

        String command = String.format(Locale.US, "SEARCH %d:%d%s NOT DELETED", startIndex,
                endIndex, dateSearchString);
        List<ImapResponse> responses = executeSimpleCommand(command);

        for (ImapResponse response : responses) {
            if (response.getTag() == null
                    && ImapResponseParser.equalsIgnoreCase(response.get(0), "SEARCH")) {
                if (response.size() > 1) {
                    return true;
                }
            }
    ...
    protected List<ImapMessage> getMessagesFromUids(final List<String> mesgUids)
        ...
        ImapSearcher searcher = new ImapSearcher() {
            ...
            public List<ImapResponse> search() throws IOException, MessagingException {
                String command =
                        String.format("UID SEARCH UID %s", combine(mesgUids.toArray(), ','));
    ...
    public void fetch(List<ImapMessage> messages, FetchProfile fetchProfile,
        ...
        String spaceSeparatedFetchFields =
                combine(fetchFields.toArray(new String[fetchFields.size()]), ' ');

        for (int windowStart = 0; windowStart < messages.size(); windowStart +=
                (FETCH_WINDOW_SIZE)) {
            ...
            try {
                String commaSeparatedUids =
                        combine(uidWindow.toArray(new String[uidWindow.size()]), ',');
                String command = String.format("UID FETCH %s (%s)", commaSeparatedUids,
                    ...
                    if (response.getTag() == null
                        ...
                        if (uid != null) {
                            try {
                                ...
                                if (K9MailLib.isDebug()) {
                                    Log.v(LOG_TAG, "Stored uid '" + uid + "' for msgSeq " + msgSeq
                                            + " into map");
                                }
                            } catch (Exception e) {
                                Log.e(LOG_TAG,
                                        "Unable to store uid '" + uid + "' for msgSeq " + msgSeq);
                            }
                        }
                        ...
                        if (message == null) {
                            if (K9MailLib.isDebug()) {
                                Log.d(LOG_TAG, "Do not have message in messageMap for UID " + uid
                                        + " for " + getLogId());
                            }
    ...
    protected void handleUntaggedResponse(ImapResponse response) {
        if (response.getTag() == null && response.size() > 1) {
            if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXISTS")) {
                ...
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Got untagged EXISTS with value " + messageCount + " for "
                            + getLogId());
                }
            }
            ...
            if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXPUNGE")
                ...
                messageCount--;
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Got untagged EXPUNGE with messageCount " + messageCount
                            + " for " + getLogId());
                }
            }
        }
        ...
        if (bs.get(0) instanceof ImapList) {
            ...
            for (int i = 0, count = bs.size(); i < count; i++) {
                if (bs.get(i) instanceof ImapList) {
                    /*
                     * For each part in the message we're going to add a new BodyPart and parse into
                     * it.
                     */
                    ...
                    /*
                     * We've got to the end of the children of the part, so now we can find out what
                     * type it is and bail out.
                     */
            ...
            /*
             * This is a body. We need to add as much information as we can find out about it to the
             * Part.
             */

            /*
             * 0| 0 body type 1| 1 body subtype 2| 2 body parameter parenthesized list 3| 3 body id
             * (unused) 4| 4 body description (unused) 5| 5 body encoding 6| 6 body size -| 7 text
             * lines (only for type TEXT, unused) Extensions (optional): 7| 8 body MD5 (unused) 8| 9
             * body disposition 9|10 body language (unused) 10|11 body location (unused)
             */
            ...
            if (MimeUtility.isMessage(mimeType)) {
                ...
                // [MESSAGE, RFC822, [NAME, Fwd: [#HTR-517941]: update plans at 1am Friday - Memory
                // allocation - displayware.eml], NIL, NIL, 7BIT, 5974, NIL, [INLINE, [FILENAME*0,
                // Fwd: [#HTR-517941]: update plans at 1am Friday - Memory all, FILENAME*1, ocation
                // - displayware.eml]], NIL]
                /*
            ...
            if (bodyParams != null) {
                /*
                 * If there are body params we might be able to get some more information out of
                 * them.
                 */
            ...
            /*
             * Set the content disposition containing at least the size. Attachment handling code
             * will use this down the road.
             */
            part.setHeader(MimeHeader.HEADER_CONTENT_DISPOSITION, contentDisposition.toString());

            /*
             * Set the Content-Transfer-Encoding header. Attachment code will use this to parse the
             * body.
             */
     ...
     *
     * <p>
     * This implementation also determines the new UIDs of the given messages on the IMAP server and
     * changes the messages' UIDs to the new server UIDs.
     * </p>
     *
    ...
    public Map<String, String> appendMessages(List<? extends Message> messages)
        ...
        try {
            ...
            for (Message message : messages) {
                ...
                if (response.size() > 1) {
                    /*
                     * If the server supports UIDPLUS, then along with the APPEND response it will
                     * return an APPENDUID response code, e.g.
                     *
                ...
                /*
                 * This part is executed in case the server does not support UIDPLUS or does not
                 * implement the APPENDUID response code.
                 */
            ...
            /*
             * We need uidMap to be null if new UIDs are not available to maintain consistency with
             * the behavior of other similar methods (copyMessages, moveMessages) which return null.
             */
    ...
    public String getUidFromMessageId(Message message) throws MessagingException {
        try {
            ...
            if (messageIdHeader.length == 0) {
                if (K9MailLib.isDebug()) {
                    Log.d(LOG_TAG, "Did not get a message-id in order to search for UID  for "
                            + getLogId());
                }
                return null;
            }
            ...
            if (K9MailLib.isDebug()) {
                Log.d(LOG_TAG, "Looking for UID for message with message-id " + messageId + " for "
                        + getLogId());
            }

            String command = String.format("UID SEARCH HEADER MESSAGE-ID %s",
    ...
    public void setFlags(Set<Flag> flags, boolean value) throws MessagingException {
        ...
        try {
            String command = String.format("UID STORE 1:* %sFLAGS.SILENT (%s)", value ? "+" : "-",
                    combineFlags(flags));
    ...
    public void setFlags(List<? extends Message> messages, final Set<Flag> flags, boolean value)
        ...
        try {
            String command = String.format("UID STORE %s %sFLAGS.SILENT (%s)", combine(uids, ','),
    ...
    protected String getLogId() {
        String id = store.getStoreConfig().toString() + ":" + getName() + "/"
