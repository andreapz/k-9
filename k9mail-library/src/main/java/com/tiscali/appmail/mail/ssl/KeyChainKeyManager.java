...
import static com.tiscali.appmail.mail.CertificateValidationException.Reason;
import static com.tiscali.appmail.mail.CertificateValidationException.Reason.RetrievalFailure;
...
/**
 * For client certificate authentication! Provide private keys and certificates during the TLS
 * handshake using the Android 4.0 KeyChain API.
 */
class KeyChainKeyManager extends X509ExtendedKeyManager {
    ...
    private PrivateKey fetchPrivateKey(Context context, String alias)
        ...
        /*
         * We need to keep reference to the first private key retrieved so it won't get garbage
         * collected. If it will then the whole app will crash on Android < 4.2 with
         * "Fatal signal 11 code=1". See https://code.google.com/p/android/issues/detail?id=62319
         */
    ...
    private String chooseAlias(String[] keyTypes, Principal[] issuers) {
        ...
        for (String keyAlgorithm : keyTypes) {
            ...
            /*
             * TODO find a more reliable test for signature algorithm. Unfortunately value varies
             * with provider. For example for "EC" it could be "SHA1WithECDSA" or simply "ECDSA".
             */
            ...
            for (X509Certificate certFromChain : mChain) {
                 ...
                 * xcertFromChain.getIssuerDN is a bouncycastle org.bouncycastle.jce.X509Principal.
                 */
            ...
            Log.w(LOG_TAG,
                    "Client certificate " + mAlias + " not issued by any of the requested issuers");
            return null;
        }
        Log.w(LOG_TAG,
                "Client certificate " + mAlias + " does not match any of the requested key types");
        return null;
    }
}
