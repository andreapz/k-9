...
public class Address implements Serializable {
    private static final Pattern ATOM =
        ...
        if (parse) {
            ...
            if (tokens.length > 0) {
                ...
                if (!TextUtils.isEmpty(name)) {
                    /*
                     * Don't use the "personal" argument if "address" is of the form: James Bond
                     * <james.bond@mi6.uk>
                     *
                     * See issue 2920
                     */
    ...
    /**
     * Parse a comma separated list of email addresses in human readable format and return an array
     * of Address objects, RFC-822 encoded.
     *
     * @param addressList
    ...
    /**
     * Parse a comma separated list of addresses in RFC-822 format and return an array of Address
     * objects.
     *
     * @param addressList
     * @return An array of 0 or more Addresses.
            ...
            for (int i = 0, count = parsedList.size(); i < count; i++) {
                ...
                if (address instanceof Mailbox) {
                    ...
                    addresses.add(new Address(mailbox.getLocalPart() + "@" + mailbox.getDomain(),
                            mailbox.getName(), false));
                } else {
                    Log.e(LOG_TAG,
                            "Unknown address type from Mime4J: " + address.getClass().toString());
                }
            }
        } catch (MimeException pe) {
            Log.e(LOG_TAG, "MimeException in Address.parse()", pe);
            // but we do an silent failover : we just use the given string as name with empty
            // address
    ...
    /**
     * Packs an address list into a String that is very quick to read and parse. Packed lists can be
     * unpacked with unpackAddressList() The packed list is a ",\u0000" separated list of:
     * address;\u0000personal
     * 
     ...
     * already there. sample -> "sample" "sample" -> "sample" ""sample"" -> ""sample"" "sample"" ->
     * "sample" sa"mp"le -> "sa"mp"le" "sa"mp"le" -> "sa"mp"le" (empty string) -> "" " -> """
     * 
     * @param s
     * @return
     */
