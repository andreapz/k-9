...
class ImapResponseParser {
    ...
    /**
     * Reads the next response available on the stream and returns an {@code ImapResponse} object
     * that represents it.
     */
        ...
        do {
            ...
            if (response.getTag() != null && !response.getTag().equalsIgnoreCase(tag)) {
                Log.w(LOG_TAG, "After sending tag " + tag
                        + ", got tag response from previous command " + response + " for " + logId);
     ...
     * <p>
     * Responses "OK", "PREAUTH", "BYE", "NO", "BAD", and continuation request responses can contain
     * {@code resp-text} tokens. We parse the {@code resp-text-code} part as tokens and read the
     * rest as sequence of characters to avoid the parser interpreting things like "{123}" as start
     * of a literal.
     * </p>
     * <p>
     * Example:
    ...
    /**
     * Reads the next token of the response. The token can be one of: String - for NIL, QUOTED,
     * NUMBER, ATOM. Object - for LITERAL. ImapList - for PARENTHESIZED LIST. Can contain any of the
     * above elements including List.
     *
    ...
    private String parseBareString(boolean allowBrackets) throws IOException {
        ...
        while (true) {
            ...
            if (ch == '(' || ch == ')' || (allowBrackets && (ch == '[' || ch == ']')) || ch == '{'
                    || ch == ' ' || ch == '"' || (ch >= 0x00 && ch <= 0x1f) || ch == 0x7f) {
    ...
    private boolean isStatusResponse(String symbol) {
        return symbol.equalsIgnoreCase(Responses.OK) || symbol.equalsIgnoreCase(Responses.NO)
