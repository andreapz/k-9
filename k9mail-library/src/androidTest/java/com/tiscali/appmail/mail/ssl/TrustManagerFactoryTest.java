...
public class TrustManagerFactoryTest {
    public static final String MATCHING_HOST = "appmail.example.com";
    ...
    private static final String K9_EXAMPLE_COM_CERT1 = "-----BEGIN CERTIFICATE-----\n"
    ...
    private X509Certificate loadCert(String encodedCert) throws CertificateException {
        ...
        return (X509Certificate) certFactory
                .generateCertificate(new ByteArrayInputStream(encodedCert.getBytes()));
    }
    ...
    public void tearDown() {
        if (mKeyStoreFile.exists() && !mKeyStoreFile.delete()) {
            throw new RuntimeException(
                    "Unable to delete key store file: " + mKeyStoreFile.getAbsolutePath());
        }
    }
     ...
     *
     * <p>
     * This test is to make sure entries in the keystore file aren't overwritten. See
     * <a href="https://code.google.com/p/k9mail/issues/detail?id=1326">Issue 1326</a>.
     * </p>
     *
    ...
    public void testGloballyTrustedCertificateChain() throws Exception {
        ...
        X509Certificate[] certificates =
                new X509Certificate[] {mLinuxComCert, mLinuxComFirstParentCert};
    ...
    public void testGloballyTrustedCertificateNotMatchingHost() throws Exception {
        X509TrustManager trustManager = TrustManagerFactory.get(NOT_MATCHING_HOST, PORT1);
        assertCertificateRejection(trustManager,
                new X509Certificate[] {mLinuxComCert, mLinuxComFirstParentCert});
    }
