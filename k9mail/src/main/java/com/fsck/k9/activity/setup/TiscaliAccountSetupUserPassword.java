...
public class TiscaliAccountSetupUserPassword extends K9Activity implements OnClickListener {
    private static final String EXTRA_ACCOUNT = "com.tiscali.appmail.TiscaliAccountSetupUserPassword.account";
    private static final String STATE_KEY_CHECKED_INCOMING = "com.tiscali.appmail.TiscaliAccountSetupUserPassword.checkedIncoming";
    ...
    private void showPassword(boolean showPassword) {
        mPasswordView.setInputType(showPassword ? InputType.TYPE_TEXT_VARIATION_PASSWORD : InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
        ...
        if (resultCode == RESULT_OK) {
            if (!mCheckedIncoming) {
        		...
        		try {
                    ...
                    URI uri = new URI(
                        oldUri.getScheme(),
                        usernameEnc + ":" + passwordEnc + ":CRAM_MD5",
                        oldUri.getHost(),
                        oldUri.getPort(),
                        null,
                        null,
                ...
                mAccount.setEmail(mUsernameView.getText().toString() + getResources().getString(R.string.account_setup_basics_domain_tiscali));
            } else {
            	/*
    ...
    protected void onNext() {
        try {
            ...
            extra.put(ImapStoreSettings.AUTODETECT_NAMESPACE_KEY, Boolean.toString(imapSettings.autoDetectNamespace));
            extra.put(ImapStoreSettings.PATH_PREFIX_KEY, imapSettings.pathPrefix);
            
            settings = new ServerSettings(ServerSettings.Type.IMAP, host, port, connectionSecurity, authType, username, password, clientCertificateAlias);
    ...
    private void failure(Exception use) {
        ...
        Toast toast = Toast.makeText(getApplication(), getString(R.string.account_setup_bad_uri, use.getMessage()), Toast.LENGTH_LONG);
        toast.show();
    }
}