...
class TextBodyBuilder {
     ...
     *
     * @return {@link com.tiscali.appmail.mail.internet.TextBody} instance that contains the entered
     *         text and possibly the quoted original message.
     */
    public TextBody buildTextHtml() {
        ...
        if (mIncludeQuotedText) {
            ...
            /*
             * Set the insertion location based upon our reply after quote setting. Additionally,
             * add some extra separators between the composed message and quoted message depending
             * on the quote location. We only add the extra separators when we're sending, that way
             * when we load a draft, we don't have to know the length of the separators to remove
             * them before editing.
             */
            if (mReplyAfterQuote) {
                quotedHtmlContent
