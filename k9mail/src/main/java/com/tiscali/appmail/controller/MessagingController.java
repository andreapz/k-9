package com.tiscali.appmail.controller;
...
import com.tiscali.appmail.cache.EmailProviderCache;
import com.tiscali.appmail.helper.Contacts;
import com.tiscali.appmail.mail.Address;
import com.tiscali.appmail.mail.AuthenticationFailedException;
import com.tiscali.appmail.mail.CertificateValidationException;
import com.tiscali.appmail.mail.FetchProfile;
import com.tiscali.appmail.mail.FetchProfile.Item;
import com.tiscali.appmail.mail.Flag;
import com.tiscali.appmail.mail.Folder;
import com.tiscali.appmail.mail.Folder.FolderType;
import com.tiscali.appmail.mail.Message;
import com.tiscali.appmail.mail.Message.RecipientType;
import com.tiscali.appmail.mail.MessageRetrievalListener;
import com.tiscali.appmail.mail.MessagingException;
import com.tiscali.appmail.mail.Part;
import com.tiscali.appmail.mail.PushReceiver;
import com.tiscali.appmail.mail.Pusher;
import com.tiscali.appmail.mail.Store;
import com.tiscali.appmail.mail.Transport;
import com.tiscali.appmail.mail.internet.MessageExtractor;
import com.tiscali.appmail.mail.internet.MimeMessage;
import com.tiscali.appmail.mail.internet.MimeMessageHelper;
import com.tiscali.appmail.mail.internet.MimeUtility;
import com.tiscali.appmail.mail.internet.TextBody;
import com.tiscali.appmail.mail.power.TracingPowerManager;
import com.tiscali.appmail.mail.power.TracingPowerManager.TracingWakeLock;
import com.tiscali.appmail.mail.store.pop3.Pop3Store;
import com.tiscali.appmail.mailstore.LocalFolder;
import com.tiscali.appmail.mailstore.LocalFolder.MoreMessages;
import com.tiscali.appmail.mailstore.LocalMessage;
import com.tiscali.appmail.mailstore.LocalStore;
import com.tiscali.appmail.mailstore.LocalStore.PendingCommand;
import com.tiscali.appmail.mailstore.MessageRemovalListener;
import com.tiscali.appmail.mailstore.UnavailableStorageException;
import com.tiscali.appmail.notification.NotificationController;
import com.tiscali.appmail.provider.EmailProvider;
import com.tiscali.appmail.provider.EmailProvider.StatsColumns;
import com.tiscali.appmail.search.ConditionsTreeNode;
...
import com.tiscali.appmail.search.SqlQueryBuilder;
...
/**
 * Starts a long running (application) Thread that will run through commands that require remote
 * mailbox access. This class is used to serialize and prioritize these commands. Each method that
 * will submit a command requires a MessagingListener instance to be provided. It is expected that
 * that listener has also been added as a registered listener using addListener(). When a command is
 * to be executed, if the listener that was provided with the command is no longer registered the
 * command is skipped. The design idea for the above is that when an Activity starts it registers as
 * a listener. When it is paused it removes itself. Thus, any commands that that activity submitted
 * are removed from the queue once the activity is no longer active.
 */
...
public class MessagingController {
    ...
    private static final Set<Flag> SYNC_FLAGS =
            EnumSet.of(Flag.SEEN, Flag.FLAGGED, Flag.ANSWERED, Flag.FORWARDED);

    private static final String PENDING_COMMAND_MOVE_OR_COPY =
            "com.tiscali.appmail.MessagingController.moveOrCopy";
    private static final String PENDING_COMMAND_MOVE_OR_COPY_BULK =
            "com.tiscali.appmail.MessagingController.moveOrCopyBulk";
    private static final String PENDING_COMMAND_MOVE_OR_COPY_BULK_NEW =
            "com.tiscali.appmail.MessagingController.moveOrCopyBulkNew";
    private static final String PENDING_COMMAND_EMPTY_TRASH =
            "com.tiscali.appmail.MessagingController.emptyTrash";
    private static final String PENDING_COMMAND_SET_FLAG_BULK =
            "com.tiscali.appmail.MessagingController.setFlagBulk";
    private static final String PENDING_COMMAND_SET_FLAG =
            "com.tiscali.appmail.MessagingController.setFlag";
    private static final String PENDING_COMMAND_APPEND =
            "com.tiscali.appmail.MessagingController.append";
    private static final String PENDING_COMMAND_MARK_ALL_AS_READ =
            "com.tiscali.appmail.MessagingController.markAllAsRead";
    private static final String PENDING_COMMAND_EXPUNGE =
            "com.tiscali.appmail.MessagingController.expunge";
    ...
    private final MemorizingMessagingListener memorizingMessagingListener =
            new MemorizingMessagingListener();
    ...
    public synchronized static MessagingController getInstance(Context context) {
        if (inst == null) {
            ...
            NotificationController notificationController =
                    NotificationController.newInstance(appContext);
        ...
        while (!stopped) {
            ...
            try {
                ...
                if (command != null) {
                    ...
                    if (K9.DEBUG) {
                        Log.i(K9.LOG_TAG, "Running command '" + command.description + "', seq = "
                                + command.sequence + "("
    ...
    private boolean isMessageSuppressed(LocalMessage message) {
        ...
        EmailProviderCache cache =
                EmailProviderCache.getCache(message.getFolder().getAccountUuid(), context);
        return cache.isMessageHidden(messageId, folderId);
    }

    private void setFlagInCache(final Account account, final List<Long> messageIds, final Flag flag,
    ...
    /**
     * Lists folders that are available locally and remotely. This method calls listFoldersCallback
     * for local folders before it returns, and then for remote folders at some later point. If
     * there are no local folders includeRemote is forced by this method. This method should be
     * called from a Thread as it may take several seconds to list the local folders. TODO this
     * needs to cache the remote folder list
     */
     ...
     * Lists folders that are available locally and remotely. This method calls listFoldersCallback
     * for local folders before it returns, and then for remote folders at some later point. If
     * there are no local folders includeRemote is forced by this method. This method is called in
     * the foreground. TODO this needs to cache the remote folder list
     */
    ...
    @VisibleForTesting
    void searchLocalMessagesSynchronous(final LocalSearch search,
            final MessagingListener listener) {
        ...
        for (final Account account : accounts) {
            ...
            MessageRetrievalListener<LocalMessage> retrievalListener =
                    new MessageRetrievalListener<LocalMessage>() {
                        @Override
    ...
    public Future<?> searchRemoteMessages(final String acctUuid, final String folderName,
            final String query, final Set<Flag> requiredFlags, final Set<Flag> forbiddenFlags,
            final MessagingListener listener) {
        if (K9.DEBUG) {
            String msg = "searchRemoteMessages (" + "acct=" + acctUuid + ", folderName = "
                    + folderName + ", query = " + query + ")";
        ...
        return threadPool.submit(new Runnable() {
            @Override
            public void run() {
                searchRemoteMessagesSynchronous(acctUuid, folderName, query, requiredFlags,
    ...
    @VisibleForTesting
    void searchRemoteMessagesSynchronous(final String acctUuid, final String folderName,
            final String query, final Set<Flag> requiredFlags, final Set<Flag> forbiddenFlags,
            final MessagingListener listener) {
        ...
        try {
            ...
            if (listener != null) {
                listener.remoteSearchFinished(folderName, 0, acct.getRemoteSearchNumResults(),
                        extraResults);
            }
        }
    ...
    public void loadSearchResults(final Account account, final String folderName,
            final List<Message> messages, final MessagingListener listener) {
        threadPool.execute(new Runnable() {
            @Override
            public void run() {
                if (listener != null) {
    ...
    private void loadSearchResultsSynchronous(List<Message> messages, LocalFolder localFolder,
        ...
        for (Message message : messages) {
            ...
            if (listener != null) {
                listener.remoteSearchAddMessage(remoteFolder.getName(), localMsg, i,
                        messages.size());
            }
        }
    }
    ...
    public void loadMoreMessages(Account account, String folder, MessagingListener listener) {
        try {
            ...
            if (localFolder.getVisibleLimit() > 0) {
                localFolder
                        .setVisibleLimit(localFolder.getVisibleLimit() + account.getDisplayCount());
            }
    ...
    /**
     * Start foreground synchronization of the specified folder. This is generally only called by
     * synchronizeMailbox.
     *
     * TODO Break this method up into smaller chunks.
     */
    @VisibleForTesting
    void synchronizeMailboxSynchronous(final Account account, final String folder,
        ...
        try {
            ...
            /*
             * Get the message list from the local store and create an index of the uids within the
             * list.
             */
            if (K9.DEBUG)
            ...
            if (providedRemoteFolder != null) {
                 ...
                 *
                 * Open the folder Upload any local messages that are marked as PENDING_UPLOAD
                 * (Drafts, Sent, Trash) Get the message count Get the list of the newest
                 * K9.DEFAULT_VISIBLE_LIMIT messages getMessages(messageCount -
                 * K9.DEFAULT_VISIBLE_LIMIT, messageCount) See if we have each message locally, if
                 * not fetch it's flags and envelope Get and update the unread count for the folder
                 * Update the remote flags of any messages we have locally with an internal date
                 * newer than the remote message. Get the current flags for any messages we have
                 * locally but did not just download Update local flags For any message we have
                 * locally but not remotely, delete the local message to keep cache clean. Download
                 * larger parts of any new messages. (Optional) Download small attachments in the
                 * background.
                 */
                ...
                if (Expunge.EXPUNGE_ON_POLL == account.getExpungePolicy()) {
                    if (K9.DEBUG)
                        Log.d(K9.LOG_TAG, "SYNC: Expunging folder " + account.getDescription() + ":"
                                + folder);
            ...
            if (remoteMessageCount > 0) {
                ...
                if (K9.DEBUG)
                    Log.v(K9.LOG_TAG, "SYNC: About to get messages " + remoteStart + " through "
                            + remoteMessageCount + " for folder " + folder);
                ...
                List<? extends Message> remoteMessageArray = remoteFolder.getMessages(remoteStart,
                        remoteMessageCount, earliestDate, null);
                ...
                if (K9.DEBUG)
                    Log.v(K9.LOG_TAG,
                            "SYNC: Got " + remoteUidMap.size() + " messages for folder " + folder);
                ...
                throw new Exception(
                        "Message count " + remoteMessageCount + " for folder " + folder);
            }

            /*
             * Remove any messages that are in the local store but no longer on the remote store or
             * are too old
             */
            ...
            if (K9.DEBUG)
                Log.d(K9.LOG_TAG, "Done synchronizing folder " + account.getDescription() + ":"
            ...
            if (K9.DEBUG)
                Log.i(K9.LOG_TAG,
                        "Done synchronizing folder " + account.getDescription() + ":" + folder);
            ...
            if (tLocalFolder != null) {
                try {
                    ...
                    Log.e(K9.LOG_TAG, "Could not set last checked on folder "
                            + account.getDescription() + ":" + tLocalFolder.getName(), e);
                }
            }
            ...
            addErrorMessage(account, null, e);
            Log.e(K9.LOG_TAG, "Failed synchronizing folder " + account.getDescription() + ":"
                    + folder + " @ " + new Date());
    ...
    private void updateMoreMessages(Folder remoteFolder, LocalFolder localFolder, Date earliestDate,
        ...
        if (remoteStart == 1) {
            ...
            boolean moreMessagesAvailable =
                    remoteFolder.areMoreMessagesAvailable(remoteStart, earliestDate);

            MoreMessages newMoreMessages =
                    (moreMessagesAvailable) ? MoreMessages.TRUE : MoreMessages.FALSE;
    ...
    /*
     * If the folder is a "special" folder we need to see if it exists on the remote server. It if
     * does not exist we'll try to create it. If we can't create we'll abort. This will happen on
     * every single Pop3 folder as designed and on Imap folders during error conditions. This allows
     * us to treat Pop3 and Imap the same in this code.
     */
    private boolean verifyOrCreateRemoteSpecialFolder(Account account, String folder,
            Folder remoteFolder, MessagingListener listener) throws MessagingException {
     ...
     * @param inputMessages A list of messages objects that store the UIDs of which messages to
     *        download.
     * @param flagSyncOnly Only flags will be fetched from the remote store if this is {@code true}.
     * @param purgeToVisibleLimit If true, local messages will be purged down to the limit of
     *        visible messages.
     *
    ...
    private int downloadMessages(final Account account, final Folder remoteFolder,
            ...
            boolean purgeToVisibleLimit) throws MessagingException {
        ...
        if (!unsyncedMessages.isEmpty()) {
            ...
            /*
             * Reverse the order of the messages. Depending on the server this may get us fetch
             * results for newest to oldest. If not, no harm done.
             */
            ...
            if (K9.DEBUG)
                Log.d(K9.LOG_TAG, "SYNC: About to fetch " + unsyncedMessages.size()
                        + " unsynced messages for folder " + folder);

            fetchUnsyncedMessages(account, remoteFolder, unsyncedMessages, smallMessages,
        ...
        if (K9.DEBUG)
            Log.d(K9.LOG_TAG,
                    "SYNC: Have " + largeMessages.size() + " large messages and "
                            + smallMessages.size() + " small messages out of "
                            + unsyncedMessages.size() + " unsynced messages");

        unsyncedMessages.clear();
        /*
         * Grab the content of the small messages first. This is going to be very fast and at very
         * worst will be a single up of a few bytes and a single download of 625k.
         */
        FetchProfile fp = new FetchProfile();
        ...
        downloadSmallMessages(account, remoteFolder, localFolder, smallMessages, progress,
                unreadBeforeStart, newMessages, todo, fp);
        smallMessages.clear();
        /*
        ...
        refreshLocalMessageFlags(account, remoteFolder, localFolder, syncFlagMessages, progress,
                todo);

        if (K9.DEBUG)
            Log.d(K9.LOG_TAG, "SYNC: Synced remote messages for folder " + folder + ", "
        ...
        // this would be a little wrong for IMAP (we'd want a folder-level pref, not an account
        // level pref.)
    ...
    private void evaluateMessageForDownload(final Message message, final String folder,
            ...
            final List<Message> unsyncedMessages, final List<Message> syncFlagMessages,
        ...
        if (localMessage == null) {
            ...
            if (K9.DEBUG)
                Log.v(K9.LOG_TAG,
                        "Message with uid " + message.getUid() + " is present in the local store");
            ...
            if (!localMessage.isSet(Flag.X_DOWNLOADED_FULL)
                ...
                String newPushState =
                        remoteFolder.getNewPushState(localFolder.getPushState(), message);
                if (newPushState != null) {
    ...
    private <T extends Message> void fetchUnsyncedMessages(final Account account,
            final Folder<T> remoteFolder, List<T> unsyncedMessages,
            final List<Message> smallMessages, final List<Message> largeMessages,
        ...
        remoteFolder.fetch(unsyncedMessages, fp, new MessageRetrievalListener<T>() {
                ...
                try {
                    if (message.isSet(Flag.DELETED) || message.olderThan(earliestDate)) {
                        if (K9.DEBUG) {
                            if (message.isSet(Flag.DELETED)) {
                                Log.v(K9.LOG_TAG,
                                        "Newly downloaded message " + account + ":" + folder + ":"
                                                + message.getUid()
                        ...
                        for (MessagingListener l : getListeners()) {
                            // TODO: This might be the source of poll count errors in the UI. Is
                            // todo always the same as ofTotal
                            l.synchronizeMailboxProgress(account, folder, progress.get(), todo);
                        }
                        return;
                    }
            ...
            public void messagesFinished(int total) {
                // FIXME this method is almost never invoked by various Stores! Don't rely on it
                // unless fixed!!
            }
    ...
    private <T extends Message> void downloadSmallMessages(final Account account,
            final Folder<T> remoteFolder, final LocalFolder localFolder, List<T> smallMessages,
            final AtomicInteger progress, final int unreadBeforeStart,
            final AtomicInteger newMessages, final int todo, FetchProfile fp)
            throws MessagingException {
        final String folder = remoteFolder.getName();
        ...
        if (K9.DEBUG)
            Log.d(K9.LOG_TAG, "SYNC: Fetching " + smallMessages.size()
                    + " small messages for folder " + folder);
            ...
            public void messageFinished(final T message, int number, int ofTotal) {
                try {
                    ...
                    final LocalMessage localMessage =
                            localFolder.storeSmallMessage(message, new Runnable() {
                                @Override
                                public void run() {
                                    progress.incrementAndGet();
                                }
                    ...
                    if (shouldNotifyForMessage(account, localFolder, message)) {
                        // Notify with the localMessage so that we don't have to recalculate the
                        // content preview.
                        notificationController.addNewMailNotification(account, localMessage,
                                unreadBeforeStart);
                    }
    ...
    private <T extends Message> void downloadLargeMessages(final Account account,
            final Folder<T> remoteFolder, final LocalFolder localFolder, List<T> largeMessages,
            final AtomicInteger progress, final int unreadBeforeStart,
            final AtomicInteger newMessages, final int todo, FetchProfile fp)
            throws MessagingException {
        final String folder = remoteFolder.getName();
        ...
        for (T message : largeMessages) {
            ...
            if (message.getBody() == null) {
                /*
                 * The provider was unable to get the structure of the message, so we'll download a
                 * reasonable portion of the messge and mark it as incomplete so the entire thing
                 * can be downloaded later if the user wishes to download it.
                 */
                fp = new FetchProfile();
                fp.add(FetchProfile.Item.BODY_SANE);
                /*
                 * TODO a good optimization here would be to make sure that all Stores set the
                 * proper size after this fetch and compare the before and after size. If they equal
                 * we can mark this SYNCHRONIZED instead of PARTIALLY_SYNCHRONIZED
                 */
                ...
                // Certain (POP3) servers give you the whole message even when you ask for only the
                // first x Kb
                if (!message.isSet(Flag.X_DOWNLOADED_FULL)) {
                    /*
                     * Mark the message as fully downloaded if the message size is smaller than the
                     * account's autodownload size limit, otherwise mark as only a partial download.
                     * This will prevent the system from downloading the same message twice.
                     *
                ...
                /*
                 * We have a structure to deal with, from which we can pull down the parts we want
                 * to actually store. Build a list of parts we are interested in. Text parts will be
                 * downloaded right now, attachments will be left for later.
                 */
            ...
            // Send a notification of this message
            if (shouldNotifyForMessage(account, localFolder, message)) {
                // Notify with the localMessage so that we don't have to recalculate the content
                // preview.
                notificationController.addNewMailNotification(account, localMessage,
                        unreadBeforeStart);
            }
    ...
    private void refreshLocalMessageFlags(final Account account, final Folder remoteFolder,
            final LocalFolder localFolder, List<Message> syncFlagMessages,
    ...
    private String getRootCauseMessage(Throwable t) {
        ...
        if (rootCause instanceof MessagingException) {
            ...
            // Remove the namespace on the exception so we have a fighting chance of seeing more of
            // the error in the
            // notification.
        ...
        putBackground("processPendingCommands", null, new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    /*
                     * Ignore any exceptions from the commands. Commands will be processed on the
                     * next round.
                     */
                }
            }
    ...
    private void processPendingCommandsSynchronous(Account account) throws MessagingException {
        ...
        try {
            for (PendingCommand command : commands) {
                ...
                /*
                 * We specifically do not catch any exceptions here. If a command fails it is most
                 * likely due to a server or IO error and it must be retried before any other
                 * command processes. This maintains the order of the commands.
                 */
                try {
     ...
     * the local message is successfully processed it is deleted so that the server message will be
     * synchronized down without an additional copy being created. TODO update the local message UID
        ...
        try {
            ...
            if (remoteMessage == null) {
                if (localMessage.isSet(Flag.X_REMOTE_COPY_STARTED)) {
                    Log.w(K9.LOG_TAG,
                            "Local message with uid " + localMessage.getUid() + " has flag "
                ...
                /*
                 * If the message does not exist remotely we just upload it and then update our
                 * local copy with the new uid.
                 */
                FetchProfile fp = new FetchProfile();
                fp.add(FetchProfile.Item.BODY);
    ...
    private void queueMoveOrCopy(Account account, String srcFolder, String destFolder,
        ...
        if (uidMap == null || uidMap.isEmpty()) {
            ...
            System.arraycopy(uidMap.keySet().toArray(EMPTY_STRING_ARRAY), 0, command.arguments, 4,
                    uidMap.keySet().size());
            System.arraycopy(uidMap.values().toArray(EMPTY_STRING_ARRAY), 0, command.arguments,
     ...
     *
     * <p>
     * TODO: This method is obsolete and is only for transition from K-9 4.0 to K-9 4.2 Eventually,
     * it should be removed.
     * </p>
     *
     * @param command Pending move/copy command in old format.
     * @param account The account the pending command belongs to.
     *
    ...
    private void processPendingMoveOrCopy(PendingCommand command, Account account)
        ...
        try {
            ...
            /*
             * We split up the localUidMap into two parts while sending the command, here we
             * assemble it back.
             */
            ...
            if (remoteSrcFolder.getMode() != Folder.OPEN_MODE_RW) {
                throw new MessagingException(
                        "processingPendingMoveOrCopy: could not open remoteSrcFolder " + srcFolder
                                + " read/write",
                        true);
            }
            ...
            if (K9.DEBUG)
                Log.d(K9.LOG_TAG,
                        "processingPendingMoveOrCopy: source folder = " + srcFolder + ", "
            ...
            /*
             * This next part is used to bring the local UIDs of the local destination folder upto
             * speed with the remote UIDs of remote destination folder.
             */
    ...
    private void processPendingMoveOrCopyOld(PendingCommand command, Account account)
        ...
        if (!remoteSrcFolder.exists()) {
            throw new MessagingException(
                    "processPendingMoveOrCopyOld: remoteFolder " + srcFolder + " does not exist",
                    true);
        }
        remoteSrcFolder.open(Folder.OPEN_MODE_RW);
        if (remoteSrcFolder.getMode() != Folder.OPEN_MODE_RW) {
            throw new MessagingException(
                    "processPendingMoveOrCopyOld: could not open remoteSrcFolder " + srcFolder
                            + " read/write",
                    true);
        }
        ...
        if (remoteMessage == null) {
            throw new MessagingException(
                    "processPendingMoveOrCopyOld: remoteMessage " + uid + " does not exist", true);
        }

        if (K9.DEBUG)
            Log.d(K9.LOG_TAG,
                    "processPendingMoveOrCopyOld: source folder = " + srcFolder + ", uid = " + uid
        ...
        if (remoteDestFolder.getMode() != Folder.OPEN_MODE_RW) {
            throw new MessagingException(
                    "processPendingMoveOrCopyOld: could not open remoteDestFolder " + srcFolder
                            + " read/write",
                    true);
        }

        if (isCopy) {
            remoteSrcFolder.copyMessages(Collections.singletonList(remoteMessage),
                    remoteDestFolder);
        } else {
            remoteSrcFolder.moveMessages(Collections.singletonList(remoteMessage),
                    remoteDestFolder);
        }
        remoteSrcFolder.close();
    ...
    void addErrorMessage(Account account, String subject, Throwable t) {
        try {
            ...
            Log.e(K9.LOG_TAG, "Could not save error message to " + account.getErrorFolderName(),
                    it);
        }
    }
    ...
    private void addErrorMessage(Account account, String subject, String body) {
        ...
        try {
            ...
            LocalFolder localFolder =
                    (LocalFolder) localStore.getFolder(account.getErrorFolderName());
    ...
    public void markAllMessagesRead(final Account account, final String folder) {
        ...
        if (K9.DEBUG) {
            Log.i(K9.LOG_TAG, "Marking all messages in " + account.getDescription() + ":" + folder
                    + " as read");
        }
        PendingCommand command = new PendingCommand();
    ...
    private void setFlagSynchronous(final Account account, final List<Long> ids, final Flag flag,
     ...
     * @param newState {@code true}, if the flag should be set. {@code false} if it should be
     *        removed.
     */
    public void setFlag(Account account, String folderName, List<? extends Message> messages,
    ...
    public void loadMessageRemote(final Account account, final String folder, final String uid,
            final MessagingListener listener) {
        ...
        try {
            ...
            if (uid.startsWith(K9.LOCAL_UID_PREFIX)) {
                ...
                android.widget.Toast
                        .makeText(context, "Message has local UID so cannot download fully",
            ...
            /*
             * commented out because this was pulled from another unmerged branch: } else if
             * (localFolder.isLocalOnly() && !force) { Log.w(K9.LOG_TAG,
             * "Message in local-only folder so cannot download fully."); // ASH move toast
             * android.widget.Toast.makeText(mApplication,
             * "Message in local-only folder so cannot download fully",
             * android.widget.Toast.LENGTH_LONG).show(); message.setFlag(Flag.X_DOWNLOADED_FULL,
             * true); message.setFlag(Flag.X_DOWNLOADED_PARTIAL, false); }
             */

            /* if (!message.isSet(Flag.X_DOWNLOADED_FULL)) */ {
                /*
                 * At this point the message is not available, so we need to download it fully if
                 * possible.
                 */
    ...
    /**
     * Stores the given message in the Outbox and starts a sendPendingMessages command to attempt to
     * send the message.
     */
    ...
    private void sendPendingMessagesSynchronous(final Account account) {
        ...
        try {
            ...
            /*
             * The profile we will use to pull all of the content for a given local message into
             * memory for sending.
             */
            FetchProfile fp = new FetchProfile();
            fp.add(FetchProfile.Item.ENVELOPE);
            ...
            for (LocalMessage message : localMessages) {
                ...
                try {
                    ...
                    if (K9.DEBUG)
                        Log.i(K9.LOG_TAG, "Send count for message " + message.getUid() + " is "
                                + count.get());
                    ...
                    try {
                        ...
                        progress++;
                        for (MessagingListener l : getListeners()) {
                            l.synchronizeMailboxProgress(account, account.getSentFolderName(),
                                    progress, todo);
                        }
                        if (!account.hasSentFolder()) {
                            ...
                            LocalFolder localSentFolder =
                                    localStore.getFolder(account.getSentFolderName());
                            if (K9.DEBUG)
                                Log.i(K9.LOG_TAG,
                            ...
                            localFolder.moveMessages(Collections.singletonList(message),
                                    localSentFolder);
                        ...
                        handleSendFailure(account, localStore, localFolder, message, e,
                                wasPermanentFailure);
    ...
    private void moveMessageToDraftsFolder(Account account, Folder localFolder, Store localStore,
    ...
    private void notifySynchronizeMailboxFailed(Account account, Folder localFolder,
            Exception exception) {
        ...
        threadPool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Log.e(K9.LOG_TAG,
                            "Count not get unread count for account " + account.getDescription(),
                            me);
                }
    ...
    public void moveMessages(final Account srcAccount, final String srcFolder,
        ...
        actOnMessageGroup(srcAccount, srcFolder, messageReferences, new MessageActor() {
            @Override
            public void act(final Account account, LocalFolder messageFolder,
                ...
                putBackground("moveMessages", null, new Runnable() {
                    @Override
                    public void run() {
                        moveOrCopyMessageSynchronous(account, srcFolder, messages, destFolder,
                                false);
                    }
                });
            }
        ...
        actOnMessageGroup(srcAccount, srcFolder, messageReferences, new MessageActor() {
            ...
            public void act(final Account account, LocalFolder messageFolder,
                ...
                putBackground("moveMessagesInThread", null, new Runnable() {
                    ...
                    public void run() {
                        try {
                            List<Message> messagesInThreads =
                                    collectMessagesInThreads(account, messages);
                            moveOrCopyMessageSynchronous(account, srcFolder, messagesInThreads,
                                    destFolder, false);
                        } catch (MessagingException e) {
    ...
    public void moveMessage(final Account account, final String srcFolder,
            final MessageReference message, final String destFolder) {
    ...
    private void moveOrCopyMessageSynchronous(final Account account, final String srcFolder,
        ...
        try {
            ...
            if (messages.size() > 0) {
                ...
                if (K9.DEBUG)
                    Log.i(K9.LOG_TAG,
                            "moveOrCopyMessageSynchronous: source folder = " + srcFolder + ", "
                                    + messages.size() + " messages, " + ", destination folder = "
                ...
                queueMoveOrCopy(account, srcFolder, destFolder, isCopy,
                        origUidKeys.toArray(new String[origUidKeys.size()]), uidMap);
            }
        ...
        try {
            ...
            if (uid != null) {
                MessageReference messageReference = new MessageReference(account.getUuid(),
                        account.getDraftsFolderName(), uid, null);
    ...
    private void deleteThreadsSynchronous(Account account, String folderName,
    ...
    private static List<Message> collectMessagesInThreads(Account account,
            List<? extends Message> messages) throws MessagingException {
        ...
        actOnMessagesGroupedByAccountAndFolder(messages, new MessageActor() {
            ...
            public void act(final Account account, final LocalFolder messageFolder,
                ...
                putBackground("deleteMessages", null, new Runnable() {
                    @Override
                    public void run() {
                        deleteMessagesSynchronous(account, messageFolder.getName(), accountMessages,
                                listener);
                    }
                });
            }
                    ...
                    public void run() {
                        for (LocalMessage message : accountMessages) {
                            try {
                                ...
                                throw new AssertionError("clearing local message content failed!",
                                        e);
                            }
                        }
                    }
    ...
    private void deleteMessagesSynchronous(final Account account, final String folder,
        ...
        try {
            ...
            if (K9.DEBUG)
                Log.d(K9.LOG_TAG, "Delete policy for account " + account.getDescription() + " is "
                        + account.getDeletePolicy());

            if (folder.equals(account.getOutboxFolderName())) {
                for (Message message : messages) {
                    // If the message was in the Outbox, then it has been copied to local Trash, and
                    // has
                    // to be copied to remote trash
                    PendingCommand command = new PendingCommand();
                    command.command = PENDING_COMMAND_APPEND;
                    command.arguments =
                            new String[] {account.getTrashFolderName(), message.getUid()};
                    queuePendingCommand(account, command);
                }
                processPendingCommands(account);
            } else if (account.getDeletePolicy() == DeletePolicy.ON_DELETE) {
                if (folder.equals(account.getTrashFolderName())) {
                    queueSetFlag(account, folder, Boolean.toString(true), Flag.DELETED.toString(),
                            uids);
                } else {
                ...
                if (K9.DEBUG)
                    Log.d(K9.LOG_TAG, "Delete policy " + account.getDeletePolicy()
    ...
    private void processPendingEmptyTrash(PendingCommand command, Account account)
            throws MessagingException {
        Store remoteStore = account.getRemoteStore();
    ...
    public void checkMail(final Context context, final Account account,
            final boolean ignoreLastCheckedTime, final boolean useManualWakeLock,
            final MessagingListener listener) {
    ...
    private void checkMailForAccount(final Context context, final Account account,
            final boolean ignoreLastCheckedTime, final MessagingListener listener) {
        if (!account.isAvailable(context)) {
            if (K9.DEBUG) {
                Log.i(K9.LOG_TAG,
        ...
        try {
            ...
            for (final Folder folder : localStore.getPersonalNamespaces(false)) {
                ...
                if (modeMismatch(aDisplayMode, fDisplayClass)) {
                    ...
                    /*
                     * if (K9.DEBUG) Log.v(K9.LOG_TAG, "Not syncing folder " + folder.getName() +
                ...
                if (modeMismatch(aSyncMode, fSyncClass)) {
                    ...
                    /*
                     * if (K9.DEBUG) Log.v(K9.LOG_TAG, "Not syncing folder " + folder.getName() +
                     * " which is in sync mode " + fSyncClass + " while account is in sync mode " +
                     * aSyncMode);
                     */

                    continue;
                }
                synchronizeFolder(account, folder, ignoreLastCheckedTime, accountInterval,
                        listener);
            }
        } catch (MessagingException e) {
            ...
            putBackground("clear notification flag for " + account.getDescription(), null,
                    new Runnable() {
                        ...
                        public void run() {
                            if (K9.DEBUG)
                                Log.v(K9.LOG_TAG, "Clearing notification flag for "
                                        + account.getDescription());
                            account.setRingNotified(false);
                            try {
                                AccountStats stats = account.getStats(context);
        ...
        putBackground("sync" + folder.getName(), null, new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (!ignoreLastCheckedTime && tLocalFolder
                            .getLastChecked() > (System.currentTimeMillis() - accountInterval)) {
                        if (K9.DEBUG)
    ...
    private boolean shouldNotifyForMessage(Account account, LocalFolder localFolder,
            Message message) {
        ...
        try {
            ...
            for (final Folder folder : localStore.getPersonalNamespaces(false)) {
                if (folder.getName().equals(account.getErrorFolderName())
                    ...
                    /*
                     * if (K9.DEBUG) Log.v(K9.LOG_TAG, "Not pushing folder " + folder.getName() +
                ...
                if (modeMismatch(aPushMode, fPushClass)) {
                    ...
                    /*
                     * if (K9.DEBUG) Log.v(K9.LOG_TAG, "Not pushing folder " + folder.getName() +
                     * " which is in push mode " + fPushClass + " while account is in push mode " +
                     * aPushMode);
                     */
                ...
                if (K9.DEBUG)
                    Log.i(K9.LOG_TAG, "Starting pusher for " + account.getDescription() + ":"
                            + folder.getName());
            ...
            if (!names.isEmpty()) {
                ...
                if (names.size() > maxPushFolders) {
                    if (K9.DEBUG)
                        Log.i(K9.LOG_TAG,
                                "Count of folders to push for account " + account.getDescription()
                                        + " is " + names.size() + ", greater than limit of "
                ...
                try {
                    ...
                    if (!store.isPushCapable()) {
                        if (K9.DEBUG)
                            Log.i(K9.LOG_TAG, "Account " + account.getDescription()
                ...
                if (K9.DEBUG)
                    Log.i(K9.LOG_TAG, "No folders are configured for pushing in account "
                            + account.getDescription());
                return false;
            }
    ...
    public void messagesArrived(final Account account, final Folder remoteFolder,
        ...
        if (K9.DEBUG)
            Log.i(K9.LOG_TAG, "Got new pushed email messages for account "
                    + account.getDescription() + ", folder " + remoteFolder.getName());
                ...
                + remoteFolder.getName(), null, new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            account.setRingNotified(false);
                            int newCount = downloadMessages(account, remoteFolder, localFolder,
                                    messages, flagSyncOnly, true);
                            ...
                            for (MessagingListener l : getListeners()) {
                                l.synchronizeMailboxFailed(account, remoteFolder.getName(),
                                        errorMessage);
                            }
                            addErrorMessage(account, null, e);
                        } finally {
    ...
    public void notifyUserIfCertificateProblem(Account account, Exception exception,
            boolean incoming) {
    ...
    private void actOnMessagesGroupedByAccountAndFolder(List<MessageReference> messages,
            MessageActor actor) {
        Map<String, Map<String, List<MessageReference>>> accountMap =
                groupMessagesByAccountAndFolder(messages);
    ...
    private void actOnMessageGroup(Account account, String folderName,
        ...
        try {
            ...
            List<LocalMessage> localMessages =
                    messageFolder.getMessagesByReference(messageReferences);
