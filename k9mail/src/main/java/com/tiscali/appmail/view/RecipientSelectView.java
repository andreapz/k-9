...
import com.tiscali.appmail.activity.AlternateRecipientAdapter;
import com.tiscali.appmail.activity.AlternateRecipientAdapter.AlternateRecipientListener;
import com.tiscali.appmail.activity.compose.RecipientAdapter;
import com.tiscali.appmail.activity.compose.RecipientLoader;
...
public class RecipientSelectView extends TokenCompleteTextView<Recipient>
    ...
    private void bindObjectView(Recipient recipient, View view) {
        ...
        RecipientAdapter.setContactPhotoOrPlaceholder(getContext(), holder.vContactPhoto,
                recipient);
        ...
        switch (id) {
            ...
            case LOADER_ID_ALTERNATES: {
                ...
                if (contactLookupUri != null) {
                    ...
                    return new RecipientLoader(getContext(), cryptoProvider,
    ...
    public boolean hasUncompletedText() {
        ...
        return !TextUtils.isEmpty(currentCompletionText)
                && !isPlaceholderText(currentCompletionText);
    }

    static private boolean isPlaceholderText(String currentCompletionText) {
        // TODO string matching here is sort of a hack, but it's somewhat reliable and the info
        // isn't easily available
        return currentCompletionText.startsWith("+")
                && currentCompletionText.substring(1).matches("[0-9]+");
    }
     ...
     * functionality, but using the custom RecipientTokenSpan class which allows us to retrieve the
     * view for redrawing at a later point.
     */
    @Override
    ...
    /**
     * Find the token view tied to a given recipient. This method relies on spans to be of the
     * RecipientTokenSpan class, as created by the buildSpanForObject method.
     */
    private View getTokenViewForRecipient(Recipient currentRecipient) {
        ...
        RecipientTokenSpan[] recipientSpans =
                text.getSpans(0, text.length(), RecipientTokenSpan.class);
    ...
    /**
     * We use a specialized version of TokenCompleteTextView.TokenListener as well, adding a
     * callback for onTokenChanged.
     */
    ...
    public static class Recipient implements Serializable {
        ...
        @Nullable // null if the contact has no photo. transient because we serialize this manually,
                  // see below.
