...
/**
 * This OutputStream is modelled after apache commons'
 * {@link org.apache.commons.io.output.DeferredFileOutputStream}, but uses a {@link FileFactory}
 * instead of directly generating temporary files itself.
 */
public class DeferredFileOutputStream extends ThresholdingOutputStream {
    ...
    public DeferredFileOutputStream(int threshold, FileFactory fileFactory) {
        ...
        // scale it so we expand the ByteArrayOutputStream at most three times (assuming quadratic
        // growth)
    ...
    protected void thresholdReached() throws IOException {
        if (outputFile != null) {
            throw new IllegalStateException(
                    "thresholdReached must not be called if we already have an output file!");
        }
        if (!isMemoryBacked()) {
            throw new IllegalStateException(
                    "currentOutputStream must be memory-based at this point!");
        }
