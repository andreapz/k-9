...
import com.tiscali.appmail.helper.HtmlSanitizer;
...
import com.tiscali.appmail.mail.internet.Viewable;
...
import com.tiscali.appmail.ui.crypto.MessageCryptoSplitter;
import com.tiscali.appmail.ui.crypto.MessageCryptoSplitter.CryptoMessageParts;
...
public class MessageViewInfoExtractor {
    ...
    @WorkerThread
    public MessageViewInfo extractMessageForView(Message message,
        ...
        ViewableExtractedText viewable =
                extractViewableAndAttachments(Collections.singletonList(rootPart), attachmentInfos);
        ...
        return MessageViewInfo.createWithExtractedContent(message, isMessageIncomplete, rootPart,
                viewable.html, attachmentInfos, cryptoResultAnnotation, attachmentResolver,
     ...
     *
     * @return A {@link ViewableExtractedText} instance containing the textual parts of the message
     *         as plain text and HTML, and a list of message parts considered attachments.
     *
     * @throws com.tiscali.appmail.mail.MessagingException In case of an error.
     */
    @VisibleForTesting
        ...
        try {
            ...
            for (Viewable viewable : viewables) {
                if (viewable instanceof Textual) {
                    ...
                    /*
                     * We made sure at least one of text/plain or text/html is present when creating
                     * the Alternative object. If one part is not present we convert the other one
                     * to make sure 'text' and 'html' always contain the same text.
                     */
    ...
    /**
     * Use the contents of a {@link com.tiscali.appmail.mail.internet.Viewable} to create the HTML
     * to be displayed.
     *
     * <p>
     * This will use {@link com.tiscali.appmail.helper.HtmlConverter#textToHtml(String)} to convert
     * plain text parts to HTML if necessary.
     * </p>
     *
    ...
    private StringBuilder buildHtml(Viewable viewable, boolean prependDivider) {
        ...
        if (viewable instanceof Textual) {
            ...
            List<Viewable> htmlAlternative =
                    alternative.getHtml().isEmpty() ? alternative.getText() : alternative.getHtml();
    ...
    private StringBuilder buildText(Viewable viewable, boolean prependDivider) {
        ...
        if (viewable instanceof Textual) {
            ...
            List<Viewable> textAlternative =
                    alternative.getText().isEmpty() ? alternative.getHtml() : alternative.getText();
     ...
     *        part's name.
     * @param prependDivider {@code true}, if the divider should be appended. {@code false},
     *        otherwise.
     */
    private void addHtmlDivider(StringBuilder html, Part part, boolean prependDivider) {
        if (prependDivider) {
            ...
            html.append(
                    "<p style=\"margin-top: 2.5em; margin-bottom: 1em; border-bottom: 1px solid #000\">");
    ...
    private void addTextDivider(StringBuilder text, Part part, boolean prependDivider) {
        if (prependDivider) {
            ...
            if (len > 0) {
                ...
                text.append(TEXT_DIVIDER.substring(0, TEXT_DIVIDER_LENGTH - FILENAME_PREFIX_LENGTH
                        - filename.length() - FILENAME_SUFFIX_LENGTH));
            } else {
