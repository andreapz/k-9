...
class MigrationTo51 {
     ...
     *
     * This is a complex migration, and ultimately we do not have enough information to recreate the
     * mime structure of the original mails. What we have: - general mail info - html_content and
     * text_content data, which is the squashed readable content of the mail - a table with message
     * headers - attachments
     *
     * What we need to do: - migrate general mail info as-is - flag mails as migrated for
     * re-download - for each message, recreate a mime structure from its message content and
     * attachments: + insert one or both of textContent and htmlContent, depending on mimeType + if
     * mimeType is text/plain, text/html or multipart/alternative and no attachments are present,
     * just insert that. + otherwise, use multipart/mixed, adding attachments after textual content
     * + revert content:// URIs in htmlContent to original cid: URIs.
     */
    public static void db51MigrateMessageFormat(SQLiteDatabase db,
        ...
        attachmentDirNew = StorageManager.getInstance(K9.app)
                .getAttachmentDirectory(account.getUuid(), account.getLocalStorageProviderId());
        attachmentDirOld = renameOldAttachmentDirAndCreateNew(account, attachmentDirNew);

        Cursor msgCursor = db.query("messages_old", new String[] {"id", "flags", "html_content",
                "text_content", "mime_type", "attachment_count"}, null, null, null, null, null);
        try {
            ...
            while (msgCursor.moveToNext()) {
                ...
                try {
                    ...
                    // we do not rely on the protocol parameter here but guess by the multipart
                    // structure
                    ...
                    if (isMaybePgpMimeEncrypted) {
                        MimeStructureState maybeStructureState =
                                migratePgpMimeEncryptedContent(db, messageId, attachmentDirOld,
                                        attachmentDirNew, mimeHeader, structureState);
                    ...
                    if (!messageHadSpecialFormat) {
                        boolean isSimpleStructured =
                                attachmentCount == 0 && Utility.isAnyMimeType(mimeType,
                        ...
                        if (isSimpleStructured) {
                            ...
                            structureState = migrateComplexMailContent(db, attachmentDirOld,
                                    attachmentDirNew, messageId, htmlContent, textContent,
    ...
    private static void cleanUpOldAttachmentDirectory(File attachmentDirOld) {
        for (File file : attachmentDirOld.listFiles()) {
            ...
            if (file.exists() && !file.delete()) {
                Log.d(K9.LOG_TAG,
                        "Failed to delete stale attachement file: " + file.getAbsolutePath());
            }
        }
    ...
    private static void copyMessageMetadataToNewTable(SQLiteDatabase db) {
        db.execSQL("INSERT INTO messages ("
                ...
                + "normalized_subject_hash, empty, read, flagged, answered" + ") SELECT "
                + "id, deleted, folder_id, uid, subject, date, sender_list, "
                + "to_list, cc_list, bcc_list, reply_to_list, attachment_count, "
                + "internal_date, message_id, preview, mime_type, "
                + "normalized_subject_hash, empty, read, flagged, answered " + "FROM messages_old");
    }
    ...
    private static void renameOldMessagesTableAndCreateNew(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE messages (" + "id INTEGER PRIMARY KEY, "
                + "deleted INTEGER default 0, " + "folder_id INTEGER, " + "uid TEXT, "
                + "subject TEXT, " + "date INTEGER, " + "flags TEXT, " + "sender_list TEXT, "
                + "to_list TEXT, " + "cc_list TEXT, " + "bcc_list TEXT, " + "reply_to_list TEXT, "
                + "attachment_count INTEGER, " + "internal_date INTEGER, " + "message_id TEXT, "
                + "preview TEXT, " + "mime_type TEXT, " + "normalized_subject_hash INTEGER, "
                + "empty INTEGER default 0, " + "read INTEGER default 0, "
                + "flagged INTEGER default 0, " + "answered INTEGER default 0, "
                + "forwarded INTEGER default 0, " + "message_part_id INTEGER" + ")");

        db.execSQL("CREATE TABLE message_parts (" + "id INTEGER PRIMARY KEY, "
                + "type INTEGER NOT NULL, " + "root INTEGER, " + "parent INTEGER NOT NULL, "
                + "seq INTEGER NOT NULL, " + "mime_type TEXT, " + "decoded_body_size INTEGER, "
                + "display_name TEXT, " + "header TEXT, " + "encoding TEXT, " + "charset TEXT, "
                + "data_location INTEGER NOT NULL, " + "data BLOB, " + "preamble TEXT, "
                + "epilogue TEXT, " + "boundary TEXT, " + "content_id TEXT, " + "server_extra TEXT"
                + ")");

        db.execSQL("CREATE TRIGGER set_message_part_root " + "AFTER INSERT ON message_parts "
                + "BEGIN "
    ...
    @Nullable
    private static MimeStructureState migratePgpMimeEncryptedContent(SQLiteDatabase db,
            long messageId, File attachmentDirOld, File attachmentDirNew, MimeHeader mimeHeader,
        ...
        Cursor cursor = db.query("attachments",
                new String[] {"id", "size", "name", "mime_type", "store_data", "content_uri",
        ...
        try {
            if (cursor.getCount() != 2) {
                Log.e(K9.LOG_TAG,
                        "Found multipart/encrypted but bad number of attachments, handling as regular mail");
                return null;
            }
            ...
            mimeHeader.setHeader(MimeHeader.HEADER_CONTENT_TYPE,
                    String.format(
                            "multipart/encrypted; boundary=\"%s\"; protocol=\"application/pgp-encrypted\"",
                            boundary));
            ...
            structureState =
                    insertMimeAttachmentPart(db, attachmentDirOld, attachmentDirNew, structureState,
                            firstPartId, firstPartSize, firstPartName, "application/pgp-encrypted",
                            firstPartStoreData, firstPartContentUriString, null, null);

            structureState = insertMimeAttachmentPart(db, attachmentDirOld, attachmentDirNew,
                    structureState, secondPartId, secondPartSize, secondPartName,
                    "application/octet-stream", secondPartStoreData, secondPartContentUriString,
                    null, null);
        } finally {
            cursor.close();
        }
    ...
    private static MimeStructureState migrateComplexMailContent(SQLiteDatabase db,
            File attachmentDirOld, File attachmentDirNew, long messageId, String htmlContent,
            String textContent, MimeHeader mimeHeader, MimeStructureState structureState)
            throws IOException {
        ...
        if (textContent != null && htmlContent != null) {
            structureState = insertBodyAsMultipartAlternative(db, structureState, null, textContent,
                    htmlContent);
            structureState = structureState.popParent();
        } else if (textContent != null) {
            structureState =
                    insertTextualPartIntoDatabase(db, structureState, null, textContent, false);
        } else if (htmlContent != null) {
            structureState =
                    insertTextualPartIntoDatabase(db, structureState, null, htmlContent, true);
        }

        structureState = insertAttachments(db, attachmentDirOld, attachmentDirNew, messageId,
                structureState);

        return structureState;
    }

    private static String replaceContentUriWithContentIdInHtmlPart(SQLiteDatabase db,
            long messageId, String htmlContent) {
        Cursor cursor = db.query("attachments", new String[] {"content_uri", "content_id"},
                "content_id IS NOT NULL AND message_id = ?",
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                htmlContent =
                        htmlContent.replaceAll(Pattern.quote(contentUriString), "cid:" + contentId);
            }
        } finally {
            cursor.close();
        }
    ...
    private static MimeStructureState migrateSimpleMailContent(SQLiteDatabase db,
            String htmlContent, String textContent, String mimeType, MimeHeader mimeHeader,
        ...
        if (MimeUtil.isSameMimeType(mimeType, "text/plain")) {
            return insertTextualPartIntoDatabase(db, structureState, mimeHeader, textContent,
                    false);
            ...
            return insertBodyAsMultipartAlternative(db, structureState, mimeHeader, textContent,
                    htmlContent);
        } else {
            throw new IllegalStateException(
                    "migrateSimpleMailContent cannot handle mimeType " + mimeType);
        }
    }

    private static MimeStructureState insertAttachments(SQLiteDatabase db, File attachmentDirOld,
            File attachmentDirNew, long messageId, MimeStructureState structureState) {
        Cursor cursor = db.query("attachments",
                new String[] {"id", "size", "name", "mime_type", "store_data", "content_uri",
                        "content_id", "content_disposition"},
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                structureState = insertMimeAttachmentPart(db, attachmentDirOld, attachmentDirNew,
                        structureState, id, size, name, mimeType, storeData, contentUriString,
                        contentId, contentDisposition);
    ...
    private static MimeStructureState insertMimeAttachmentPart(SQLiteDatabase db,
            File attachmentDirOld, File attachmentDirNew, MimeStructureState structureState,
            long id, int size, String name, String mimeType, String storeData,
            String contentUriString, String contentId, String contentDisposition) {
        if (K9.DEBUG) {
            Log.d(K9.LOG_TAG, "processing attachment " + id + ", " + name + ", " + mimeType + ", "
                    + storeData + ", " + contentUriString);
        }
        ...
        mimeHeader.setHeader(MimeHeader.HEADER_CONTENT_DISPOSITION, String.format(Locale.US,
                "%s;\r\n filename=\"%s\";\r\n size=%d", contentDisposition, name, size)); // TODO:
                                                                                          // Should
                                                                                          // use
                                                                                          // encoded
                                                                                          // word
        ...
        boolean hasContentTypeAndIsInline =
                !TextUtils.isEmpty(contentId) && "inline".equalsIgnoreCase(contentDisposition);
        int messageType = hasContentTypeAndIsInline ? MESSAGE_PART_TYPE__HIDDEN_ATTACHMENT
                : MESSAGE_PART_TYPE__UNKNOWN;
        ...
        cv.put("data_location",
                attachmentFileToMove != null ? DATA_LOCATION__ON_DISK : DATA_LOCATION__MISSING);
        ...
        if (attachmentFileToMove != null) {
            boolean moveOk = attachmentFileToMove
                    .renameTo(new File(attachmentDirNew, Long.toString(partId)));
            if (!moveOk) {
    ...
    private static void updateFlagsForMessage(SQLiteDatabase db, long messageId,
            String messageFlags, MigrationsHelper migrationsHelper) {
        ...
        db.execSQL("UPDATE messages SET flags = ? WHERE id = ?",
                new Object[] {flagsString, messageId});
    }

    private static MimeStructureState insertBodyAsMultipartAlternative(SQLiteDatabase db,
            MimeStructureState structureState, MimeHeader mimeHeader, String textContent,
        ...
        int dataLocation = textContent != null || htmlContent != null ? DATA_LOCATION__IN_DATABASE
                : DATA_LOCATION__MISSING;
    ...
    private static MimeStructureState insertTextualPartIntoDatabase(SQLiteDatabase db,
            MimeStructureState structureState, MimeHeader mimeHeader, String content,
        ...
        mimeHeader.setHeader(MimeHeader.HEADER_CONTENT_TRANSFER_ENCODING,
    ...
    private static MimeHeader loadHeaderFromHeadersTable(SQLiteDatabase db, long messageId) {
        Cursor headersCursor = db.query("headers", new String[] {"name", "value"}, "message_id = ?",
                new String[] {Long.toString(messageId)}, null, null, null);
        try {
            MimeHeader mimeHeader = new MimeHeader();
    ...
    /**
     * Objects of this class hold immutable information on a database position for one part of the
     * mime structure of a message.
     *
     * An object of this class must be passed to and returned by every operation which inserts mime
     * parts into the database. Each mime part which is inserted must call the {#applyValues()}
     * method on its ContentValues, then obtain the next state object by calling the appropriate
     * next*() method.
     *
     * While the data carried by this object is immutable, it contains some state to ensure that the
     * operations are called correctly and in order.
     *
     * Because the insertion operations required for the database migration are strictly linear, we
     * do not require a more complex stack-based data structure here.
     */
    @VisibleForTesting
    static class MimeStructureState {
        ...
        private MimeStructureState(Long rootPartId, Long prevParentId, long parentId,
                int nextOrder) {
        ...
        public void applyValues(ContentValues cv) {
            if (isValuesApplied || isStateAdvanced) {
                throw new IllegalStateException(
                        "applyValues must be called exactly once, after a call to next*");
            }
            if (rootPartId != null && parentId == -1L) {
                throw new IllegalStateException(
                        "applyValues must not be called after a root nextChild call");
            }
        ...
        public MimeStructureState popParent() {
            if (prevParentId == null) {
                throw new IllegalStateException(
                        "popParent must only be called if parent depth is >= 2");
            }
